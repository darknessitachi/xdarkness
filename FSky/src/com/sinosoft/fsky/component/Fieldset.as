package com.sinosoft.fsky.component
{
	...
	[IconFile("/assets/images/cmp.gif")]
	 ...
	 * @date April 1st, 2009
	 *
	 * Updated by Robertus Lilik Haryanto on May 27th, 2010
	 */
	public class Fieldset extends Container implements IConstraintLayout, IFontContextComponent {
		...
		public function Fieldset() {
			...
			myStyleManager = StyleManager.getStyleManager(moduleFactory);
			style = myStyleManager.getStyleDeclaration("TitledBorderBox");
			if (!style) {
			...
			myStyleManager.setStyleDeclaration("TitledBorderBox", style, true);
		}
		...
		public function set borderDropShadow(dropShadow:Boolean):void {
			if (dropShadow != _borderDropShadow) {
				...
				if (border) {
					border.filters = (dropShadow ? [ new DropShadowFilter(2, 45, 0xDADADA, 0.4) ] : []);
				}
		...
		protected function createTitleTextField(childIndex:int = -1):void {
			...
			if (!titleTextField) {
				titleTextField = new Label();///IUITextField(createInFontContext(UITextField));
				...
				titleTextField.setStyle("fontSize", 14);
				titleTextField.x = 15;
				titleTextField.y = 1;
			}
		}
		...
		override protected function createBorder():void {
			if (!border && isBorderNeeded()) {
				...
				border.filters = (borderDropShadow ? [ new DropShadowFilter(2, 45, 0xDADADA, 0.4) ] : []);
		...
		private function measureTitleText():Rectangle {
			...
			if (titleTextField && titleTextField.text) {
				...
				textWidth = titleTextField.width;//metrics.width;
				textHeight = titleTextField.height;//metrics.height;
			}
		...
		protected function sizeTitleTextField(w:Number, h:Number):void {
			if (titleChanged) {
				...
				titleTextField.setActualSize(measuredW, measuredH); titleChanged = false;
			}
		}
		...
		protected function drawBorder(w:Number, h:Number):void {
			if (border) {
				...
				var borderColor:uint = uint(getNumberStyle("borderColor", 0xDADADA));
