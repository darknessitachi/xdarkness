...
import com.abigdreamer.java.net.orm.SchemaUtil;
...
public class DataCollection extends Mapx {
		...
		for (int i = 0; i < size(); i++) {
			...
			if ((value == null) || (value.equals(""))) {
				...
				for (int j = 0; j < t.length; j++) {
					...
					sb.append(XString.javaEncode(t[j]));
					sb.append("\"");
				}
	...
	protected void translatDataTableToXML(DataTable dt, Element ele, String type) {
		if (dt == null) {
			throw new RuntimeException(
					"DataCollection:生成XML时发生错误，传入的DataTable为空!");
		}
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			...
			for (int j = 0; j < dcs.length; j++) {
				...
				if (j == 0) {
					...
					sb.append("\"" + XString.javaEncode(v) + "\"");
				} else if (v == null) {
					sb.append(",");
				} else {
					sb.append(",\"" + XString.javaEncode(v) + "\"");
	...
	public static String dataTableToJS(DataTable dt) {
		if (dt == null) {
			throw new RuntimeException(
					"DataCollection:生成JS时发生错误，传入的DataTable为空!");
		}
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			...
			for (int j = 0; j < dcs.length; j++) {
				...
				if (v == null) {
					...
					String txt = XString.javaEncode(v);
	...
	public void parseXML(String xml) {
		...
		xml = XString.replaceEx(xml,
				XString.urlDecode("%C2%A0", "UTF-8"), " ");
		xml = XString.clearForXML(xml);
		...
		try {
			...
			for (int i = 0; i < elements.size(); i++) {
				...
				if (type.equals("String")) {
					...
					String[] t = str.split("\",\"");
					for (int j = 0; j < t.length; j++) {
						t[j] = XString.javaDecode(t[j]);
					}
					t[0] = t[0].substring(1);
					t[(t.length - 1)] = t[(t.length - 1)].substring(0,
							t[(t.length - 1)].length() - 2);
					put(id, t);
					...
					Class cSchema = Class.forName(ele
	...
	private Mapx parseMap(Element ele) {
		...
		String[] arr = XString.splitEx(str, ",\"");
		for (int i = 0; i < arr.length; i++) {
			String[] arr2 = XString.splitEx(str, "\":");
			if (arr2.length != 2) {
				continue;
			}
			String k = arr2[0];
	...
	private void mapToXML(Mapx map, Element ele) {
		...
		for (int i = 0; i < ks.length; i++) {
			...
			sb.append(ks[i]);
			sb.append(":\"");
			sb.append(XString.javaEncode(map.getString(ks[i])));
			sb.append("\"");
		}
		sb.append("}");
		ele.addCDATA(sb.toString());
	...
	protected DataTable parseDataTable(Element ele) {
		String str = ele.element("columns").getText();
		String[] t = XString.splitEx(str, "],[");
		DataColumn[] dcs = new DataColumn[t.length];
		t[0] = t[0].substring(2);
		int length = t.length;
		t[(length - 1)] = t[(length - 1)].substring(0,
				t[(length - 1)].length() - 2);
		...
		for (int i = 0; i < t.length; i++) {
			...
			for (int j = 0; j < r.length; j++) {
				if ((dcs[j].getColumnType() == 1)
						|| (dcs[j].getColumnType() == 10)) {
					values[i][j] = XString.javaDecode(r[j]);
					if ((r[j].equals("_SKY_NULL"))
							|| (XString.isEmpty(r[j]))) {
						values[i][j] = null;
					}
				} else if ((r[j].equals("_SKY_NULL"))
						|| (XString.isEmpty(r[j]))) {
					values[i][j] = null;
...
/*
 * com.xdarkness.framework.data.DataCollection JD-Core Version: 0.6.0
 */