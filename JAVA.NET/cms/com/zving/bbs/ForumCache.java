 ...
 public class ForumCache extends CacheProvider
 {
   ...
   public void onTypeNotFound(String type) {
     if (type.equals("Config")) {
       ...
       for (int i = 0; i < set1.size(); ++i) {
         set1.get(i);
         CacheManager.set(getProviderName(), type, set1.get(i).getID(), set1.get(i));
       }
     }
     if (type.equals("Forum")) {
       ...
       for (int i = 0; i < set2.size(); ++i) {
         set2.get(i);
         CacheManager.set(getProviderName(), type, set2.get(i).getID(), set2.get(i));
       }
     }
     if (type.equals("Group")) {
       ...
       for (int i = 0; i < set3.size(); ++i) {
         set3.get(i);
         CacheManager.set(getProviderName(), type, set3.get(i).getID(), set3.get(i));
       }
     }
     if (type.equals("Score")) {
       ...
       for (int i = 0; i < set4.size(); ++i) {
         set4.get(i);
         CacheManager.set(getProviderName(), type, set4.get(i).getID(), set4.get(i));
       }
     }
   }
   ...
   public static ZCForumSet getChildForumSet(String siteID, String forumID) {
     ...
     for (int i = 0; i < vs.length; ++i) {
       ZCForumSchema schema = (ZCForumSchema)vs[i];
       if ((schema.getSiteID() != Long.parseLong(siteID)) || (!"Y".equals(schema.getVisible())) || 
         (!StringUtil.isNotEmpty(forumID)) || (schema.getParentID() != Long.parseLong(forumID))) continue;
       set.add(schema);
     }
 ...
 * Qualified Name:     com.zving.bbs.ForumCache
