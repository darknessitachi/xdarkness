   ...
   public void del()
   {
     ...
     for (int i = 0; i < set.size(); ++i) {
       ...
       if (forumCheck.getParentID() == 0L) {
         ...
         if (count > 0L) {
           this.Response.setStatus(0);
           this.Response.setMessage("不能删除分区\"" + forumCheck.getName() + "\",该分区下还有未被选中的板块!");
           return;
         }
       }
     }
   ...
   public void dg1Edit()
   {
     ...
     if (($V("ForumAdmin").trim().length() == 0) || (ForumUtil.isExistMember($V("ForumAdmin")))) {
       if (($V("ForumAdmin").trim().length() == 0) || (isExistAdmin(trans, $V("ForumAdmin"), forum))) {
         ...
         ForumUtil.addAdminID(trans, forum.getID(), forum.getForumAdmin()); break label178:
       }
 ...
 * Qualified Name:     com.zving.bbs.admin.ForumAdmin
