 ...
 public class ForumOption extends Page
 {
   public static Mapx basicInit(Mapx params)
   {
     ...
     if (forum.getParentID() != 0L) {
       DataTable dt = new QueryBuilder("select Name,ID from ZCForum where SiteID=? and ParentID=0 order by orderflag", 
   ...
   public static void selectGroup(DataGridAction dla) {
     long SiteID = ForumUtil.getCurrentBBSSiteID();
     DataTable dtGroup = new QueryBuilder("select Name,ID from ZCForumGroup where SiteID=? and (SystemName!='游客' or SystemName is null) order by ID", 
       SiteID).executeDataTable();
   ...
   public void saveGroup() {
     ...
     this.Response.put("Group", StringUtil.join(values));
   }
 }
