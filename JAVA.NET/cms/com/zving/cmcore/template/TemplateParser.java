 ...
 public class TemplateParser
 {
   ...
   public void parse() {
     ...
     if (head.toLowerCase().startsWith("<z:config")) {
       parseConfig(head);
       this.template = this.template.substring(this.template.indexOf("\n") + 1);
     } else {
       Errorx.addMessage("第1行不是模板配置信息!");
       this.config = new TemplateConfig("未命名", "Unknown", null, null, null, "<%", "%>");
     }
     ...
     for (int i = 0; i < cs.length; ++i) {
       ...
       if (scriptStartIndex > 0) {
         if ((c != '>') || 
           (this.template.indexOf(this.config.getScriptEnd(), i - this.config.getScriptEnd().length() + 1) != i - 
           this.config.getScriptEnd().length() + 1)) continue;
         TemplateFragment tf = new TemplateFragment();
         tf.Type = 4;
         tf.FragmentText = this.template.substring(scriptStartIndex + this.config.getScriptStart().length(), i - 
         ...
         if ((htmlStartIndex != -1) && (htmlStartIndex != i)) {
           TemplateFragment tf = new TemplateFragment();
           tf.Type = 1;
           tf.FragmentText = this.template.substring(htmlStartIndex, i);
         ...
         if (holderStartIndex >= 0) {
           if ((c == '}') && (holderStartIndex + 2 < i)) {
             TemplateFragment tf = new TemplateFragment();
             tf.Type = 3;
             tf.FragmentText = this.template.substring(holderStartIndex + 2, i);
         ...
         if ((lowerTemplate.indexOf("<z:", i) == i) || (lowerTemplate.indexOf("<cms:", i) == i) || 
         ...
         else if ((lowerTemplate.indexOf("</z:", i) == i) || (lowerTemplate.indexOf("</cms:", i) == i) || 
           (lowerTemplate.indexOf("</custom:", i) == i)) {
           String tagEnd = this.template.substring(i, this.template.indexOf(">", i) + 1);
           TemplateFragment tf = (TemplateFragment)this.currentParent.getData();
           if (tf == null) {
           ...
           tf.FragmentText = this.template.substring(getTagEnd(cs, tf.StartCharIndex + 1) + 1, i);
           ...
           if ((this.template.indexOf(this.config.getScriptStart(), i) != i) || 
             (htmlStartIndex == -1) || (htmlStartIndex == i)) continue;
           TemplateFragment tf = new TemplateFragment();
           tf.Type = 1;
           tf.FragmentText = this.template.substring(htmlStartIndex, i);
           tf.StartLineNo = htmlStartLineNo;
   ...
   public void parseConfig(String head) {
     ...
     this.config = new TemplateConfig(Name, Type, Author, Version, Description, ScriptStart, ScriptEnd);
   }
 ...
 * Qualified Name:     com.zving.cmcore.template.TemplateParser
