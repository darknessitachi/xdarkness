 ...
 public class ColumnUtil
 {
   ...
   {
     DataTable columnDT = new QueryBuilder("select columncode from zdcolumnrela where relatype='1' and relaid = ?", 
     ...
     DataTable valueDT = new QueryBuilder("select * from zdcolumnvalue where relatype='2' and relaid in (" + 
       ...
       for (int k = colCount; k < dt.getColCount(); ++k)
         for (int index = 0; index < valueDT.getRowCount(); ++index) {
           if ((!dt.getString(j, "ID").equals(valueDT.getString(index, "RelaID"))) || 
             (!dt.getDataColumn(k).getColumnName().equals(valueDT.getString(index, "ColumnCode")))) continue;
   ...
   public static void extendCatalogColumnData(DataRow dr, long siteID, String levelStr) {
     DataTable valueDT = new QueryBuilder("select a.InputType,b.ColumnCode,b.TextValue from zdcolumn a,zdcolumnvalue b where a.ID = b.ColumnID and b.relatype='0' and b.relaid ='" + 
   ...
   private static String getHtml(DataRow dr, DataTable valueDT) {
     ...
     if ("Y".equals(isMandatory)) {
       ...
       if (ImageInput.equals(inputType)) {
         verifyStr = verifyStr + "&&图片不能为空|Script=checkMandatory(\"_C_" + columnCode + "\")";
       }
     }
       ...
       if (valueDT != null) {
         for (int i = 0; i < valueDT.getRowCount(); ++i) {
           ...
           if ((!columnCode.equalsIgnoreCase("_C_" + r.getString("columnCode"))) || 
             (r.getString("TextValue") == null)) continue;
           defaultValue = 
             (Config.getContextPath() + Config.getValue("UploadDir") + "/" + 
 ...
 * Qualified Name:     com.zving.cms.dataservice.ColumnUtil
