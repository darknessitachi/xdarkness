 ...
 public class CustomTable extends Page
 {
   ...
   public void del() {
     ...
     ZCCustomTableSet set = new ZCCustomTableSchema().query(
       new QueryBuilder("where SiteID=? and ID in (" + ids + 
       ")", Application.getCurrentSiteID()));
     tran.add(
       new QueryBuilder("delete from ZCCustomTable where SiteID=? and ID in (" + ids + ")", 
       Application.getCurrentSiteID()));
     ...
     try {
       ...
       if ($V("Type").equals("Link")) {
         ...
         for (int i = 0; i < dt.getRowCount(); ++i) {
           ...
           else if ((column.getDataType().equals("1")) && (column.getLength() > 1000L))
             column.setInputType("A");
           else {
   ...
   public static void modifyTable(DataAccess da, String tableCode, Mapx map, ArrayList deletedList, ZCCustomTableColumnSet set) throws Exception
   {
       ...
       for (int j = 0; j < dt.getRowCount(); ++j) {
         for (int k = 0; k < scs.length; ++k) {
           ...
           if (v != null) {
             ...
             if ((sc.getColumnType() == 4) || (sc.getColumnType() == 6) || 
               (sc.getColumnType() == 3)) {
   ...
   public static Mapx initDataEditGrid(Mapx map) {
     ZCCustomTableColumnSet set = new ZCCustomTableColumnSchema().query(
       new QueryBuilder("where TableID=?", map
       .getString("ID")));
   ...
   public static void dataEditGridBind(DataGridAction dga)
   {
     ...
     ZCCustomTableColumnSet set = new ZCCustomTableColumnSchema().query(
       new QueryBuilder("where TableID=?", table
       .getID()));
     DataAccess da = null;
     DataTable dt = null;
   ...
   public void delData()
   {
     ...
     try {
       da.setAutoCommit(false);
       ZCCustomTableColumnSet set = new ZCCustomTableColumnSchema().query(
         new QueryBuilder("where TableID=?", 
         $V("ID")));
   ...
   public static void exportExcel(HttpServletRequest request, HttpServletResponse response) throws Exception {
     ...
     ZCCustomTableColumnSet set = new ZCCustomTableColumnSchema().query(new QueryBuilder("where TableID=?", ID));
   ...
   public static void uploadData(HttpServletRequest request, HttpServletResponse response)
   {
     try
     {
       ...
       FileItem fileItem = null; FileItem idItem = null;
       while (iter.hasNext()) {
         ...
         if (!item.isFormField()) {
           ...
           if ((OldFileName != null) && (!OldFileName.equals("")) && (item.getSize() != 0L)) {
             fileItem = item;
           }
         }
       }
 ...
 * Qualified Name:     com.zving.cms.dataservice.CustomTable
