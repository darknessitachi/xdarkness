 ...
 public class CustomTableUtil
 {
   ...
   public static DataTable getData(String tableCode, long databaseID, QueryBuilder wherePart, int pageSize, int pageIndex)
   {
     if ((wherePart != null) && 
       (!wherePart.getSQL().trim().toLowerCase().startsWith("where"))) {
       throw new RuntimeException("指定的wherePart不是以where开头的字符串");
         ...
         for (int j = 0; j < set.size(); ++j) {
           ...
           if (v != null) {
             if ((column.getMaxLength() != 0) && (v.length() < column.getMaxLength())) {
               str1 = column.getName() + "数据过长，最大允许" + column.getMaxLength() + "个字符!\\n";
               return str1;
             }
             ZCCustomTableColumnSchema column;
             try
 ...
 * Qualified Name:     com.zving.cms.dataservice.CustomTableUtil
