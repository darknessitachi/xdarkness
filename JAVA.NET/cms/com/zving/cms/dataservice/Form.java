 ...
 public class Form extends Page
 {
     ...
     if (StringUtil.isEmpty(content)) {
       ...
       for (int i = 0; i < set.size(); ++i) {
         ...
         if ("S".equals(inputType)) {
           ...
           td.InnerHTML = 
             ("<input ztype='Select' style='width:180px' verify='" + verify + "' id='" + id + 
             "' name='" + id + "'>");
         } else if ("C".equals(inputType)) {
           td.InnerHTML = ((arr == null) ? "" : HtmlUtil.arrayToCheckboxes(id, arr));
         } else if ("D".equals(inputType)) {
           td.InnerHTML = 
             ("<input ztype='Date' style='width:180px' verify='" + verify + "' id='" + id + 
             "' name='" + id + "'>");
         } else if ("C".equals(inputType)) {
           String html = "<input ztype='Date' style='width:80px' verify='" + verify + "' id='" + id + 
             "_Date' name='" + id + "_Date' onchange='_onDateTimeChange(this)'>";
           html = html + "<input ztype='Time' style='width:80px' verify='" + verify + "' id='" + id + 
             "_Time' name='" + id + "_Time'  onchange='_onDateTimeChange(this)'>";
           html = html + "<input type='hidden' style='width:150px' verify='" + verify + "' id='" + id + "' name='" + id + "'>";
           td.InnerHTML = html;
         } else if ("R".equals(inputType)) {
           td.InnerHTML = ((arr == null) ? "" : HtmlUtil.arrayToRadios(id, arr, arr[0]));
         } else if (("A".equals(inputType)) || (column.getLength() > 1000L)) {
           td.InnerHTML = 
             ("<textarea style='width:400px;height:150px' verify='" + verify + "' id='" + id + 
             "' name='" + id + "'></textarea>");
         } else {
           td.InnerHTML = 
             ("<input class='inputText' style='width:180px' verify='" + verify + "' id='" + id + 
             "' name='" + id + "'>");
         }
         tr.addTD(td);
         ht.addTR(tr);
       ...
       td.setColSpan("2");
       td.InnerHTML = 
         ("<input class='inputButton' id='_CustomTableSubmit_" + table.getID() + 
   ...
   public static String getViewFormContent(ZCCustomTableSchema table, int index)
   {
     ...
     content = Pattern.compile("<select", 2).matcher(content).replaceAll(
       "<div ztype='select'");
     content = Pattern.compile("\\/select>", 2).matcher(content).replaceAll("/div>");
     content = Pattern.compile("<option", 2).matcher(content).replaceAll("<span");
     content = Pattern.compile("\\/option>", 2).matcher(content).replaceAll("/span>");
     return content;
   }
