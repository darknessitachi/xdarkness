 ...
 public class VoteReport extends Ajax
 {
   public void getVotePieData()
   {
     ...
     DataTable dt = new QueryBuilder("select item,score from zcvoteitem where subjectid = ? order by score desc,OrderFlag,id", subjectID)
       .executeDataTable();
     if (dt.getRowCount() == 0) {
       $S("Data", "");
       return;
   ...
   public static DataTable getList1(Mapx params, DataRow parentDR) {
     ...
     String type = "单选";
     DataTable dt = new QueryBuilder("select * from ZCVoteSubject where voteID =? and type!='W' order by OrderFlag,ID", voteID)
       .executeDataTable();
   ...
   public static DataTable getList2(Mapx params, DataRow parentDR) {
     ...
     DataTable dt = new QueryBuilder("select * from zcvoteitem where voteID =? and SubjectID=? order by score desc,OrderFlag,id", voteID, 
 ...
 * Qualified Name:     com.zving.cms.dataservice.VoteReport
