 ...
 public class CatalogUtil
 {
   ...
   public static String getLink(String catalogID, String levelStr)
   {
     ZCCatalogSchema catalog = CMSCache.getCatalog(catalogID);
     ZCSiteSchema site = SiteUtil.getSchema(catalog.getSiteID());
     String ext = ServletUtil.getUrlExtension(getSchema(catalogID).getListTemplate());
     String indexPage = "";
     if (ext.equals(".jsp"))
   ...
   {
     DataTable dt = new QueryBuilder("select * from zccatalog where siteid =? and type = ?", 
       Application.getCurrentSiteID(), type).executeDataTable();
     ...
     for (int i = 0; i < dt.getRowCount(); ++i) {
       if (Priv.getPriv(User.getUserName(), PrivType, dt.getString(i, "InnerCode"), "article_browse")) {
 ...
 * Qualified Name:     com.zving.cms.pub.CatalogUtil
