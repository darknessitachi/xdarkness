 ...
 public class VideoLib extends Page
 {
   ...
   public static void treeDataBind(TreeAction ta) {
     ...
     if (ta.isLazyLoad()) {
       ...
       qb.add(ta.getLevel());
       dt = qb.executeDataTable();
       Catalog.prepareSelectedCatalogData(dt, codes, 5, SiteID, ta.getLevel());
     }
     ta.setRootText("视频库");
     dt = dt.filter(new Filter() {
       public boolean filter(Object obj) {
         DataRow dr = (DataRow)obj;
         return Priv.getPriv(User.getUserName(), "video", dr.getString("InnerCode"), "video_browse");
       }
     });
     ta.bindData(dt);
   ...
   public void add()
   {
     ...
     if (trans.commit()) {
       CatalogUtil.update(catalog.getID());
       UserLog.log("Resource", "AddTypeVideo", "新建视频分类:" + catalog.getName() + " 成功", this.Request
         .getClientIP());
       this.Response.setLogInfo(1, "新建视频分类成功!");
     } else {
       UserLog.log("Resource", "AddTypeVideo", "新建视频分类:" + catalog.getName() + " 失败", this.Request
         .getClientIP());
   ...
   public void delLib() {
     ...
     ZCCatalogSet catalogSet = new ZCCatalogSchema().query(
       ...
       "%"));
     Transaction trans = new Transaction();
     ZCVideoSet videoSet = new ZCVideoSchema().query(
     ...
     for (int i = 0; i < videoSet.size(); ++i) {
       ...
       ZCVideoRelaSet VideoRelaSet = new ZCVideoRelaSchema().query(
         new QueryBuilder(" where id =?", videoSet
         .get(i).getID()));
 ...
 * Qualified Name:     com.zving.cms.resource.VideoLib
