 ...
 public class TrendReport extends Page
 {
   ...
   public static DataTable getTreadData(Date start, Date end, String type, String subtype, String item) {
     ...
     if ((type.equals("Client")) && (subtype.equals("District"))) {
       QueryBuilder qb = new QueryBuilder("select * from ZCStatItem where SiteID=? and Type=? and SubType=? and Period>=? and Period<=? and Item like ?");
       ...
       QueryBuilder qb = new QueryBuilder("select * from ZCStatItem where SiteID=? and Type=? and SubType=? and Item=? and Period>=? and Period<=?");
