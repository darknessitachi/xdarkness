 ...
 import com.zving.framework.utility.Big5Convert;
 ...
 import com.zving.statical.TemplateException;
 ...
 public class PageGenerator
 {
   ...
   public boolean staticCatalog(ZCCatalogSchema catalog, boolean isGenerateDetail, int publishPages)
   {
     ...
     if (StringUtil.isNotEmpty(rssTemplate)) {
       ...
       if (tp != null) {
         ...
         tp.define("TemplateData", catalogTemplateData);
         tp.define("catalog", catalogTemplateData.getCatalog());
         this.task.setCurrentInfo("正在生成栏目\"" + catalog.getName() + "\" Rss文件");
   ...
   private boolean staticListPages(ZCSiteSchema site, ZCCatalogSchema catalog, String fileName, TemplateParser catalogParser, CmsTemplateData templateData, boolean generateDetail, int publishPages)
   {
     ...
     if (((generateDetail) && (isCurrentCatalog)) || (singlePageCatalog)) {
       String detailTemplate = this.templateDir + "/" + site.getAlias() + "/" + catalog.getDetailTemplate();
     ...
     if (total < 1)
     {
       ...
       for (int k = 0; k < pageCount; ++k) {
         ...
         DataTable listTable = templateData.getPagedDocList(item, catalogName, displayLevel, hasAttribute, 
         ...
         if (detailParser != null) {
           for (int i = 0; i < listTable.getRowCount(); ++i) {
             ...
             boolean isArticle = (catalogType == 1) || (catalogType == 2) || 
               (catalogType == 3) || (catalogType == 8);
             String titleColName;
             String titleColName;
             if (isArticle)
               titleColName = "Title";
             ...
             if (i != listTable.getRowCount() - 1) {
               docRow.set("NextLink", listTable.getDataRow(i + 1).get("Link"));
               docRow.set("NextTitle", listTable.getDataRow(i + 1).get(titleColName));
             } else if (i == listTable.getRowCount() - 1)
             {
               DataTable nextDt = templateData.getPagedDocList(item, catalogName, displayLevel, 
                 hasAttribute, type, condition, 1, (k + 1) * pageSize);
             ...
             if ((isArticle) && ("4".equals((String)docRow.get("Type")))) {
               continue;
             }
             ...
             staticDoc(item, detailParser, detailTemplateData, docRow, this.staticDir + "/" + 
               site.getAlias() + "/" + h.getFullPath());
           }
         }
       }
   ...
   public boolean staticSite(long siteID)
   {
     ...
     ZCCatalogSet catalogSet = catalog.query(
       new QueryBuilder("where siteid=? and parentid=0 and type in (1,9)", 
       siteID));
     for (int i = 0; i < catalogSet.size(); ++i) {
   ...
   public boolean staticSiteIndex(ZCSiteSchema site) {
     ...
     ZCPageBlockSet blockSet = new ZCPageBlockSchema().query(
       new QueryBuilder("where exists (select blockid from ZCTemplateBlockRela where filename=? and blockid=ZCPageBlock.ID)", 
       template + "_" + level));
   ...
   public boolean staticDoc(String docType, Schema doc)
   {
     ...
     HtmlNameParser nameParser = new HtmlNameParser(site.toDataRow(), catalog.toDataRow(), docDataRow, catalog
       .getDetailNameRule());
     HtmlNameRule h = nameParser.getNameRule();
   ...
   public String previewDoc(String docType, DataRow docDataRow, int currentPage) {
     ...
     templateParser.addClass("com.zving.cms.pub.CatalogUtil");
     templateParser.addClass("com.zving.cms.pub.SiteUtil");
     templateParser.addClass("com.zving.cms.pub.PubFun");
   ...
   private boolean staticDoc(String docType, TemplateParser parser, CmsTemplateData templateData, DataRow docDataRow, String fileName)
   {
     ...
     if ("1".equals(docDataRow.getString("TemplateFlag"))) {
       ...
       detailTemplateName = this.templateDir + "/" + templateData.getSite().getString("Alias") + detailTemplateName;
       detailTemplateName = detailTemplateName.replaceAll("/+", "/");
       TemplateParser currentDetailParser = getParser(templateData.getSite().getLong("Alias"), detailTemplateName, templateData
         .getLevel());
   ...
   private boolean staticInnerPageBlock(ZCSiteSchema site, ZCCatalogSchema catalog, String templateFile) {
     QueryBuilder qb = new QueryBuilder("where exists (select blockid from  ZCTemplateBlockRela where filename=? and blockid=zcpageblock.id)", 
       templateFile);
     ZCPageBlockSet set = new ZCPageBlockSchema().query(qb);
     if (set.size() > 0) {
       for (int i = 0; i < set.size(); ++i) {
   ...
   private TemplateParser getParser(long siteID, String template, int level, boolean isPageBlock) {
     ...
     TemplateParser parser = ParserCache.get(siteID, template, level, isPageBlock, this.fileList);
     return parser;
   }
   ...
   private void convertBig5(String html, String fileName)
   {
     if ("Y".equals(Config.getValue("Big5ConvertFlag"))) {
       ...
       if (fileName.indexOf("cn/") != -1) {
         ...
         big5FileName = big5FileName.replaceAll(this.staticDir, this.staticDir + "/big5/");
