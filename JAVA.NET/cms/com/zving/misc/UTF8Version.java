 ...
 public class UTF8Version
 {
   ...
   public void convert()
   {
     ...
     for (int i = 0; i < this.list.size(); ++i) {
       ...
       this.currentBase = StringUtil.replaceEx(arr[0], "\\", "/");
   ...
   private void convertDir(String src) {
     ...
     for (int i = 0; (fs != null) && (i < fs.length); ++i) {
       ...
       if (f.isDirectory()) {
         ...
         if ((dest.indexOf("wwwroot") >= 0) && (this.currentBase.indexOf("wwwroot") < 0)) {
           continue;
         }
         ...
         if (f.isFile()) {
           ...
           if ((destFile.exists()) && (f.lastModified() < this.lastConvertTime)) {
             continue;
           }
         }
