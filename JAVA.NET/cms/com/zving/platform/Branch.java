 ...
 public class Branch extends Page
 {
   public static Mapx initDialog(Mapx params)
   {
     ...
     if (StringUtil.isNotEmpty(branchInnerCode)) {
       ...
       params.put("ParentName", 
         new QueryBuilder("select name from zdbranch where branchInnercode=?", branch
   ...
   public static DataTable getBranchTable() {
     DataTable dt = new QueryBuilder("select Name,BranchInnerCode,TreeLevel,ParentInnerCode from zdbranch order by orderflag")
       .executeDataTable();
   ...
   public void add() {
     ...
     if (StringUtil.isEmpty(parentInnerCode)) {
       ...
       trans.add(
         new QueryBuilder("update zdbranch set IsLeaf='N' where branchInnerCode =?", pBranch
         .getBranchInnerCode()));
       if (trans.commit())
         this.Response.setLogInfo(1, "新建成功");
       else
     ...
     for (int i = 0; i < ids.length; ++i) {
       ...
       if (branch.fill()) {
         if ("0".equals(branch.getParentInnerCode())) {
           ...
           UserLog.log("System", "DelBranch", "删除机构:" + branch.getName() + "失败", this.Request
             .getClientIP());
           return;
         }
   ...
   public void sortBranch() {
       ...
       for (int i = 0; i < DT.getRowCount(); ++i) {
         if (DT.getString(i, "BranchInnerCode").equals(nextBranch)) {
           ...
           do { branchList.add(orderDT.getDataRow(m));
             ...
             ++m; if (orderDT == null) break label262;  }
     ...
     for (int i = 0; (branchList != null) && (i < branchList.size()); ++i) {
       ...
       trans.add(
         new QueryBuilder("update zdbranch set orderflag = ? where BranchInnerCode = ?", i, dr
