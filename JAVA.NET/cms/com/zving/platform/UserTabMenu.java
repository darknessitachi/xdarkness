 ...
 public class UserTabMenu extends Page
 {
   ...
   public void save()
   {
       ...
       for (int j = 0; j < rolePrivDT.getColCount(); ++j) {
         if (rolePrivDT.getDataColumn(j).getColumnName().toLowerCase().indexOf("_") > 0) {
           rolePrivDT.set(i, j, (RolePriv.getRolePriv(RoleCodes, "menu", SiteID + "-" + 
             rolePrivDT.getString(i, "ID"), rolePrivDT.getDataColumn(j).getColumnName().toLowerCase())) ? 
             "1" : "0");
         }
       }
     ...
     QueryBuilder qb = new QueryBuilder(" where OwnerType=? and Owner=? and PrivType='menu' and ID like ?", 
       "U", $V("UserName"));
     qb.add(SiteID + "%");
     ...
     for (int i = 0; i < resultDT.getRowCount(); ++i) {
       if ((!"1".equals(resultDT.getString(i, "menu_browse"))) && 
         (!"-1".equals(resultDT.getString(i, "menu_browse")))) continue;
