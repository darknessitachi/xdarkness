 ...
 public class BZCDeployJobSet extends SchemaSet
 {
   ...
   public BZCDeployJobSet(int initialCapacity, int capacityIncrement) {
     ...
     this.UpdateAllSQL = "update BZCDeployJob set ID=?,ConfigID=?,SiteID=?,Source=?,Target=?,Method=?,Operation=?,Host=?,Port=?,UserName=?,Password=?,Status=?,RetryCount=?,Prop1=?,Prop2=?,Prop3=?,Prop4=?,AddUser=?,AddTime=?,ModifyUser=?,ModifyTime=?,BackupNo=?,BackupOperator=?,BackupTime=?,BackupMemo=? where ID=? and BackupNo=?";
