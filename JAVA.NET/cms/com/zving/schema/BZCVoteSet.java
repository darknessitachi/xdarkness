 ...
 public class BZCVoteSet extends SchemaSet
 {
   ...
   public BZCVoteSet(int initialCapacity, int capacityIncrement) {
     ...
     this.UpdateAllSQL = "update BZCVote set ID=?,Code=?,SiteID=?,Title=?,Total=?,StartTime=?,EndTime=?,IPLimit=?,VerifyFlag=?,Width=?,RelaCatalogID=?,VoteCatalogID=?,Prop1=?,Prop2=?,Prop3=?,Prop4=?,AddUser=?,AddTime=?,ModifyUser=?,ModifyTime=?,BackupNo=?,BackupOperator=?,BackupTime=?,BackupMemo=? where ID=? and BackupNo=?";
