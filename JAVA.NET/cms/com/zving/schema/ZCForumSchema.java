 ...
 public class ZCForumSchema extends Schema
 {
   ...
   public ZCForumSchema()
   {
     ...
     this.UpdateAllSQL = "update ZCForum set ID=?,SiteID=?,ParentID=?,Type=?,Name=?,Status=?,Pic=?,Visible=?,Info=?,ThemeCount=?,Verify=?,Locked=?,UnLockID=?,AllowTheme=?,EditPost=?,ReplyPost=?,Recycle=?,AllowHTML=?,AllowFace=?,AnonyPost=?,URL=?,Image=?,Password=?,UnPasswordID=?,Word=?,PostCount=?,ForumAdmin=?,TodayPostCount=?,LastThemeID=?,LastPost=?,LastPoster=?,OrderFlag=?,prop1=?,prop2=?,prop3=?,prop4=?,AddUser=?,AddTime=?,ModifyUser=?,ModifyTime=? where ID=?";
   ...
   public void setV(int i, Object v) {
     ...
     if (i != 39) return; this.ModifyTime = ((Date)v); return;
   }
