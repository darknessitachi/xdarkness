       ...
       if (rela.indexOf("-1") >= 0) {
         ...
         for (int j = 0; j < catalogs.getRowCount(); ++j) {
           qb = new QueryBuilder("select * from zcarticle where siteid=" + ft.getSiteID() + " and status=" + 
             30 + " and catalogid =?", catalogs.getString(j, "ID"));
         ...
         for (int j = 0; j < catalogs.length; ++j) {
           QueryBuilder qb = new QueryBuilder("select * from zcarticle where siteid=" + ft.getSiteID() + 
             " and status=" + 30 + " and catalogid =?", catalogs[j]);
           int total = DataAccess.getCount(DBConnPool.getDBConnConfig().DBType, qb);
           for (int i = 0; i * this.PageSize < total; ++i) {
             DataTable dt = qb.executePagedDataTable(this.PageSize, i);
             CatalogUtil.addCatalogName(dt, "CatalogID");
       ...
       if (rela.indexOf("-1") >= 0) {
         ...
         for (int j = 0; j < catalogs.getRowCount(); ++j) {
           QueryBuilder qb = new QueryBuilder("select * from zcarticle where siteid=" + ft.getSiteID() + 
             ...
             30, catalogs.getString(j, "id"), this.lastDate);
         ...
         DataTable dt = new QueryBuilder("select id from bzcarticle where status=30 and siteid=? and backuptime>?", 
         ...
         String[] catalogs = rela.split(",");
         for (int j = 0; j < catalogs.length; ++j) {
           QueryBuilder qb = new QueryBuilder("select * from zcarticle where status=30 and catalogid=? and (addtime>=? or modifytime>=?)", 
         ...
         DataTable dt = new QueryBuilder("select id from bzcarticle where status=30 and siteid=? and catalogid in (" + 
