 ...
 import com.zving.schema.ZWInstanceSchema;
 import com.zving.schema.ZWInstanceSet;
 ...
 public class WorkflowUtil
 {
   ...
   public static Context createInstance(Transaction tran, long workflowID, String name, String dataID, String dataVersionID)
     throws Exception
   {
     ...
     WorkflowAction.executeMethod(context, findWorkflow(instance.getWorkflowID()).getStartNode(), "Post");
     return context;
   }
   ...
   public static WorkflowStep findStep(long stepID)
     throws WorkflowException
   {
     ...
     throws WorkflowException
   {
     ...
     for (int i = 0; i < set.size(); ++i) {
       ...
       if ((step.getState().equals("Underway")) && (step.getOwner().equals(User.getUserName()))) {
         return step;
       }
     }
     for (int i = 0; i < set.size(); ++i) {
       ...
       if ((step.getState().equals("Unread")) && 
         ...
         ("1".equals(findWorkflow(step.getWorkflowID()).getData().get("NotNeedApply")))) {
         return step;
       }
     }
     ...
     throws Exception
   {
     ...
     for (int i = 0; i < set.size(); ++i) {
       ...
       if ((step.getOwner() != null) && (step.getOwner().equals(User.getUserName()))) {
     ...
     if (!underwayFlag) {
       for (int i = 0; i < set.size(); ++i) {
         step = set.get(i);
         if ((!step.getState().equals("Unread")) || 
           (!hasPriv(step.getAllowOrgan(), step.getAllowRole(), step.getAllowUser()))) continue;
         if (!"1".equals(flow.getData().get("NotNeedApply"))) {
           list.add(WorkflowAction.getApplyAction(step.getNodeID()));
         }
         else {
           Transaction tran = new Transaction();
           Context ctx = new Context(tran, step);
   ...
   public static void applyStep(long instanceID, int nodeID)
     throws Exception
   {
   ...
   public static void forceEnd(long instanceID, int nodeID)
     throws Exception
   {
     Transaction tran = new Transaction();
   ...
   public static void forceEnd(Transaction tran, long instanceID, int nodeID) throws Exception
   {
     ...
     else
     {
       ...
       Context context = new Context(tran, instance, newStep);
       findAdapter().onStepCreate(context);
       WorkflowAction.executeMethod(context, findWorkflow(instance.getWorkflowID()).getEndNode(), "Pre");
     }
 
     instance.setState("Completed");
     tran.add(instance, 2);
     tran.add(new QueryBuilder("update ZWStep set State=? where InstanceID=?", "Finish", instanceID));
   }
   ...
   private static boolean hasPriv(String[] branchInnerCodes, String[] roleCodes, String[] userNames)
   {
     ...
     if ((flag) && 
       (roleCodes != null)) {
       label89: List list = PubFun.getRoleCodesByUserName(User.getUserName());
   ...
   public static boolean isStartStep(long instanceID)
   {
     ...
     for (int i = 0; i < set.size(); ++i) {
       if (findWorkflow(set.get(i).getWorkflowID()).getStartNode().getID() == set.get(i).getNodeID()) {
         return true;
       }
     }
     return false;
