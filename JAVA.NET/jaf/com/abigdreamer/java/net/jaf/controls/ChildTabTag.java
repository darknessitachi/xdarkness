...
import com.abigdreamer.java.net.util.StringFormat;
...
public class ChildTabTag extends BodyTagSupport {
	...
	public int doAfterBody() throws JspException {
		...
		try {
			...
			if ((XString.isNotEmpty(this.onClick))
					&& (!this.onClick.trim().endsWith(";"))) {
			...
			sb
					.append("<div style='-moz-user-select:none;' onselectstart='return false' ");
			if ("Disabled".equalsIgnoreCase(type)) {
				sb.append("id='" + this.id + "' " + vStr + " targetURL='"
						+ this.src + "' class='divchildtab" + type + "' >");
			} else {
				if (this.lazy)
					this.src = ("src='javascript:void(0);' targetURL='"
							+ this.src + "'");
				else {
...
/*
 * com.xdarkness.framework.controls.ChildTabTag JD-Core Version: 0.6.0
 */