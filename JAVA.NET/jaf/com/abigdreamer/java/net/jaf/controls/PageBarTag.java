     ...
     throws JspException
   {
     try
     {
       ...
       this.pageIndex = Integer.parseInt(
         ...
         .getAttribute(this.target + "_SKY_PAGEINDEX"));
       this.pageSize = Integer.parseInt(
         (String)this.pageContext
         .getAttribute(this.target + "_SKY_SIZE"));
       String html = null;
       ...
       html = html + "<script>DataGrid.setParam('" + this.target + 
         "','PageBarType'," + this.type + ");";
       if (this.afloat) {
         html = html + "new Afloat($('_PageBar_" + this.target + "').parentNode);";
       }
       html = html + "</script>";
       this.type = 0;
...
/*    com.xdarkness.framework.controls.PageBarTag
