...
public class SSIMediator {
	...
	public void setVariableValue(String variableName, String variableValue) {
		if (!isNameReserved(variableName))
			this.ssiExternalResolver.setVariableValue(variableName,
					variableValue);
	}
	...
	protected void setDateVariables(boolean fromConstructor) {
		boolean alreadySet = this.ssiExternalResolver
				.getVariableValue(this.className + ".alreadyset") != null;

		if ((!fromConstructor) || (!alreadySet)) {
			this.ssiExternalResolver.setVariableValue(this.className
					+ ".alreadyset", "true");
			Date date = new Date();
			...
			this.ssiExternalResolver.setVariableValue(this.className
					+ ".LAST_MODIFIED", retVal);
		}
	}
}

/*
 * com.xdarkness.framework.ssi.SSIMediator JD-Core Version: 0.6.0
 */