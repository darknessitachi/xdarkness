...
import com.abigdreamer.java.net.io.FileUtil;
 ...
 public class CharsetConvert
 {
   ...
   public static byte[] GBKToUTF8(String chinese, boolean bomFlag)
   {
     ...
     if (bomFlag) {
       return ArrayUtils.addAll(XString.BOM, fullByte);
     }
     return fullByte;
   }
   ...
   public static byte[] UTF8ToGBK(String str)
   {
     try
     {
       ...
       if (XString.hexEncode(ArrayUtils.subarray(bs, 0, 3)).equals("efbbbf")) {
     ...
     for (int i = 0; (fs != null) && (i < fs.length); i++) {
       ...
       if (f.isDirectory()) {
         ...
         if (name.equals("web.xml")) {
           txt = XString.replaceEx(txt, "<page-encoding>GBK</page-encoding>", 
             "<page-encoding>UTF-8</page-encoding>");
         }
         if (name.equals("framework.xml"))
           txt = XString.replaceEx(txt, "<config name=\"Charset\">GBK</config>", 
         ...
         if (XString.isUTF8(FileUtil.readByte(f))) continue;
         try {
           String txt = new String(XString.GBKToUTF8(FileUtil.readText(f, "GBK")), "UTF-8");
           FileUtil.writeText(dest, txt, "UTF-8", name.endsWith(".java"));
         } catch (UnsupportedEncodingException e) {
           e.printStackTrace();
         }
   ...
   public static void dirUTF8ToGBK(String src)
   {
     ...
     for (int i = 0; (fs != null) && (i < fs.length); i++) {
       ...
       if (f.isDirectory()) {
         ...
         if (name.endsWith(".xml")) {
           if (name.equals("web.xml")) {
             txt = XString.replaceEx(txt, "<page-encoding>UTF-8</page-encoding>", 
               "<page-encoding>GBK</page-encoding>");
           }
           if (name.equals("framework.xml")) {
             txt = XString.replaceEx(txt, "<config name=\"Charset\">UTF-8</config>", 
           ...
           try {
             txt = new String(XString.UTF8ToGBK(txt), "GBK");
           } catch (UnsupportedEncodingException e) {
             e.printStackTrace();
           }
           FileUtil.writeText(dest, txt, "UTF-8");
         ...
         if (!XString.isUTF8(FileUtil.readByte(f))) continue;
         try {
           String txt = new String(XString.UTF8ToGBK(FileUtil.readText(f, "UTF-8")), "GBK");
