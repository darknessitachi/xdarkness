...
public class MainFilter implements Filter {
	...
	public void init(FilterConfig config) throws ServletException {
		LogUtil.info("----" + WebConfig.getAppCode() + "(" + WebConfig.getAppName()
				+ "): MainFilter Initialized----");
		ServletContext sc = config.getServletContext();
		Config.configMap.put("System.ContainerInfo", sc.getServerInfo());
//		WebConfig.getJBossInfo();
		WebConfig.ServletMajorVersion = sc.getMajorVersion();
		WebConfig.ServletMinorVersion = sc.getMinorVersion();

		this.uptime = System.currentTimeMillis();
		WebConfig.setValue("App.Uptime", this.uptime+"");
	...
	public void doFilter(ServletRequest req, ServletResponse rep,
		...
		if ((WebConfig.ServletMajorVersion == 2)
				&& (WebConfig.ServletMinorVersion == 3))
		...
		if (WebConfig.isComplexDepolyMode()) {
			User.setValue("App.ContextPath", contextPath);
		}
		WebConfig.setValue("App.ContextPath", contextPath);

		if ((!WebConfig.isInstalled) && (url.indexOf("Install.jsp") < 0)
		...
		if ((WebConfig.isInstalled)
				&& ((WebConfig.isNeedCheckPatch) || (WebConfig.isPatching))) {
