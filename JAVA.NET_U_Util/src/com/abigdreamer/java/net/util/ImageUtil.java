...
//		return "1".equals(Config.getValue("ImageLibType"));
	...
	public static void scaleRateImageFile(String fromFileName,
			String toFileName, int toWidth, int toHeight) throws IOException,
			MagickException {
		if (existsImageMagick())
			ImageMagickUtil.scaleRateImageFile(fromFileName, toFileName,
					toWidth, toHeight);
		else
			ImageJDKUtil.scaleRateImageFile(fromFileName, toFileName, toWidth,
					toHeight);
	}

	public static void scaleRateImageFile(String fromFileName,
			String toFileName, double rate) throws MagickException, IOException {
		if (existsImageMagick())
	...
	public static void scaleFixedImageFile(String fromFileName,
			String toFileName, int toWidth, int toHeight)
			throws MagickException, IOException {
		if (existsImageMagick())
			ImageMagickUtil.scaleFixedImageFile(fromFileName, toFileName,
					toWidth, toHeight);
		else
			ImageJDKUtil.scaleFixedImageFile(fromFileName, toFileName, toWidth,
					toHeight);
	}
	...
	public static void pressText(String filePath, String pressText, int color,
		...
		if (existsImageMagick())
			ImageMagickUtil.pressText(filePath, pressText, color, fontSize,
					position);
		else
			ImageJDKUtil.pressText(filePath, pressText, color, fontSize,
					position);
	}
...
/*
 * com.xdarkness.framework.utility.ImageUtil JD-Core Version: 0.6.0
 */