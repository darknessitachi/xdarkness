...
public class ExplorerTag extends BodyTagSupport {
	...
	public int doAfterBody() throws JspException {
		try {
			String ExplorerPath = this.pageContext.getRequest().getParameter(
					"ExplorerPath");
			...
			if ((this.type == 0) || (this.type == 1)) {
				...
				try {
					...
					if ((this.filter != null) && (!"".equals(this.filter))) {
						...
						IOFileFilter suffixFilter = new SuffixFileFilter(
								this.filter.split("\\|"));

						IOFileFilter allFilter = FileFilterUtils.orFileFilter(
								dirFilter, suffixFilter);
...
/*
 * com.xdarkness.framework.controls.ExplorerTag JD-Core Version: 0.6.0
 */