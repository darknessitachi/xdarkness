...
/**
 * 
 * Create on May 14, 2010 11:27:40 AM
 * @author XDarkness
 * @version 1.0
 */
public class DataRow implements Serializable {
	...
	public String getString(int index) {
		if (this.values[index] != null) {
			if ((!"".equals(this.values[index]))
					&& (this.columns[index].getColumnType() == 0)) {
				if (!(this.values[index] instanceof Date)) {
					return String.valueOf(this.values[index]);
				}
				if (XString.isNotEmpty(this.columns[index].getDateFormat())) {
		...
		for (int i = 0; i < map.size(); i++) {
			...
			for (int j = 0; j < this.columns.length; j++)
				if (key.toString().equalsIgnoreCase(
					...
					Object v = vs[i];
					if ((this.columns[j].ColumnType == 0)
	...
	public void fill(Object[] values) {
		...
		if (values.length != getColumnCount()) {
			throw new RuntimeException("执行fill操作数组长度为" + values.length
					+ "，要求的长度为" + getColumnCount() + "！");
		}
		for (int i = 0; i < values.length; i++) {
			if ((this.columns[i].ColumnType == 0)
					&& (!Date.class.isInstance(values[i]))) {
