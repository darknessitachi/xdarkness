...
public class DataTable implements Serializable, Cloneable {
				...
				if (index >= begin) {
					...
					for (int j = 1; j <= columnCount; j++) {
						if (this.columns[(j - 1)].getColumnType() == 10) {
							...
							if ((latin1Flag) && (XString.isNotEmpty(str))) {
								try {
									str = new String(
	...
	public void deleteColumn(int columnIndex) {
		if (columns == null || this.columns.length == 0) {
			return;
		}
		if ((columnIndex < 0)
	...
	public DataTable filter(String whereString) {
		...
		return (DataTable) ObjectHelper.objectClone(filter(new Filter() {
