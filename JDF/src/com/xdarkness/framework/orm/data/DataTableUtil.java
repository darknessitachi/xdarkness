package com.xdarkness.framework.orm.data;
...
public class DataTableUtil {
	...
	public static DataTable txtToDataTable(String fileName, String filedspliter) {
		return txtToDataTable(fileName, null, Constant.GlobalCharset,
				filedspliter, "\\n");
	}
	...
	public static DataTable txtToDataTable(String fileName,
		...
		return txtToDataTable(fileName, null, Constant.GlobalCharset,
				filedspliter, rowspliter);
	}

	public static DataTable txtToDataTable(String fileName, String encoding,
			String filedspliter, String rowspliter) {
		return txtToDataTable(fileName, null, encoding, filedspliter,
				rowspliter);
	}
	...
	public static DataTable txtToDataTable(String fileName,
			String[] columnNames, String filedspliter) {
		return txtToDataTable(fileName, columnNames, Constant.GlobalCharset,
				filedspliter, "\\n");
	}

	public static DataTable txtToDataTable(String fileName,
			String[] columnNames, String encoding, String filedspliter,
			String rowspliter) {
	...
	public static DataTable txtToDataTable(String content,
			String[] columnNames, String filedspliter, String rowspliter) {
	...
	public static void dataTableToTxt(DataTable dt, OutputStream os,
			String[] columnNames, String filedspliter, String rowspliter) {
		try {
			IOUtils.write(dataTableToTxt(dt, columnNames, filedspliter,
					rowspliter), os);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static String dataTableToTxt(DataTable dt, String[] columnNames,
			String filedspliter, String rowspliter) {
	...
	public static void dataTableToExcel(DataTable dt, String fileName,
			String[] columnNames, String[] widths) {
		try {
			FileOutputStream fos = new FileOutputStream(fileName);
		...
		try {
			...
			for (int i = 0; i < dt.getColCount(); i++) {
				...
				if ((widths != null) && (widths.length > i)) {
					sheet.setColumnWidth((short) i, (short) (int) (Short
							.parseShort(widths[i]) * 37.5D));
				}
			}
			...
			for (int i = 0; i < dt.getRowCount(); i++) {
				...
				for (int j = 0; j < dt.getColCount(); j++) {
					...
					if (dt.get(i, j) == null)
						...
						cell.setCellValue(DateUtil.toString(
								(Date) dt.get(i, j), "yyyy-MM-dd"));
					else {
	...
	public static void dataTableToExcel(DataTable dt, OutputStream os,
			String[] columnNames, String[] widths, String[] columnIndexes) {
		HSSFWorkbook wb = new HSSFWorkbook();
		HSSFSheet sheet = wb.createSheet("First");
		try {
			HSSFFont fontBold = wb.createFont();
			...
			for (int i = 0; i < columnNames.length; i++) {
				...
				row.setHeightInPoints(30.0F);
				if ((widths != null) && (widths.length > i)) {
					sheet.setColumnWidth((short) i, (short) (int) (Integer
							.parseInt(widths[i]) * 37.5D));
				}
			}
	...
	public static DataTable xlsToDataTable(String fileName, int sheetNo)
			throws Exception {
		HSSFWorkbook book = new HSSFWorkbook(new FileInputStream(fileName));
		return xlsToDataTable(book, sheetNo);
	}

	public static DataTable xlsToDataTable(HSSFWorkbook book, int sheetNo)
			throws Exception {
	...
	public static DataTable extendBackupData(DataTable dt, String sql,
			String key) {
		for (int i = 0; i < dt.getRowCount(); i++) {
			DataRow dr = dt.getDataRow(i);
			String keyValue = dr.getString(key);
			DataTable backdt = new QueryBuilder(sql, keyValue)
					.setPageSize(1).setPageIndex(0).executePagedDataTable();
			if (i == 0) {
			...
			if (backdt.getRowCount() == 1) {
				System.arraycopy(backdt.getDataRow(0).getDataValues(), 0, dt
						.getDataRow(i).getDataValues(), dt.getColCount()
