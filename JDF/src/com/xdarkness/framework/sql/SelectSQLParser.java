...
public class SelectSQLParser {
	...
	public void setSQL(String sql) {
		if ((sql == null) || (sql == "")) {
			throw new RuntimeException(
					"The select sql which is need to be parse is empty!");
		}
		this.SQL = sql.trim();
	...
	public void parse() throws Exception {
		if (!this.lowerSQL.startsWith("select ")) {
			throw new Exception("String：\"" + this.SQL
		...
		for (int i = startIndex; i < havingPart.length(); i++) {
			...
			if (havingPart.charAt(i) == ' ') {
				if (lowerHavingPart.substring(i).trim().startsWith("and ")) {
					havingConditions.add(havingPart.substring(startIndex, i)
							.trim());
	...
	public String getMSSQLPagedSQL() {
		...
		if (this.OrderByFields != null) {
			...
			String tmp = ("," + XString.join(this.Columns, ",") + ",")
					.toLowerCase();
			if (tmp.indexOf(",id,") >= 0) {
