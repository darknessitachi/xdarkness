...
public class SSIFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if (ExtendManager.hasAction("BeforeSSIFilter")) {
			ExtendManager.executeAll("BeforeSSIFilter", new Object[] { request,
					response, chain });
		}
		...
		ResponseIncludeWrapper responseIncludeWrapper = new ResponseIncludeWrapper(
				this.config.getServletContext(), req, res, basos);
		...
		SSIExternalResolver ssiExternalResolver = new SSIServletExternalResolver(
				this.config.getServletContext(), req, res,
		...
		long lastModified = ssiProcessor.process(reader, responseIncludeWrapper
				.getLastModified(), writer);
...
/*
 * com.xdarkness.framework.ssi.SSIFilter JD-Core Version: 0.6.0
 */