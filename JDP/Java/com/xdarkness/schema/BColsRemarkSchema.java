 ...
 * 表主键：id, BackupNo<br>
 */
public class BColsRemarkSchema extends Schema {
	private String id;
	...
	public static final SchemaColumn[] _Columns = new SchemaColumn[] {
		...
		new SchemaColumn("BackupNo", DataColumn.STRING, 25, 15 , 0 , true , true),
		new SchemaColumn("BackupOperator", DataColumn.STRING, 26, 200 , 0 , true , false),
		new SchemaColumn("BackupTime", DataColumn.DATETIME, 27, 0 , 0 , true , false),
		new SchemaColumn("BackupMemo", DataColumn.STRING, 28, 50 , 0 , false , false)
	};

	public static final String _TableCode = "BColsRemark";

	public static final String _NameSpace = "com.xdarkness.schema";

	protected static final String _InsertAllSQL = "insert into BColsRemark values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

	protected static final String _UpdateAllSQL = "update BColsRemark set id=?,addtime=?,moder=?,modtime=?,tablename=?,colname=?,alias=?,categoryid=?,minvalue=?,maxvalue=?,multilines=?,rigor=?,Fmttypetime=?,openout=?,colchanged=?,Required=?,ValidExp=?,MultiSelected=?,devinfo=?,inputtype=?,colorder=?,tracelog=?,usedRefid=?,usedRefidRoot=?,dynamicbind=?,BackupNo=?,BackupOperator=?,BackupTime=?,BackupMemo=? where id=? and BackupNo=?";

	protected static final String _DeleteSQL = "delete from BColsRemark  where id=? and BackupNo=?";

	protected static final String _FillAllSQL = "select * from BColsRemark  where id=? and BackupNo=?";

	public BColsRemarkSchema(){
	...
	public BColsRemarkSet query() {
		return query(null, -1, -1);
	}

	public BColsRemarkSet query(QueryBuilder qb) {
		return query(qb, -1, -1);
	}

	public BColsRemarkSet query(int pageSize, int pageIndex) {
		return query(null, pageSize, pageIndex);
	}

	public BColsRemarkSet query(QueryBuilder qb , int pageSize, int pageIndex){
		return (BColsRemarkSet)querySet(qb , pageSize , pageIndex);
	}
	...
	public void setV(int i, Object v) {
		...
		if (i == 25){BackupNo = (String)v;return;}
		if (i == 26){BackupOperator = (String)v;return;}
		if (i == 27){BackupTime = (Date)v;return;}
		if (i == 28){BackupMemo = (String)v;return;}
	}
	...
	public Object getV(int i) {
		...
		if (i == 25){return BackupNo;}
		if (i == 26){return BackupOperator;}
		if (i == 27){return BackupTime;}
		if (i == 28){return BackupMemo;}
		return null;
	}
