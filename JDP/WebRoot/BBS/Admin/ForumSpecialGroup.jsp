...
function add(){
	...
	function addSave(){
		...
		Server.sendRequest("com.xdarkness.bbs.admin.ForumUserGroup.addSpecialGroup",dc,function(response){
	...
	function eidtColor(){
		...
		Server.sendRequest("com.xdarkness.bbs.admin.ForumUserGroup.editSpecialGroup",dc,function(response){
			Dialog.alert(response.Message);
			if(response.Status==1){
				DataList.loadData('list1');
			}
	...
	function del(ID){
		Dialog.confirm("删除后不可恢复，确认要删除？",function(){
			...
			dc.add("ID",ID);
			Server.sendRequest("com.xdarkness.bbs.admin.ForumUserGroup.delSpecailGroup",dc,function(response){
				Dialog.alert(response.Message);
				if(response.Status==1){
					DataList.loadData('list1');
				}
	...
	function save(){
		var dc = Form.getData($F("form1"));
		Server.sendRequest("com.xdarkness.bbs.admin.ForumUserGroup.save",dc,function(response){
			Dialog.alert(response.Message);
				if(response.Status==1){
					DataList.loadData('list1');
				}
	...
	function edit(ID){
		...
		dc1.addAll(dc3);
		dc1.addAll(dc4);
		Server.sendRequest("com.xdarkness.bbs.admin.ForumUserGroupOption.editSave",dc1,function(response){
	...
	function imageSave() {
		...
		Server.sendRequest("com.xdarkness.bbs.admin.ForumUserGroup.imageSave",dc,function(response){
			Dialog.alert(response.Message);
			if(response.Status==1){
				$D.close();
				DataList.loadData('list1');
...
<table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#eeeeee" class="forumTable">
	...
	<sky:datalist id="list1" method="com.xdarkness.bbs.admin.ForumUserGroup.getListSpecialGroup" page="false">
		<tr nowrap="nowrap">
