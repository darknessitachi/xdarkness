...
<script type="text/javascript">
	...
	function del(ID){
		Dialog.confirm("确定删除选中的帖子吗？",function(){
			...
			dc.add("ID",ID);
			dc.add("SiteID",$V("SiteID"));
	   		Server.sendRequest("com.xdarkness.bbs.MyPost.del",dc,function(response){
	...
	function applyDel(ID) {
		var dc = new DataCollection();
			dc.add("ID",ID);
			dc.add("SiteID",$V("SiteID"));
	   		Server.sendRequest("com.xdarkness.bbs.PostAudit.applyDel",dc,function(response){
	   			Dialog.alert(response.Message);
				if(response.Status==1 || response.Status==2){
					DataList.loadData("list1");
				}
...
function save(){
	...
	Server.sendRequest("com.xdarkness.bbs.PostAdd.editPost",dc,function(response){
		if(response.Status==1){
...
<input type="hidden" value="${SiteID}" id="SiteID">
  <tr valign="top">
    ...
    <form method="post" name="moderate">
      <table width="100%" cellpadding="0" cellspacing="0" bordercolor="#eeeeee" class="forumTable">
        ...
        <sky:datalist id="list1" size="10" method="com.xdarkness.bbs.MyPost.getMyPost">
        <tbody>
          <tr>
            <td>${Name}</td>
  ...
  <div id="footfilter">
    <form method="get" id="form3"><input type="hidden" name="fid" value="5"> 查看 <sky:select name="filter" id="time">
...
</sky:select>  <sky:select name="ascdesc" id="ascdesc">
