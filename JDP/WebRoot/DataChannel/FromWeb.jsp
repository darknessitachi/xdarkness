...
function addSave(){
	...
	*/
	...
	Server.sendRequest("com.xdarkness.datachannel.FromWeb.add",dc,function(response){
...
function del(){
	...
	Dialog.confirm("确定要删除该任务吗？",function(){
		Server.sendRequest("com.xdarkness.datachannel.FromWeb.del",dc,function(response){
...
function delResult(){
	...
	Dialog.confirm("采集数据是指从指定URL下载的文本文件和<br>图片文件,<font color=red>不包括已经转入栏目的文章</font>。<br><br>确定要清空该任务的采集数据吗？",function(){
		var dc = new DataCollection();	
		dc.add("IDs",arr.join());
		Server.sendRequest("com.xdarkness.datachannel.FromWeb.delResult",dc,function(response){
			Dialog.alert(response.Message);
		});											  
	},null,350);
} 
...
function execute(){
	...
	Dialog.confirm("确定要执行该任务吗？",function(){
		var dc = new DataCollection();	
		dc.add("ID",arr[0]);
		Server.sendRequest("com.xdarkness.datachannel.FromWeb.execute",dc,function(response){
			if(response.Status==1){
...
function cancel(){
	...
	Dialog.confirm("确定要中止执行该任务吗？",function(){
		var dc = new DataCollection();	
		dc.add("ID",arr[0]);
		Server.sendRequest("com.xdarkness.datachannel.FromWeb.cancel",dc,function(response){
			Dialog.alert(response.Message);
		});
	});
}
...
function dealData(){
	...
	var dc = new DataCollection();	
	dc.add("ID",arr[0]);
	Server.sendRequest("com.xdarkness.datachannel.FromWeb.dealData",dc,function(response){
		if(response.Status==1){
			taskID = response.get("TaskID");
	...
	<tr valign="top">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="6" class="blockTable">
			...
			<tr>
				...
				<sky:button onClick="add()"><img src="../Icons/icon007a2.gif" width="20" height="20" />新建</sky:button> 
				<sky:button onClick="edit()"><img src="../Icons/icon007a4.gif" width="20" height="20" />修改</sky:button> 
				<sky:button onClick="del()"><img src="../Icons/icon007a3.gif" width="20" height="20" />删除</sky:button> 
				<sky:button onClick="delResult()"><img src="../Icons/icon007a17.gif" width="20" height="20" />清空采集数据</sky:button>
				<sky:button onClick="dealData()"><img src="../Icons/icon026a8.gif" width="20" height="20" />处理数据</sky:button> 
				<sky:button onClick="execute()"><img src="../Icons/icon403a12.gif" width="20" height="20" />执行任务</sky:button> 
				<sky:button onClick="cancel()"><img src="../Icons/icon404a3.gif" width="20" height="20" />中止执行</sky:button></td>
			</tr>
			<tr>
				<td style="padding-top:0px;padding-left:6px;padding-right:6px;padding-bottom:8px;">
				<sky:datagrid id="dg1" method="com.xdarkness.datachannel.FromWeb.dg1DataBind" page="false">
