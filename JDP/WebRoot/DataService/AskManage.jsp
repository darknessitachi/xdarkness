...
function addSaveTeam(){
	...
	Server.sendRequest("com.xdarkness.cms.dataservice.QuestionTeam.add",dc,function(response){
...
function delTeam(param){
	...
	Dialog.confirm("确认删除 <b style='color:#F00'>"+Name+"</b> 分组吗？",function(){
		...
		Server.sendRequest("com.xdarkness.cms.dataservice.QuestionTeam.del",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					window.parent.parent.location.reload();
				}
...
function editSaveTeam(){
	...
	Server.sendRequest("com.xdarkness.cms.dataservice.QuestionTeam.edit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
...
function addSave(){
	...
	Server.sendRequest("com.xdarkness.cms.dataservice.Question.add",dc,function(response){
...
function editSave(){
	...
	Server.sendRequest("com.xdarkness.cms.dataservice.Question.edit",dc,function(response){
		Dialog.alert(response.Message);
		if(response.Status==1){
				$D.close();
				var QuestionInnerCode = Tree.CurrentItem.getAttribute("cid");
...
function del(){
	...
	Dialog.confirm("此操作会删除分类下的问题信息,确定要删除吗？",function(){
		var dc = new DataCollection();	
		dc.add("IDs",arr.join());
		Server.sendRequest("com.xdarkness.cms.dataservice.Question.del",dc,function(response){
...
<table width="100%" border="0" cellspacing="6" cellpadding="0" style="border-collapse: separate; border-spacing: 6px;">
  <tr valign="top">
    <td width="180"><table oncontextmenu="return false;" width="180" border="0" cellspacing="0" cellpadding="6"
        ...
        <tr>
          <td style="padding:6px;" class="blockTd"><sky:tree id="tree1"
					style="height:430px" method="com.xdarkness.cms.dataservice.QuestionTeam.treeDataBind">
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="6" class="blockTable">
        ...
        <tr>
          <td style="padding:4px 5px;"><sky:button id="add" onClick="add()"> <img src="../Icons/icon021a2.gif" />新建问题</sky:button>
            <sky:button id="edit" onClick="edit()"> <img src="../Icons/icon021a4.gif" />修改问题</sky:button>
            <sky:button id="del" onClick="del()"> <img src="../Icons/icon021a3.gif" />删除问题</sky:button>
            <sky:button id="export" onClick="exportExcel()"> <img src="../Icons/icon021a7.gif" />导出</sky:button>
            <div style="float: right; white-space: nowrap;">
			<span>
				<span id="boxEmpType" style="display:none">
					<div xtype='select' id="EmpType" onChange="searchEmpType()">
						<span value="0">全部</span><span value="1">在职</span>
						<span value="2">闲置</span> <span value="3">离职</span>
					</div>
				</span>
				<span>
				<div xtype='select' id="searchType" onChange="SelectSeachType()">
					<span value="0"></span>
        ...
        <tr>
		  ...
		  <sky:datagrid id="dg1" method="com.xdarkness.cms.dataservice.Question.dg1DataBind" size="15">
              <table width="100%" cellpadding="2" cellspacing="0" class="dataTable">
                <tr xtype="head" class="dataTableHead">
                  <td width="6%" xtype="RowNo"><strong>序号</strong></td>
