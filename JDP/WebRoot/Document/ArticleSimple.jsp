  ...
  var content = contents.join(PAGE_SPLIT);
	...
	Server.sendRequest("com.xdarkness.cms.document.Article.checkBadWord",dc,function(response){
		if(response.Status == 1){
			...
			if (isBadWordExist) {
				Dialog.wait("正在处理...");
				Server.sendRequest("com.xdarkness.cms.document.Article.save",dc,function(response){
					Dialog.closeEx();
					if(response.Status==1){
...
function del(){
	...
	Dialog.confirm("确认删除吗？点击”确认“删除，点击”取消“返回。",function(){
		...
		Server.sendRequest("com.xdarkness.cms.document.Article.del",dc,function(response){
			if(response.Status==1){
				Dialog.alert("文章删除成功");
		    	window.close();
			}else{
		...
		if(!$("CommentFlag").checked){
	  	...
	  	Server.sendRequest("com.xdarkness.cms.document.Article.checkBadWord",dc,function(response){
			if(response.Status == 1){
				...
				if (isBadWordExist) {
					Dialog.wait("正在发布...");
					Server.sendRequest("com.xdarkness.cms.document.Article.saveAndPublish",dc,function(response){
						Dialog.closeEx();
						if(response.Status==1){
							var st = response.get("SaveTime");
							$("SaveTime").innerHTML = st;
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		<tr>
			...
			<div>
				...
				onClick="closeX()">
				<img src="../Icons/icon403a11.gif" />返回</sky:button> 
				</div>
			</td>
		</tr>
	</table>
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="0"
						...
						<tr>
							...
							<table width="95%" border="0" cellpadding="0" cellspacing="4"
								...
								<tr align="left">
									<td>类型</td>
									<td><sky:select id="NewsType" onChange="changeDocType()">
							...
							<table width="95%" border="0" cellpadding="0" cellspacing="4"
								...
								<tr align="left" valign="top">
									...
									<td><input name="text3" type="text" class="input1"
										...
										xtype="Date" style="font-family:Arial;font-size:10px;" /> <input
										name="text2" type="text" class="input1" id="DownlineTime"
										value="${DownlineTime}" size="10" xtype="Time"
