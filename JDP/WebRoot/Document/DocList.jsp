...
function del(){
	...
	Dialog.confirm("确认删除选中的文档吗？",function(){
		...
		Server.sendRequest("com.xdarkness.cms.document.ArticleList.del",dc,function(response){
...
function topublish(){
	...
	var dc = new DataCollection();
	dc.add("IDs",arr.join());
	Server.sendRequest("com.xdarkness.cms.document.ArticleList.topublish",dc,function(response){
...
function publish(){
	...
	var dc = new DataCollection();
	dc.add("IDs",arr.join());
	Server.sendRequest("com.xdarkness.cms.document.ArticleList.publish",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
function changeToPublish(){
	...
	var dc = new DataCollection();
	dc.add("IDs",arr.join());
	Server.sendRequest("com.xdarkness.cms.document.ArticleList.changeToPublish",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
			$("dg1").loadData();
...
function publishIndexSave(){
	...
	$DW.msg();
  var dc = new DataCollection();
	Server.sendRequest("com.xdarkness.cms.site.CatalogSite.publishIndex",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
			$D.close();
		}else{
...
function moveSave(){
	...
	Dialog.wait("正在转移文档...");
	Server.sendRequest("com.xdarkness.cms.document.ArticleList.move",dc,function(response){
		Dialog.closeEx();
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
function copySave(){
	...
	Dialog.wait("正在复制文档...");
	Server.sendRequest("com.xdarkness.cms.document.ArticleList.copy",dc,function(response){
		Dialog.closeEx();
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
function down(){
	...
	var dc = new DataCollection();
	dc.add("ArticleIDs",arr.join(","));
	Server.sendRequest("com.xdarkness.cms.document.ArticleList.down",dc,function(response){
		if(response.Status==1){
...
function up(){
	...
	var dc = new DataCollection();
	dc.add("ArticleIDs",arr.join(","));
	Server.sendRequest("com.xdarkness.cms.document.ArticleList.up",dc,function(response){
		if(response.Status==1){
			Dialog.alert("操作成功!",function(){
				DataGrid.loadData("dg1");
			});
...
function sortOrderSave(){
	...
	Server.sendRequest("com.xdarkness.cms.document.ArticleList.sortArticle",dc,function(response){
		Dialog.closeEx();
		if(response.Status==1){
			Dialog.alert("操作成功!",function(){
				$D.close();
...
function setTopSave(){	
	var dc = $DW.Form.getData("form2");
	Server.sendRequest("com.xdarkness.cms.document.ArticleList.setTop",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData("dg1");
...
function setNotTop(id){	
	...
	var dc = new DataCollection();
	dc.add("IDs",arr.join(","));
	Server.sendRequest("com.xdarkness.cms.document.ArticleList.setNotTop",dc,function(response){
		if(response.Status==1){
			Dialog.alert("操作成功!",function(){
				DataGrid.loadData("dg1");
			});
	...
	<tr valign="top">
		...
		<table width="180" border="0" cellspacing="0" cellpadding="6"
			...
			<tr>
				<td style="padding:6px;" class="blockTd"><sky:tree id="tree1"
					...
					method="com.xdarkness.cms.document.ArticleList.treeDataBind" level="2"
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0"
			...
			<tr>
				...
				<div style="float: left"><sky:button id="BtnAdd"
					priv="article_modify" onClick="add()">
					<img src="../Icons/icon003a2.gif" width="20" height="20" />新建</sky:button> <sky:button
					id="BtnEdit" priv="article_modify"
					onClick="DataGrid.edit(event,'dg1')">
					<img src="../Icons/icon003a4.gif" width="20" height="20" />编辑</sky:button> <sky:button
					id="BtnToPublish" priv="article_modify" onClick="topublish()">
					<img src="../Icons/icon003a6.gif" width="20" height="20" />待发布</sky:button> <sky:button
					id="BtnPublish" priv="article_modify" onClick="publish()">
					<img src="../Icons/icon003a13.gif" width="20" height="20" />发布</sky:button> <sky:button
					...
					<img src="../Icons/icon003a12.gif" width="20" height="20" />复制</sky:button> <sky:button
					id="BtnMove" priv="article_modify" onClick="move()">
					<img src="../Icons/icon003a7.gif" width="20" height="20" />转移</sky:button> <sky:button
					id="BtnOrder" priv="article_modify" onClick="sortOrder()">
					<img src="../Icons/icon400a4.gif" width="20" height="20" />排序</sky:button> <sky:button
					id="BtnDel" priv="article_modify" onClick="del()">
					<img src="../Icons/icon003a3.gif" width="20" height="20" />删除</sky:button></div>
				</td>
			</tr>
			<tr>
				<td style="padding:2px 10px;">
				<div style="float:left;">列出: <sky:select id="Type"
		  ...
		  </sky:select> &nbsp;从 <input type="text" id="StartDate" style="width:90px; "
					xtype='date'> 至 <input type="text" id="EndDate"
					style="width:90px; " xtype='date'> &nbsp;关键词: <input
			...
			<tr>
				...
				<sky:datagrid id="dg1" method="com.xdarkness.cms.document.ArticleList.dg1DataBind" size="15">
					<table width="100%" cellpadding="2" cellspacing="0"
						...
						<tr xtype="head" class="dataTableHead">
							...
							<td width="3%" height="30" xtype="Selector" field="id">&nbsp;</td>
