...
function addSave(){
	...
	Server.sendRequest("com.xdarkness.cms.site.CatalogColumn.add",dc,function(response){
...
function editSave(){
	...
	dc.add("CatalogID",$V("CatalogID"));
	Server.sendRequest("com.xdarkness.cms.site.CatalogColumn.save",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData("dg1");
...
function delSave(){
	...
	dc.add("CatalogID",$V("CatalogID"));
	dc.add("IDs",arr.join());
	Server.sendRequest("com.xdarkness.cms.site.CatalogColumn.del",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				DataGrid.loadData("dg1");
				$D.close();
...
<sky:init>
	...
	<table width="100%" cellpadding="0" cellspacing="0"
		...
		<tr>
			...
			<sky:button id="BtnAdd" priv="article_manage"
				onClick="add()">
				<img src="../Icons/icon024a2.gif" />新建</sky:button> <sky:button id="BtnEdit"
				priv="article_manage" onClick="edit()">
				<img src="../Icons/icon024a4.gif" />编辑</sky:button> <sky:button id="BtnDel"
				priv="article_manage" onClick="del()">
				<img src="../Icons/icon024a3.gif" />删除</sky:button></td>
		</tr>
	</table>
	<sky:datagrid id="dg1"
		method="com.xdarkness.cms.site.CatalogColumn.dg1DataBind" page="false">
		<table width="100%" cellpadding="2" cellspacing="0" class="dataTable">
			<tr xtype="head" class="dataTableHead">
				...
				<td width="10%" align="center" xtype="Checkbox" field="IsMandatory"
