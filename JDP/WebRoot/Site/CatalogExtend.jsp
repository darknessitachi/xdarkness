...
function addSave(){
	...
	Server.sendRequest("com.xdarkness.cms.site.CatalogExtend.add",dc,function(response){
...
function editSave(){
	...
	dc.add("CatalogID",$V("CatalogID"));
	Server.sendRequest("com.xdarkness.cms.site.CatalogExtend.save",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData("dg1");
...
function del(){
	...
	Dialog.confirm("您确认要删除吗？",function(){
		...
		dc.add("CatalogID",$V("CatalogID"));
		dc.add("IDs",arr.join());
		Server.sendRequest("com.xdarkness.cms.site.CatalogExtend.del",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					DataGrid.loadData("dg1");
				}
	...
	<table width="100%" cellpadding="0" cellspacing="0"
		...
		<tr>
			...
			<sky:button id="BtnAdd" priv="article_manage"
				...
				<img src="../Icons/icon018a3.gif" />删除</sky:button></td>
		</tr>
	</table>
	<sky:datagrid id="dg1"
		method="com.xdarkness.cms.site.CatalogExtend.dg1DataBind" page="false"
		autoFill="true">
		<table width="100%" cellpadding="2" cellspacing="0" class="dataTable">
			<tr xtype="head" class="dataTableHead">
				<td width="6%" xtype="RowNo">&nbsp;</td>
				<td width="5%" xtype="selector" field="ColumnID">&nbsp;</td>
