...
public class Post extends Ajax {
	...
	public static void loadFile(DataTable dt) {
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			...
			DataTable dtFile = new QueryBuilder(
					"select * from ZCForumAttachment where PostID=?", ID)
					.executeDataTable();
			StringBuffer sb = new StringBuffer();
			if (dt.getRowCount() > 0) {
				for (int j = 0; j < dtFile.getRowCount(); j++) {
					float size = (float) Long.parseLong(dtFile.getString(j,
							"AttSize"));
					...
					String[] ext = { "jpg", "gif", "zip", "rar", "bmp", "png",
							"doc", "xls", "html", "js", "mov", "mp4", "flv",
							"rm", "wmv", "swf", "txt", "mp3", "avi", "ppt",
							"pdf", "pptx", "xlsx", "docx" };
					...
					sb.append("<a href='#;' onclick='downLoad("
							+ dtFile.getString(j, "ID") + ")'>"
							+ dtFile.getString(j, "Name") + "</a>(" + fileSize
							+ ") 已下载：" + dtFile.getString(j, "DownCount")
							+ "次<br/><br/>");
					if (dtFile.getString(j, "Type").equals("image")) {
						sb.append("<img  onload='resetSize(this)' src='/"
	...
	public void add() {
		...
		if ((XString.isNotEmpty($V("file"))) && ($V("file").length() > 0)) {
	...
	public static Mapx init(Mapx params) {
		...
		QueryBuilder qb = new QueryBuilder(
				"select ThemeID,ForumID from ZCPost where Invisible='Y' and VerifyFlag='Y' and first='Y' and ThemeID>?",
				ThemeID);
		if ((!XString.isEmpty(user)) && (user.equals("current")))
			qb.append(" and AddUser=?", User.getUserName());
		else if ((!XString.isEmpty(user)) && (user.equals("reply")))
			qb
					.append(
							" and exists (select 1 from ZCPost where first='N' and AddUser=? and ThemeID=ZCPost.ThemeID)",
							User.getUserName());
		else if (!XString.isEmpty(LastPoster))
		...
		sql = XString.replaceEx(sql, "ThemeID>", "ThemeID<") + " desc";
		qb.setSQL(sql);
		DataTable dtPrevious = qb.executePagedDataTable(1, 0);

		String ThemeIDNext = dtNext.getRowCount() > 0 ? dtNext.getString(0,
				"ThemeID") : null;
		String ThemeIDPrevious = dtPrevious.getRowCount() > 0 ? dtPrevious
				.getString(0, "ThemeID") : null;
		String ForumIDNext = dtNext.getRowCount() > 0 ? dtNext.getString(0,
				"ForumID") : null;
		...
		if (ForumUtil.isReplyPost(SiteID, ForumID)) {
			map.put("NewReplyButton",
					"<a href='#;' onclick='reply()'>回复本话题</a>");
		}
		if (((rule.getRule("AllowTheme")) || (ForumUtil
				.isAdmin(ForumID, SiteID) > 0))
	...
	public void del() {
		...
		if (flag) {
			...
			forum
					.setPostCount(forum.getPostCount() - theme.getReplyCount()
							- 1);
			qb = new QueryBuilder(
			...
			if (count == 0) {
				qb = new QueryBuilder(
						"select Subject,AddUser from ZCTheme where status='Y' and VerifyFlag='Y' and ForumID=? and ID<? order by ID desc",
						forum.getID(), theme.getID());
		...
		if (trans.commit()) {
			CacheManager.set("Forum", "Forum", forum.getID(), forum);
			UserLogPage.log("Forum", "DelPost", "删除用户" + post.getAddUser()
	...
	public void hide() {
		...
		post.setID($V("ID"));
		post.fill();
		if ((XString.isNotEmpty(post.getIsHidden()))
				&& (post.getIsHidden().equals("Y")))
			post.setIsHidden("N");
		else {
			post.setIsHidden("Y");
		}
		if (post.update()) {
			UserLogPage.log("Forum", "HidePost", "解除/屏蔽用户" + post.getAddUser()
					+ "被屏蔽的帖子成功", this.request.getClientIP());
			this.response.setLogInfo(1, "操作成功！");
		} else {
			UserLogPage.log("Forum", "HidePost", "解除/屏蔽用户" + post.getAddUser()
					+ "被屏蔽的帖子失败", this.request.getClientIP());
	...
	private static DataTable addColumns(String SiteID, String ForumID,
			DataTable dt) {
		dt.insertColumn("FirstSubject");
		if (dt.getRowCount() > 0) {
			dt.set(0, "FirstSubject", "<h4>" + dt.getString(0, "Subject")
					+ "</h4>");
		}
		...
		DataTable dtAdmin = new QueryBuilder(
				"select ID,Name,Image,Color from ZCForumGroup where SiteID=? and (SystemName='版主' or SystemName='超级版主' or SystemName='系统管理员') order by ID",
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			...
			if (priv.hasPriv("AllowEdit"))
				sb
						.append("|<a href='javascript:void(0)' onclick='isOperaterMember("
								+ dt.get(i, "ID") + ",1)'>编辑</a>");
			...
			if (priv.hasPriv("Hide")) {
				if ((XString.isNotEmpty(dt.getString(i, "IsHidden")))
			...
			if ((dt.getString(i, "UseSelfImage").equals("N"))
					&& (XString.isNotEmpty(dt.getString(i, "Image")))) {
			...
			if ((XString.isNotEmpty(dt.getString(i, "IsHidden")))
				...
				if (ForumUtil.isAdmin(ForumID, SiteID) > 0) {
					...
					dt
							.set(i, "Message",
									"<div class='shield_post'><span>提示：</span>该帖已被屏蔽</div>");
					dt.set(i, "File", "");
				}
			}
			for (int j = 0; j < dtAdmin.getRowCount(); j++) {
				if ((dt.getLong(i, "AdminID") != dtAdmin.getLong(j, "ID"))
						|| ((dtAdmin.get(j, "Name").equals("版主")) && (!ForumUtil
								.isForumAdmin(ForumID, dt.getString(i,
										"AddUser"))))) {
					continue;
				}
...
/*
 * com.xdarkness.bbs.Post JD-Core Version: 0.6.0
 */