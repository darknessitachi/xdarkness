...
import com.xdarkness.cms.site.FileList;
import com.xdarkness.datachannel.CommonFtp;
import com.xdarkness.datachannel.SFtp;
...
public class Deploy {
	...
	public boolean addOneJob(long configID, boolean immediate) {
		...
		String sourcePath = staticDir + "/" + ApplicationPage.getCurrentSiteAlias()
				+ config.getSourceDir();
	...
	public long copyOuterDir() {
		LongTimeTask ltt = new LongTimeTask() {
			public void execute() {
				ZCDeployConfigSet configSet = new ZCDeployConfigSchema()
						.query();
				if (configSet.size() > 0)
					for (int i = 0; i < configSet.size(); i++) {
						ZCDeployJobSchema job = new ZCDeployJobSchema();
						ZCDeployJobSet jobs = job
								.query(new QueryBuilder(
										"where RetryCount=0 and status<>? and configID=? order by id",
										2L, configSet.get(i).getID()));
						if ((jobs != null) && (jobs.size() > 0)) {
							LogUtil.getLogger().info(
									"执行分发任务 任务数：" + jobs.size());
	...
	public ZCDeployJobSet getJobs(long siteID, ArrayList list, String operation) {
		...
		for (int j = 0; j < list.size(); j++) {
			...
			srcPath = srcPath.replace('\\', '/').replaceAll("///", "/")
					.replaceAll("//", "/");

			String baseDir = Config.getContextRealPath()
					+ Config.getValue("Statical.TargetDir").replace('\\', '/');
			...
			QueryBuilder qb = new QueryBuilder(
					" where UseFlag =1 and siteid=? and ? like concat(sourcedir,'%')",
					siteID, srcPath);
			if (Config.isSQLServer()) {
				qb.setSQL(" where siteid=? and charindex(sourcedir,?)=0");
			}
			if (Config.isDB2()) {
				qb.setSQL(" where siteid=? and locate(sourcedir,'" + srcPath
						+ "')=0");
	...
	public boolean executeBatchJob(ZCDeployConfigSchema config,
			ZCDeployJobSet jobs) {
		...
		if ("DIR".equals(deployMethod)) {
			...
			try {
				ftp.connect(config.getHost(), (int) config.getPort(), config
			...
			if (connectFlag) {
				for (int i = 0; i < jobs.size(); i++) {
					...
					label1106: jobLog.setMessage(message);
					jobLog.setEndTime(new Date());
					LogUtil.getLogger().info(message);
			...
			SFtp sftp = new SFtp();
			try {
				sftp.connect(config.getHost(), (int) config.getPort(), config
						.getUserName(), config.getPassword());
				connectFlag = true;
			...
			if (connectFlag) {
				for (int i = 0; i < jobs.size(); i++) {
					...
					label1730: jobLog.setMessage(message);
					jobLog.setEndTime(new Date());
					LogUtil.getLogger().info(message);
...
/*
 * com.xdarkness.cms.datachannel.Deploy JD-Core Version: 0.6.0
 */