...
public class Publisher {
	...
	public boolean publishArticle(ZCArticleSet articleSet,
			boolean generateCatalog, LongTimeTask task) throws SQLException {
	...
	public boolean publishArticle(Transaction trans, ZCArticleSet articleSet,
			boolean generateCatalog, LongTimeTask task) throws SQLException {
		if ((articleSet == null) || (articleSet.size() <= 0)) {
			return true;
		}
		...
		for (int i = 0; i < articleSet.size(); i++) {
			...
			if (article.getStatus() == 40L) {
				...
				String workflowName = CatalogUtil.getWorkflow(article
						.getCatalogID());

				if ((XString.isNotEmpty(workflowName))
						&& (article.getStatus() == 10L)) {
					String msg = "文档需审核，不能发布。";
					if (task != null) {
						task.addError(msg);
					}
					Errorx.addError(msg);
				} else if ((XString.isNotEmpty(workflowName))
						&& (article.getStatus() == 0L)
					...
					Errorx.addError(msg);
						...
						&& (article.getPublishDate().getTime() > new Date()
								.getTime())) {
					...
					trans
							.add(new QueryBuilder(
									"update zcimage set status=30 where status<>? and exists (select 1 from zcimagerela where relaid=? and id=zcimage.id)",
									30L, article.getID()));
					trans
							.add(new QueryBuilder(
									"update zcvideo set status=30 where status<>? and exists (select id from zcvideorela where relaid=? and id=zcvideo.id)",
									30L, article.getID()));
					trans
							.add(new QueryBuilder(
					...
					trans
							.add(new QueryBuilder(
									"update zcaudio set status=30 where status<>? and exists (select id from zcaudiorela where relaid=? and id=zcaudio.id)",
									30L, article.getID()));
				}
			}
		}
		if (generateCatalog) {
			...
			for (int k = 0; k < articleSet.size(); k++) {
				catalogMap.put(articleSet.get(k).getCatalogID(), articleSet
						.get(k).getCatalogID()+"");
				String pid = CatalogUtil.getParentID(articleSet.get(k)
						.getCatalogID());
				while ((XString.isNotEmpty(pid)) && (!"null".equals(pid))
						&& (!"0".equals(pid))) {
			...
			for (int j = 0; j < catalogMap.size(); j++) {
				String listpage = CatalogUtil.getData(vs[j].toString())
						.getString("ListPage");
				if ((XString.isEmpty(listpage)) || ("0".equals(listpage))
						|| ("-1".equals(listpage))) {
					listpage = "20";
				}
				publishCatalog(Long.parseLong(vs[j].toString()), false, false,
	...
	public boolean publishDocs(String docType, SchemaSet docSet,
			boolean generateCatalog, LongTimeTask task) {
		...
		for (int i = 0; i < docSet.size(); i++) {
			...
			catalogMap.put(row.getString("catalogid"), row
					.getString("catalogid"));
	...
	public boolean publishArticle(ZCArticleSet articleSet) throws SQLException {
		return publishArticle(articleSet, true, null);
	}

	public boolean publishArticle(ZCArticleSet articleSet, LongTimeTask task) throws SQLException {
		return publishArticle(articleSet, true, task);
	}

	public boolean publishGoods(ZSGoodsSet goodsSet, LongTimeTask task) throws SQLException {
		return publishGoods(goodsSet, true, task);
	}

	public boolean publishGoods(ZSGoodsSet goodsSet, boolean generateCatalog,
			LongTimeTask task) throws SQLException {
		Transaction trans = new Transaction();
	...
	public boolean publishGoods(Transaction trans, ZSGoodsSet goodsSet,
			boolean generateCatalog, LongTimeTask task) throws SQLException {
		PageGenerator p = new PageGenerator(task);
		if (task != null) {
		...
		for (int i = 0; i < goodsSet.size(); i++) {
			...
			if (goods.getStatus().equals("40")) {
				...
				String workflowName = CatalogUtil.getWorkflow(goods
						.getCatalogID());
				if ((XString.isNotEmpty(workflowName))
						&& (goods.getStatus().equals("10"))) {
					...
					QueryBuilder qb = new QueryBuilder(
							"update zsgoods set modifyuser=?,modifytime=?,status=? where id=?");
					if (goods.getPublishDate() == null) {
						qb = new QueryBuilder(
								"update zsgoods set publishdate=?,modifyuser=?,modifytime=?,status=? where id=?");
						qb.add(date);
		...
		if (generateCatalog) {
			...
			for (int k = 0; k < goodsSet.size(); k++) {
				catalogMap.put(goodsSet.get(k).getCatalogID(), goodsSet.get(k)
						.getCatalogID()+"");
				String pid = CatalogUtil.getParentID(goodsSet.get(k)
						.getCatalogID());
				while ((XString.isNotEmpty(pid)) && (!"null".equals(pid))
						&& (!"0".equals(pid))) {
					catalogMap.put(pid, pid);
	...
	public boolean publishCatalog(long catalogID, boolean child,
			boolean detail, int publishSize) {
		return publishCatalog(catalogID, child, detail, publishSize, null);
	}

	public boolean publishCatalog(long catalogID, boolean child,
			boolean detail, LongTimeTask task) {
		return publishCatalog(catalogID, child, detail, 0, task);
	}

	public boolean publishCatalog(long catalogID, boolean child,
			boolean detail, int publishPages, LongTimeTask task) {
		PageGenerator p = new PageGenerator(task);
		...
		if ((child) && (!p.staticChildCatalog(catalogID, detail, publishPages))) {
			return false;
		}
		...
		if (detail) {
			...
			trans.add(new QueryBuilder(
					"update zcarticle set publishDate = ?,status=30"
							+ wherePart, new Date()));
			if (!trans.commit()) {
				return false;
			}
	...
	public void deletePubishedFile(ZCImageSchema image) {
		...
		String filePath = staticDir + "/"
				+ SiteUtil.getAlias(image.getSiteID()) + "/upload/"
	...
	public void deletePubishedFile(SchemaSet set) {
		...
		for (int i = 0; i < set.size(); i++) {
			...
			if ((doc instanceof ZCArticleSchema)) {
				...
				String basePath = staticDir
						+ SiteUtil.getAlias(image.getSiteID()) + "/"
						+ image.getPath();
				Mapx configFields = new Mapx();
				configFields.putAll(ConfigImageLib.getImageLibConfig(image
						.getSiteID()));
				int count = Integer.parseInt(configFields.get("Count")
						.toString());
				...
				String basePath = staticDir
						+ SiteUtil.getAlias(attach.getSiteID()) + "/"
						+ attach.getPath();
				fileList.add(basePath + attach.getSrcFileName());
				if ((XString.isNotEmpty(attach.getImagePath()))
				...
				fileList.add(staticDir + SiteUtil.getAlias(attach.getSiteID())
						+ "/" + CatalogUtil.getPath(attach.getCatalogID())
						+ "/" + attach.getID() + ".shtml");
				...
				fileList.add(staticDir + SiteUtil.getAlias(video.getSiteID())
						+ "/" + CatalogUtil.getPath(video.getCatalogID()) + "/"
						+ video.getID() + ".shtml");
			} else if ((doc instanceof ZCAudioSchema)) {
				ZCAudioSchema audio = (ZCAudioSchema) doc;
				String basePath = staticDir
						+ SiteUtil.getAlias(audio.getSiteID()) + "/"
						+ audio.getPath() + audio.getSrcFileName();
				...
				fileList.add(staticDir + SiteUtil.getAlias(audio.getSiteID())
						+ "/" + CatalogUtil.getPath(audio.getCatalogID()) + "/"
						+ audio.getID() + ".shtml");
			}
		...
		LongTimeTask ltt = new LongTimeTask() {
			...
			public void execute() {
				...
				if ((set != null) && (set.size() > 0)) {
					...
					String listpage = CatalogUtil.getData(
							set.getObject(0).toDataRow().getString(
									"CatalogID")).getString("ListPage");
					if ((XString.isEmpty(listpage))
							|| ("0".equals(listpage))
							|| ("-1".equals(listpage))) {
						listpage = "20";
					}
					p.publishCatalog(set.getObject(0).toDataRow()
							.getLong("CatalogID"), false, false, Integer
							.parseInt(listpage));
					setPercent(100);
				}
			}
...
/*
 * com.xdarkness.cms.datachannel.Publisher JD-Core Version: 0.6.0
 */