...
import com.xdarkness.schema.ZCVoteItemSet;
...
import com.xdarkness.schema.ZCVoteSubjectSet;
...
public class Vote extends Page {
	public static Mapx initDialog(Mapx params) {
		...
		if (XString.isEmpty(ID)) {
			...
			params.put("IPLimit", HtmlUtil.codeToRadios("IPLimit", "YesOrNo",
					"N", false));
			params.put("Prop4", HtmlUtil.codeToRadios("Prop4", "YesOrNo", "Y",
					false));
			params.put("VerifyFlag", HtmlUtil.codeToRadios("VerifyFlag",
					"VerifyFlag", "N"));
			...
			params = vote.toMapx();
			params.put("IPLimit", HtmlUtil.codeToRadios("IPLimit", "YesOrNo",
					vote.getIPLimit(), false));
			params.put("Prop4", HtmlUtil.codeToRadios("Prop4", "YesOrNo", vote
					.getProp4(), false));
			params.put("VerifyFlag", HtmlUtil.codeToRadios("VerifyFlag",
					"VerifyFlag", vote.getVerifyFlag()));
			...
			if (vote.getVoteCatalogID() == 0L) {
				...
				params.put("VoteCatalogName", CatalogUtil.getName(vote
						.getVoteCatalogID()));
			}
		}
		return params;
	...
	public static Mapx JSCodeDialog(Mapx params) {
		...
		JSCode = JSCode
				...
				+ new StringBuffer(String.valueOf(Config.getContextPath()))
						.append(Config.getValue("Statical.TargetDir")).append(
								"/").append(ApplicationPage.getCurrentSiteAlias())
						.append("/js/vote_").append(vote.getID()).toString()
	...
	public static void dg1DataBind(DataGridAction dga) {
		...
		if (!RelaCatalogID.equals("0"))
			qb
					.append(" and RelaCatalogID = ?  order by ID desc",
							RelaCatalogID);
		else {
	...
	public static void dg2DataBind(DataGridAction dga) {
		QueryBuilder qb = new QueryBuilder(
				"select * from ZCVotelog where VoteID = ? order by ID desc",
				dga.getParam("ID"));
		dga.setTotal(qb);
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			String[] resArr = XString.splitEx(dt.getString(i, "Result"),
					"$|");
			StringBuffer sb = new StringBuffer();
			for (int j = 0; j < resArr.length - 1; j++) {
				String[] strArr = XString.splitEx(resArr[j], "$&");
				if (strArr.length > 1) {
					sb.append(strArr[1]);
					sb.append("，");
				}
			}
			if (XString.splitEx(sb.toString(), "，").length > 1) {
				dt.set(i, "OtherContents", sb.substring(0, sb.length() - 1));
			}
			if (XString.isEmpty(dt.getString(i, "AddUser"))) {
				dt.set(i, "AddUser", "匿名");
			}
		}
		dga.bindData(dt);
	...
	public void add() {
		String Code = $V("Code");
		QueryBuilder qb = new QueryBuilder(
				"select count(*) from ZCVote where Code = ?", Code);
		if (qb.executeInt() > 0) {
	...
	private static void dealArticle(ZCVoteSchema vote, Transaction tran) {
		DataTable subject = new QueryBuilder(
				"select * from zcvotesubject where voteid=?", vote.getID())
				.executeDataTable();
		if (subject.getRowCount() > 0) {
			DataTable dt = new QueryBuilder(
					"select ID,Title from ZCArticle where CatalogID=? and Status=?",
					vote.getVoteCatalogID(), 30L).executeDataTable();
			...
			tran.add(insertSet, OperateType.INSERT);
		}
	}
	...
	public void edit() {
		...
		vote.setSiteID(ApplicationPage.getCurrentSiteID());
		if ((XString.isNotEmpty($V("VoteCatalogID")))
				&& (Long.parseLong($V("VoteCatalogID")) != vote
						.getVoteCatalogID())) {
			trans.add(new QueryBuilder("delete from zcvoteitem where VoteID=?",
					vote.getID()));
		}
		vote.setRelaCatalogID($V("RelaCatalogID"));
		if (XString.isNotEmpty($V("VoteCatalogID"))) {
		...
		trans.add(vote, OperateType.UPDATE);
		dealArticle(vote, trans);
		if (trans.commit()) {
	...
	public static boolean generateJS(String ID) {
		...
		if (now.before(vote.getStartTime())) {
			sb.append("document.write('对不起，此调查还没有开始！开始时间为："
					+ vote.getStartTime() + "，请您到时候再来投票');");
			flag = false;
		}
		...
		if (flag) {
			sb
					.append("document.write(\"<div id='vote_"
							+ ID
							+ "' class='votecontainer' style='text-align:left' >\");\n");
			String serviceUrl = Config.getValue("ServicesContext");
			sb.append("document.write(\" <form id='voteForm_" + ID
					+ "' name='voteForm_" + ID + "' action='" + serviceUrl
					+ Config.getValue("Vote.ActionURL")
					+ "' method='post' target='_blank'>\");\n");
			sb
					.append("document.write(\" <input type='hidden' id='ID' name='ID' value='"
							+ ID + "'>\");\n");
			sb
			...
			ZCVoteSubjectSet subjectSet = subject.query(new QueryBuilder(
					" where voteID =? order by OrderFlag,ID", vote.getID()));
			for (int i = 0; i < subjectSet.size(); i++) {
				...
				sb.append("document.write(\"  <dt id='" + subject.getID()
						+ "'>" + (i + 1) + "." + subject.getSubject()
						+ "</dt>\");\n");
				ZCVoteItemSchema item = new ZCVoteItemSchema();
				ZCVoteItemSet itemSet = item
						.query(new QueryBuilder(
								"where voteID = ? and subjectID = ? order by OrderFlag,ID",
				...
				for (int j = 0; j < itemSet.size(); j++) {
					...
					if ("0".equals(item.getItemType()))
						...
						if ("W".equals(subject.getType()))
							sb
									.append("document.write(\"<dd><input id='Subject_"
											+ subject.getID()
											+ "' name='Subject_"
											+ subject.getID()
											+ "' type='text' value=''/></dd>\");\n");
		...
		String file = new StringBuffer(String.valueOf(Config
	...
	public void del() {
		...
		ZCVoteItemSet itemset = new ZCVoteItemSchema().query(new QueryBuilder(
				"where voteid in (" + ids + ")"));
		Transaction trans = new Transaction();
		trans.add(set, OperateType.DELETE_AND_BACKUP);
		trans.add(subjectset, OperateType.DELETE_AND_BACKUP);
		trans.add(itemset, OperateType.DELETE_AND_BACKUP);
		trans.add(new QueryBuilder("delete from zcvotelog where voteid in ("
				+ ids + ")"));
		if (trans.commit()) {
	...
	public void handStop() {
		...
		trans.add(new QueryBuilder(
				"update zcvote set EndTime = ? where id in (" + ids + ") ",
				new Date()));
		if (trans.commit()) {
	...
	public static DataTable getVoteItems(Mapx params, DataRow parentDR) {
		String voteID = params.getString("ID");
		DataTable dt = new QueryBuilder(
				"select * from ZCVoteItem where voteID =? and subjectID = ?",
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			...
			if ("0".equals(dt.getString(i, "ItemType")))
				html = "<label><input name='Subject_"
						+ dt.getString(i, "SubjectID") + "' type='" + inputType
						+ "' value='" + dt.getString(i, "id") + "' />"
						+ dt.getString(i, "item") + "</label>\n";
			else if ("1".equals(dt.getString(i, "ItemType"))) {
				if ("W".equals(parentDR.getString("Type")))
					html = "<input id='Subject_" + dt.getString(i, "SubjectID")
							+ "' name='Subject_" + dt.getString(i, "SubjectID")
							+ "' type='text' value=''/>\n";
				else
					html = "<lable><input name='Subject_"
							+ dt.getString(i, "SubjectID")
							+ "' type='"
							+ inputType
							+ "' value='"
							+ dt.getString(i, "id")
							...
							+ dt.getString(i, "id")
							+ "' type='text' value='' onClick=\"clickInput('Subject_"
							+ dt.getString(i, "SubjectID") + "_Item_"
							+ dt.getString(i, "id") + "');\"/>\n";
				...
				if ("W".equals(parentDR.getString("Type")))
					html = "<textarea style=\"height:60px;width:400px;vertical-align:top;\" id='Subject_"
							+ dt.getString(i, "SubjectID")
							+ "' name='Subject_"
							+ dt.getString(i, "SubjectID") + "'/></textarea>\n";
				else {
					html = "<lable><input name='Subject_"
							+ dt.getString(i, "SubjectID")
							+ "' type='"
							...
							+ dt.getString(i, "SubjectID") + "_Item_"
							+ dt.getString(i, "id") + "' name='Subject_"
							+ dt.getString(i, "SubjectID") + "_Item_"
							+ dt.getString(i, "id")
							+ "'  onClick=\"clickInput('Subject_"
							+ dt.getString(i, "SubjectID") + "_Item_"
...
/*
 * com.xdarkness.cms.dataservice.Vote JD-Core Version: 0.6.0
 */