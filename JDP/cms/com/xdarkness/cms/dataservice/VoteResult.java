...
import com.xdarkness.schema.ZCVoteLogSchema;
...
import com.xdarkness.framework.util.ServletUtil;
 ...
 public class VoteResult
 {
   public static boolean vote(HttpServletRequest request, HttpServletResponse response)
   {
     ...
     if ((XString.isEmpty(ID)) && (XString.isEmpty(VoteCatalogID))) {
       try {
     ...
     if (XString.isNotEmpty(VoteCatalogID)) {
     ...
     if (XString.isEmpty(qs)) {
       ...
       if (flag) {
         ...
         if (XString.isNotEmpty(VoteCatalogID))
           for (int i = 0; i < subjectSet.size(); i++) {
             ...
             if ("W".equals(subjectType)) {
               ...
               if (XString.isNotEmpty(itemTextValue))
                 resultsb.append(value + "$&" + itemTextValue + slit);
               else
                 resultsb.append(value + slit);
             }
             else if (XString.isNotEmpty(value)) {
               String[] arrs = XString.splitEx(value, ",");
         ...
         if ((XString.isNotEmpty(itemIDsb.toString())) && (itemIDsb.length() > 1)) {
           trans.add(
         ...
         trans.add(voteLog, OperateType.INSERT);
         if (trans.commit())
           try {
   ...
   public static void voteResultDetail(HttpServletRequest request, HttpServletResponse response) {
     ...
     String[] resultArr = XString.splitEx(votelog.getResult(), "$|");
     int i = 0; for (int j = 0; i < resultArr.length; i++) {
       String str = resultArr[i];
       if (XString.isEmpty(str)) {
         continue;
       }
       String itemID = XString.splitEx(str, "$&")[0];
       if (XString.isEmpty(itemID)) {
         continue;
       }
       if (itemID.indexOf(":") > 0) {
         ...
         if (XString.isNotEmpty(subjectName)) {
           j++;
         ...
         if (item.getRowCount() > 0) {
           ...
           if (("1".equals(item.getString(0, "ItemType"))) && (XString.splitEx(resultArr[i], "$&").length > 1)) {
             sb.append("&nbsp;&nbsp;选择原因:" + XString.splitEx(resultArr[i], "$&")[1]);
           }
           td.InnerHTML = sb.toString();
         }
       }
...
/*    com.xdarkness.cms.dataservice.VoteResult
