...
import com.xdarkness.schema.ZCNotesSchema;
import com.xdarkness.schema.ZCNotesSet;
...
public class Notes extends Page {
	public static Mapx init(Mapx params) {
		...
		if ((o1 != null) && (o2 != null)) {
			...
			if ("-1".equals(Flag))
				curDate = DateUtil.toString(DateUtil.addMonth(
						DateUtil.parse(s), -1));
		...
		sb
				.append("<table width='100%' cellspacing='0' cellpadding='0' border='1' bordercolor='#aaccee'>");
		sb.append("<tr bgcolor='#DCF0FB' align='center'>");
		sb
				.append("<td width='15%' height='30'><font class='green'>星期日</font></td>");
		...
		firstDayOnWeek = DateUtil.getDayOfWeek(DateUtil
			...
			for (int i = 1; i <= 7; i++) {
				...
				if (((i < firstDayOnWeek) && (n == 1)) || (m > dayInMonth)) {
					...
					QueryBuilder qb = new QueryBuilder(
							"select title,id from zcnotes where notetime=? and adduser=?");
					if (Config.isDB2()) {
						qb
								.setSQL("select title,id from zcnotes where date(notetime)=? and adduser=?");
					}
					qb.add(tDate);
					...
					for (int j = 0; j < dt.getRowCount(); j++) {
						...
						sb
								.append("<a herf='#;' style='cursor:pointer;' onClick='edit("
										+ (m - 1)
										+ ","
										+ dt.get(j, 1).toString()
		...
		map.put("nowDate", DateUtil.toString(DateUtil.parse(curDate),
				"yyyy年MM月"));
...
/*
 * com.xdarkness.cms.document.Notes JD-Core Version: 0.6.0
 */