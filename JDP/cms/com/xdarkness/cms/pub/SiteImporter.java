...
import com.xdarkness.cms.site.Site;
...
public class SiteImporter {
	...
	public boolean importSite(String poolName) {
		...
		this.isNewSite = (("0".equals(this.map.getString("ID"))) || (XString
				.isEmpty(this.map.getString("ID"))));
	...
	public void dealOneEntry(byte[] bs, Object obj) throws Exception {
		...
		if ((obj instanceof String)) {
					...
					if (!this.customTableMap.containsKey(name)) {
						...
						for (int j = 0; j < scs.length; j++) {
							...
							SchemaColumn sc = new SchemaColumn(cdr
									...
									.getString("isMandatory")), "Y".equals(cdr
									.getString("isPrimaryKey")));
							scs[j] = sc;
						}
						TableCreator tc = new TableCreator(this.da
								.getConnection().getDBConfig().DBType);
			...
			dealRelaTable((SiteTableRela.TableRela) obj, (DataTable) FileUtil
					.unserialize(bs));
		}
	}

	private void dealCustomTable(DataTable dt, QueryBuilder qb)
			throws Exception {
		try {
			qb.clearBatches();
			qb.getParams().clear();
	...
	public void dealFile(String fileName, byte[] bs) {
		...
		String root = Config.getContextRealPath()
	...
	public void dealZCSite(DataRow dr) {
		if (this.isNewSite) {
			...
			trans.commit(false);
			addIDMapping("ZCSite", String.valueOf(this.siteID), String
					.valueOf(this.newSiteID));
		} else {
			ZCSiteSchema site = new ZCSiteSchema();
			...
			se.exportSite(Config.getContextRealPath() + "WEB-INF/data/backup/"
					+ site.getAlias() + "_" + System.currentTimeMillis()
					+ ".dat");

			addIDMapping("ZCSite", dr.getString("ID"), String.valueOf(site
					.getID()));
			site.deleteAndBackup();
	...
	public void dealSiteIDTable(String tableName, DataTable dt)
		...
		if (tableName.equalsIgnoreCase("ZCSite")) {
			...
			if (!tableName.equalsIgnoreCase("ZDMember")) {
				...
				QueryBuilder qb = new QueryBuilder("select count(1) from "
						+ tableName + " where 1=1 ");
	...
	public void setSiteIDTableMaxNo(String tableName, DataRow dr,
			boolean newIDFlag) {
		try {
			if (dr.getDataColumn("SiteID") != null)
				dr.set("SiteID", getIDMapping("ZCSite", String
						.valueOf(this.siteID)));
		} catch (Throwable t) {
			t.printStackTrace();
		...
		for (int i = 0; i < this.NoRelas.length; i++) {
			...
			if (nr.TableCode.equalsIgnoreCase(tableName)) {
				...
				if (newIDFlag) {
					if ((nr.TableCode.equals("ZCCatalog"))
							&& (nr.FieldName.equals("InnerCode"))) {
						if (id.length() == 6) {
							id = CatalogUtil.createCatalogInnerCode(null);
						} else {
							String parent = id.substring(0, id.length() - 6);
							parent = getIDMapping(nr.TableCode + "."
									+ nr.FieldName, parent);
				...
				addIDMapping(type + "." + nr.FieldName, dr
						.getString(nr.FieldName), id);
		...
		for (int i = 0; i < this.TableRelas.length; i++)
			if (this.TableRelas[i].TableCode.equals(tableName)) {
				...
				String id = getIDMapping(type + "."
						+ this.TableRelas[i].RelaField, dr
	...
	public void setRelaTableMaxNo(SiteTableRela.TableRela tr, DataRow dr,
			boolean newIDFlag) {
		for (int i = 0; i < this.NoRelas.length; i++) {
			SiteTableRela.NoType nr = this.NoRelas[i];
			if (nr.TableCode.equalsIgnoreCase(tr.TableCode)) {
				...
				addIDMapping(nr.TableCode + "." + nr.FieldName, dr
						.getString(nr.FieldName), id);
				dr.set(nr.FieldName, id);
			}
		}
	...
	public void dealRelaTable(SiteTableRela.TableRela tr, DataTable dt)
			throws Exception {
		...
		SchemaSet set = (SchemaSet) Class.forName(
				"com.xdarkness.schema." + tr.TableCode + "Set").newInstance();
		for (int i = 0; i < dt.getRowCount(); i++) {
			Schema schema = (Schema) Class.forName(
					"com.xdarkness.schema." + tr.TableCode + "Schema")
					.newInstance();
			DataRow dr = dt.getDataRow(i);
	...
	public static Mapx getSiteInfo(String file) throws Exception {
		...
		try {
			...
			bs = ZipUtil.unzip(bs);
			DataTable dt = (DataTable) FileUtil.unserialize(bs);
			CaseIgnoreMapx localCaseIgnoreMapx = dt.getDataRow(0)
					.toCaseIgnoreMapx();
			return localCaseIgnoreMapx;
		} finally {
			if (fin != null)
				try {
					fin.close();
	...
	public byte[] convertCharset(byte[] bs) throws UnsupportedEncodingException {
		if ((Constant.GlobalCharset.equals("GBK"))
				&& (this.ExportCharset.equals("UTF-8"))) {
			return CharsetConvert.webFileUTF8ToGBK(bs);
		}
		if ((Constant.GlobalCharset.equals("UTF-8"))
				&& (this.ExportCharset.equals("GBK"))) {
	...
	public static void main(String[] args) {
		...
		SiteImporter si = new SiteImporter("G:/sky.dat");
		si.setSiteInfo(map);
		si.importSite();
	}
}
...
/*
 * com.xdarkness.cms.pub.SiteImporter JD-Core Version: 0.6.0
 */