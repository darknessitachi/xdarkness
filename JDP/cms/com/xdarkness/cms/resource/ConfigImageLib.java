...
public class ConfigImageLib extends Page {
	...
	public void save() {
		...
		Document doc = null;
		try {
			String configXML = new QueryBuilder(
					"select ConfigXML from zcsite where id = ?", siteID)
					.executeString();
		...
		Element WaterMark = ImageLibConfig.addElement("WaterMark")
				.addAttribute("HasWaterMark",
						$V("HasWaterMark") == null ? "0" : $V("HasWaterMark"))
		...
		WaterMark.addElement("Text").addAttribute("FontName", "宋体")
				.addAttribute("FontSize", $V("FontSize")).addAttribute(
						"FontColor", $V("FontColor")).addText($V("Text"));

		ImageLibConfig.addComment("多缩略图");
		int count = Integer.parseInt($V("Count"));
		Element AbbrImages = ImageLibConfig.addElement("AbbrImages")
				.addAttribute("Count", count + "");
		for (int i = 1; i <= count; i++) {
			String index = $V("AbbrImageIndex" + i);
			Element AbbrImage = AbbrImages.addElement("AbbrImage")
					.addAttribute("ID", i + "").addAttribute("HasAbbrImage",
							$V("HasAbbrImage" + index)).addAttribute(
							"SizeType", $V("SizeType" + index)).addAttribute(
							"Width", $V("Width" + index)).addAttribute(
							"Height", $V("Height" + index));
			WaterMark = AbbrImage.addElement("WaterMark").addAttribute(
					"HasWaterMark",
					$V("HasWaterMark" + index) == null ? "0"
							: $V("HasWaterMark" + index)).addAttribute(
					"WaterMarkType", $V("WaterMarkType" + index)).addAttribute(
					"Position", $V("Position" + index));
			WaterMark.addElement("Image").addAttribute("src",
					$V("Image" + index));
			WaterMark.addElement("Text").addAttribute("FontName", "宋体")
		...
		try {
			...
			writer.flush();
			writer.close();
			new QueryBuilder("update zcsite set ConfigXML=? where ID = ?", sw
					.toString(), siteID).executeNoQuery();
		} catch (IOException e) {
			this.response.setLogInfo(0, "保存失败");
			e.printStackTrace();
			return;
	...
	public void saveSeparate() {
		...
		if (XString.isNotEmpty($V("ImageSeparateURLPrefix"))) {
		...
		if (XString.isNotEmpty($V("VideoSeparateURLPrefix"))) {
		...
		if (XString.isNotEmpty($V("AudioSeparateURLPrefix"))) {
		...
		if (XString.isNotEmpty($V("AttachmentSeparateURLPrefix"))) {
	...
	public static Mapx getImageLibConfig(String siteID) {
		...
		if ((Separate == null) || (Separate.elements().size() == 0)) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				map.put(name + "Flag", (XString.isEmpty(flag))
						|| (flag.equals("N")) ? "N" : "Y");
		...
		for (int i = 0; i < elements.size(); i++) {
			...
			map.put("HasAbbrImage" + ID, AbbrImage
					.attributeValue("HasAbbrImage"));
			...
			map.put("HasWaterMark" + ID, WaterMark
					.attributeValue("HasWaterMark"));
			map.put("WaterMarkType" + ID, WaterMark
					.attributeValue("WaterMarkType"));
	...
	public static Mapx initSeparate(Mapx map) {
		return getImageLibConfig(ApplicationPage.getCurrentSiteID());
	}
}

/*
 * com.xdarkness.cms.resource.ConfigImageLib JD-Core Version: 0.6.0
 */