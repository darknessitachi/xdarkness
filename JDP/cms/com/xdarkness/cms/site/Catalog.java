...
import com.xdarkness.cms.resource.AttachmentLib;
...
public class Catalog extends Page {
	...
	public static Mapx init(Mapx params) {
		...
		if (o1 != null) {
			...
			if (catalog.fill()) {
				...
				if (XString.isEmpty(publishFlag)) {
					publishFlag = "Y";
				}
				map.put("PublishFlag", HtmlUtil.codeToRadios("PublishFlag",
						"YesOrNo", publishFlag));

				String allowContribute = catalog.getAllowContribute();
				if (XString.isEmpty(allowContribute)) {
					allowContribute = "N";
				}
				map.put("AllowContribute", HtmlUtil.codeToRadios(
						"AllowContribute", "YesOrNo", allowContribute));

				String singleFlag = catalog.getSingleFlag();
				if ((XString.isEmpty(singleFlag))
						|| (singleFlag.equals("N")))
					map.put("SingleFlagN", "checked");
				else {
					map.put("SingleFlagY", "checked");
				}
				map.put("SingleFlag", HtmlUtil.codeToRadios("SingleFlag",
						"YesOrNo", singleFlag));
				DataTable workflowDT = new QueryBuilder(
						"select name,ID from ZWWorkflow").executeDataTable();
				map.put("Workflow", HtmlUtil.dataTableToOptions(workflowDT,
						catalog.getWorkflow(), true));

				String keywordFlag = catalog.getKeywordFlag();
				if (XString.isEmpty(keywordFlag)) {
					keywordFlag = "N";
				}

				if (XString.isNotEmpty(catalog.getIndexTemplate())) {
					String s1 = "<a href='javascript:void(0);' onclick=\"openEditor('"
							+ catalog.getIndexTemplate()
							+ "')\"><img src='../Platform/Images/icon_edit.gif'"
							+ " width='14' height='14'></a>";
					map.put("edit", s1);
				}
				if (XString.isNotEmpty(catalog.getListTemplate())) {
					String s1 = "<a href='javascript:void(0);' onclick=\"openEditor('"
				...
				if (XString.isNotEmpty(catalog.getDetailTemplate())) {
					String s1 = "<a href='javascript:void(0);' onclick=\"openEditor('"
				...
				if (XString.isNotEmpty(catalog.getRssTemplate())) {
					String s1 = "<a href='javascript:void(0);' onclick=\"openEditor('"
							+ catalog.getRssTemplate()
							+ "')\"><img src='../Platform/Images/icon_edit.gif'"
							+ " width='14' height='14'></a>";
					map.put("edit3", s1);
				}

				if (XString.isEmpty(catalog.getDetailNameRule())) {
					map.put("DetailNameRule",
							"/${catalogpath}/${document.id}.shtml");
				}

				if (XString.isEmpty(catalog.getListNameRule())) {
					map
							.put("ListNameRule",
									"/${catalogpath}/${year}/${month}/");
				}
	...
	public static Mapx initStructure(Mapx params) {
		...
		QueryBuilder qb = new QueryBuilder(
				"select Name,ID,TreeLevel,ParentID from ZCCatalog ");
		...
		structure = XString.replaceEx(structure, "　", "  ");
	...
	public static Mapx initDialog(Mapx params) {
		...
		params.put("PublishFlag", HtmlUtil.codeToRadios("PublishFlag",
				"YesOrNo", "Y"));
		params.put("SingleFlagN", "checked");
		params.put("AllowContribute", HtmlUtil.codeToRadios("AllowContribute",
				"YesOrNo", "N"));
	...
	public static void treeDataBind(TreeAction ta) {
		...
		String[] codes = getSelectedCatalogList(IDs, CatalogShowConfig
				.getArticleCatalogShowLevel());
		...
		if (ta.isLazyLoad()) {
			...
			if (!CatalogShowConfig.isArticleCatalogLoadAllChild()) {
				qb.append(" and TreeLevel<?",
						Integer.parseInt(parentTreeLevel) + 3);
				ta.setExpand(false);
			} else {
				ta.setExpand(true);
			}
			...
			qb.add(ta.getLevel());
			dt = qb.executeDataTable();
			prepareSelectedCatalogData(dt, codes, catalogType, siteID, ta
					.getLevel());
		}
	...
	public static void prepareSelectedCatalogData(DataTable dt, String[] codes,
			int catalogType, String siteID, int level) {
	...
	public static void addSelectedBranches(TreeAction ta, String[] codes) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			for (int j = 0; j < codes.length; j++)
				if (item.getData().getString("InnerCode").equals(
						codes[j].toString())) {
					item.setLazy(false);
	...
	public static void treeResourceDataBind(TreeAction ta) {
		...
		if (catalogTypeStr == null) {
			catalogTypeStr = ta.getParams().getString(
					"Cookie.ResourceCatalog.CatalogType");
		}
		int catalogType = catalogTypeStr != null ? Integer
				.parseInt(catalogTypeStr.toString()) : 4;
		if (4 == catalogType)
	...
	public void dg1Edit() throws SQLException {
	...
	public void add() {
		...
		if ((catalog != null) && (trans.commit())) {
			...
			UserLogPage.log("Catalog", "AddCataLog", "增加栏目:" + catalog.getName()
					+ "成功！", this.request.getClientIP());
			this.response.setStatus(1);
		} else {
			this.response.setStatus(0);
			UserLogPage.log("Catalog", "AddCataLog", "增加栏目:"
					+ this.request.getString("Name") + "失败！", this.request
					.getClientIP());
			this.response.setMessage("添加栏目发生错误！"
					+ this.request.getString("ErrorMessage"));
		}
	}
	...
	public ZCCatalogSchema add(DataCollection dc, Transaction trans) {
		...
		if ((url == null) || ("".equals(url)) || ("http://".equals($V("URL")))
				|| ("https://".equals($V("URL")))) {
			if (url == null) {
				url = "";
			}
		...
		if (XString.isEmpty(listPageSize))
		...
		trans
				.add(new QueryBuilder(
						"update zccatalog set orderflag=orderflag+1 where orderflag>? and type=?",
						orderFlag, type));
	...
	public long add(ZCCatalogSchema parent, ZCCatalogSchema catalog,
			Transaction trans) {
		...
		if (parent != null) {
			...
			parent.setIsLeaf(0L);
			trans.add((ZCCatalogSchema) parent.clone(), OperateType.UPDATE);
		} else {
			ZCSiteSchema site = new ZCSiteSchema();
			site.setID(ApplicationPage.getCurrentSiteID());
			site.fill();
		...
		String innerCode = CatalogUtil.createCatalogInnerCode(parent
				.getInnerCode());
		catalog.setInnerCode(innerCode);
		catalog.setBranchInnerCode(User.getBranchInnerCode());
		catalog.setAddUser(User.getUserName());
	...
	public static void initCatalogConfig(ZCCatalogSchema catalog,
			Transaction trans) {
	...
	public void save() {
		...
		if ((!name.equals(catalog.getName()))
				&& (checkNameExists(name, catalog.getParentID()))) {
			this.response.setLogInfo(0, "栏目名称" + name + "已经存在。");
			UserLogPage.log("Catalog", "UpdateCataLog", "修改栏目:" + catalog.getName()
		...
		if (XString.isEmpty($V("ListPageSize"))) {
		...
		if ("1".equals(wfExtend)) {
			DataTable tempDT = new QueryBuilder(
			...
			if ((tempDT.getRowCount() > 0)
					&& (XString.isNotEmpty(oldWorkflow))
			...
			if (tempDT.getRowCount() > 0) {
				...
				for (int i = 0; i < tempDT.getRowCount(); i++) {
					if (!CatalogUtil.getWorkflow(tempDT.getString(i, 0))
						...
						tempStr = tempStr
								+ CatalogUtil.getName(tempDT.getString(i, 0))
								+ " ";
					}
				}
				if (XString.isNotEmpty(tempStr)) {
					this.response.setLogInfo(0, tempStr
							+ "栏目下还存在‘流转中’的文章,不能更改工作流!");
					return;
				}
			}
	...
	public static void importTreeDataBind(TreeAction ta) {
		...
		String type = ta.getParams().get("Type") == null ? null : ta
		...
		if (parentID == 0L) {
			...
			rootText = new QueryBuilder(
					"select Name from ZCCatalog where ID=?", String
	...
	private static DataTable getTreeDataTable(String text) {
		...
		for (int i = 0; i < catalogs.length; i++) {
			...
			if (XString.isEmpty(catalog.trim())) {
				continue;
			}
			catalog = XString.toDBC(catalog);
			catalog = XString.rightTrim(catalog);
			int length = 0;
			...
			Object[] catalogRow = { new Long(i + 1), new Long(0L),
					new Integer(currentLevel), catalog.trim() };
	...
	public void importCatalog() {
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			...
			catalog.setURL(url);
			if ((XString.isNotEmpty(type)) && (!"null".equals(type)))
	...
	public void move() {
		...
		if (parentID == 0L) {
			...
			tran.add(site, OperateType.UPDATE);
		} else {
			ZCCatalogSchema pCatalog = new ZCCatalogSchema();
			...
			if ((catalog.getTreeLevel() == TreeLevel)
					&& (catalog.getParentID() == parentID)) {
				this.response.setStatus(0);
				this.response.setMessage("选择的栏目和目标栏目处于同一级，不能转移。");
				return;
			}
			...
			pCatalog.setChildCount(pCatalog.getChildCount() + 1L);
			pCatalog.setIsLeaf(0L);
			tran.add(pCatalog, OperateType.UPDATE);
		}
		...
		if (oldParentID == 0L) {
			...
			tran.add(new QueryBuilder(
					"update zccatalog set childcount=childcount-1 where id=?",
					oldParentID));
			tran
					.add(new QueryBuilder(
							"update zccatalog set isLeaf=1 where childcount=0 and id=?",
							oldParentID));
		}

		String orderflag = getCatalogOrderFlag(parentID, catalog.getType());
		String innerCode = TreeLevel == 1L ? CatalogUtil
				.createCatalogInnerCode("") : CatalogUtil
				.createCatalogInnerCode(parentInnerCode);

		CatalogStat.updateInnerCode(tran, catalog.getSiteID(), catalog
		...
		tran.add(catalog, OperateType.UPDATE);
		tran.add(new QueryBuilder("update zccatalog set orderflag = "
				+ orderflag + 1 + " where id =?", catalogID));
		tran.add(new QueryBuilder(
				"update zcarticle set CatalogInnerCode=? where catalogid=?",
				innerCode, catalogID));
		tran.add(new QueryBuilder(
				"update bzcarticle set CatalogInnerCode=? where catalogid=?",
				innerCode, catalogID));

		Mapx map = new Mapx();
		if (catalog.getChildCount() > 0L) {
			ZCCatalogSet childCatalogSet = new ZCCatalogSchema()
					.query(new QueryBuilder("where parentid=?", catalogID));
			for (int i = 0; i < childCatalogSet.size(); i++) {
				Mapx childMap = moveChild(childCatalogSet.get(i), TreeLevel,
						innerCode, orderflag + 2 + i);
				map.putAll(childMap);
			}
		}
		...
		String count = new QueryBuilder(
				"select count(*) from zccatalog where innercode like '"
						+ CatalogUtil.getInnerCode(catalogID) + "%' and type="
						+ catalog.getType()).executeString();
		tran.add(new QueryBuilder("update zccatalog set orderflag=" + orderflag
				+ count + " where orderflag > " + orderflag + " and type="
				+ catalog.getType() + " and innercode not like '"
	...
	public Mapx moveChild(ZCCatalogSchema catalog, long TreeLevel,
			String parentInnerCode, String orderflag) {
		Mapx map = new Mapx();
		...
		map.put("update zccatalog set TreeLevel=" + TreeLevel + ",innercode='"
				+ innerCode + "',modifyuser='" + User.getUserName()
				+ "',modifytime='" + DateUtil.getCurrentDate() + "',OrderFlag="
				+ orderflag + " where id=" + catalogID, new Integer(7));

		map.put("update zcarticle set CatalogInnerCode='" + innerCode
				+ "' where catalogid=" + catalogID, new Integer(7));
		map.put("update bzcarticle set CatalogInnerCode='" + innerCode
				+ "' where catalogid=" + catalogID, new Integer(7));
	...
	public void publish() {
		if ("0".equals($V("type"))) {
			long id = publishAllTask(ApplicationPage.getCurrentSiteID());
			this.response.setStatus(1);
			$S("TaskID", id);
		} else {
			long catalogID = Long.parseLong($V("CatalogID"));
	...
	public void del() {
		...
		if (trans.commit()) {
			...
			UserLogPage.log("Site", "DelCataLog", "删除栏目成功:" + name, this.request
					.getClientIP());

			this.response.setStatus(1);
		} else {
			UserLogPage.log("Site", "DelCataLog", "删除栏目失败:" + name, this.request
					.getClientIP());
			this.response.setStatus(0);
	...
	public void batchDel() {
		String IDs = $V("IDs");
		if ((XString.isEmpty(IDs)) || (IDs.equals("0"))) {
		...
		for (int i = 0; i < arr.length; i++) {
			String id = arr[i];
			if ((XString.isNotEmpty(id)) && (!id.equals("0"))) {
				String innerCode = CatalogUtil.getInnerCode(id);
		...
		if (trans.commit()) {
			for (int i = 0; i < catalogs.length; i++) {
				...
				UserLogPage.log("Site", "DelCataLog", "删除栏目成功:" + name,
			...
			UserLogPage.log("Site", "DelCataLog", "批量删除栏目失败", this.request
					.getClientIP());
			this.response.setStatus(0);
	...
	public void clean() {
		...
		trans
				.add(new QueryBuilder(
						...
						ID));
		trans.add(new QueryBuilder("delete from zcarticle where catalogid=?",
				ID));

		trans
				.add(new QueryBuilder(
						"delete from zdcolumnvalue where relatype=? and exists(select id from zcarticle where catalogid=? and id=zdcolumnvalue.relaid)",
						"2", ID));

		trans
				.add(new QueryBuilder(
						"delete from zcimagerela where RelaType=? and exists(select id from zcarticle where catalogid=? and id=zcimagerela.RelaID)",
						"ArticleImage", ID));

		trans
				.add(new QueryBuilder(
						"delete from ZCAttachmentRela where RelaType=? and exists(select id from zcarticle where catalogid=? and id=ZCAttachmentRela.RelaID)",
						"ArticleAttach", ID));

		trans
				.add(new QueryBuilder(
						"delete from ZCVideoRela where RelaType=? and exists(select id from zcarticle where catalogid=? and id=ZCVideoRela.RelaID)",
						"ArticleVideo", ID));

		trans
				.add(new QueryBuilder(
						"delete from ZCAudioRela where exists(select id from zcarticle where catalogid=? and id=ZCAudioRela.RelaID)",
						ID));

		trans
				.add(new QueryBuilder(
						"delete from ZCVoteItem where exists(select id from zcarticle where catalogid=? and id=ZCVoteItem.VoteDocID)",
						ID));

		trans
				.add(new QueryBuilder(
						"delete from ZCComment where exists(select id from zcarticle where catalogid=? and id=ZCComment.RelaID)",
						ID));
	...
	public static void deleteCatalog(Transaction trans, long ID) {
		...
		trans.add(new ZCCatalogConfigSchema().query(new QueryBuilder(
				" where siteID=? and CatalogInnerCode like ?", catalog
						.getSiteID(), catalog.getInnerCode() + "%")),
				OperateType.DELETE_AND_BACKUP);
		...
		trans.add(blockSet, OperateType.DELETE_AND_BACKUP);

		String idsStr = "'" + ids.replaceAll(",", "','") + "'";

		ZDColumnRelaSet columnRelaSet = new ZDColumnRelaSchema()
				.query(new QueryBuilder(" where RelaType=? and RelaID in("
						+ idsStr + ")", "1"));
		trans.add(columnRelaSet, OperateType.DELETE_AND_BACKUP);

		ZDColumnValueSet columnValueSet1 = new ZDColumnValueSchema()
				.query(new QueryBuilder(" where RelaType=? and RelaID in("
						+ idsStr + ")", "1"));
		trans.add(columnValueSet1, OperateType.DELETE_AND_BACKUP);
		...
		if (Config.isDB2()) {
			wherepart = " where exists (select ID from zcarticle where cataloginnercode like '"
					+ catalog.getInnerCode()
					+ "%' and char(ID)=zdcolumnvalue.relaID )";
		}
		ZDColumnValueSet columnValueSet2 = new ZDColumnValueSchema()
				.query(new QueryBuilder(wherepart));
		trans.add(columnValueSet2, OperateType.DELETE_AND_BACKUP);

		if (catalog.getType() == 3L) {
			DataTable delids = new QueryBuilder(
					"select * from ZCMagazineIssue where ID not in (" + ids
							+ ") order by Year desc,PeriodNum desc")
					.executeDataTable();
			if ((delids != null) && (delids.getRowCount() > 0)) {
				...
				if (magazine.fill()) {
					...
					if ((currentyear.equalsIgnoreCase(issues.getYear()))
							&& (periodnum.equalsIgnoreCase(issues
									.getPeriodNum()))) {
						curentyearflag = true;
					} else {
				...
				trans.add(new ZCMagazineSchema().query(new QueryBuilder(
						"where ID in (" + ids + ")")), OperateType.DELETE_AND_BACKUP);
	...
	public static void createDefaultCatalog(long siteID, int type) {
		DataTable dt = new QueryBuilder(
				"select id from zccatalog where siteid =?  and type=?", siteID,
				type).executePagedDataTable(1, 0);
		...
		defaultCatalog.setAlias(XString.getChineseFirstAlpha(Name));
		...
		trans.add(defaultCatalog, OperateType.INSERT);
		trans.commit();
	}
	...
	public static boolean sortCatalog(String CatalogID, int Move,
			String CatalogType) {
		ZCCatalogSchema catalog = new ZCCatalogSchema();
		catalog.setID(CatalogID);
		if (!catalog.fill()) {
		...
		DataTable allDT = new QueryBuilder(
				"select * from zccatalog where siteID = ? and Type =? order by orderflag,innercode",
				ApplicationPage.getCurrentSiteID(), CatalogType).executeDataTable();

		int selfSize = new QueryBuilder(
				"select count(1) from zccatalog where siteID = "
						+ ApplicationPage.getCurrentSiteID()
						+ " and innercode like '" + catalog.getInnerCode()
		...
		if (Move > 0) {
			...
			for (int i = allDT.getRowCount() - 1; i >= 0; i--) {
				if ((isMove)
						...
						&& ((TreeLevel.equals("1")) || (allDT.getString(i,
								"InnerCode").startsWith(catalog.getInnerCode()
								.substring(0,
										catalog.getInnerCode().length() - 6))))) {
					count++;
				}
				if (allDT.getString(i, "ID").equals(CatalogID)) {
					...
					if (allDT.getString(i, "InnerCode").startsWith(
							catalog.getInnerCode())) {
						continue;
					}
				...
				if ((count != Move)
						&& ((count == 0) || (i - 1 < 0) || (count >= Move) || (Integer
								.parseInt(allDT.getString(i - 1, "TreeLevel")) >= Integer
								.parseInt(TreeLevel)))
			...
			if (count > 0) {
				...
				for (int i = 0; i < catalogList.size(); i++) {
					...
					trans.add(new QueryBuilder(
							"update zccatalog set orderflag = ? where ID = ?",
			...
			for (int i = 0; i < allDT.getRowCount(); i++) {
				...
				if ((count != Move)
						&& ((count == 0) || (i + 1 >= allDT.getRowCount())
								|| (count >= Move) || (Integer.parseInt(allDT
								.getString(i + 1, "TreeLevel")) >= Integer
								.parseInt(TreeLevel)))
						&& ((count == 0) || (count >= Move) || (i != allDT
								.getRowCount() - 1)))
					continue;
	...
	public static String checkAliasExists(String alias, long parentID) {
		return checkAliasExists(alias, ApplicationPage.getCurrentSiteID(), parentID);
	}

	public static String checkAliasExists(String alias, long siteID,
			long parentID) {
		QueryBuilder qb = new QueryBuilder(
				"select name from zccatalog where alias=? and parentID=? and siteID=?",
				alias, parentID);
		qb.add(siteID);
	...
	public static boolean checkNameExists(String Name, long parentID) {
		int count = new QueryBuilder(
				"select count(*) from zccatalog where name=? and parentID=? and siteID="
						+ ApplicationPage.getCurrentSiteID(), Name, parentID)
				.executeInt();
		return count > 0;
	}
	...
	public static String getCatalogOrderFlag(String ParentID, String CatalogType) {
		...
		if ((XString.isEmpty(ParentID)) || ("0".equals(ParentID))) {
			parentDT = new QueryBuilder(
					"select * from zccatalog where siteID =? and type =? and TreeLevel = 1 order by orderflag,innercode",
					ApplicationPage.getCurrentSiteID(), CatalogType)
					.executeDataTable();
		} else {
			String innercode = CatalogUtil.getInnerCode(ParentID);
			parentDT = new QueryBuilder(
					"select * from zccatalog where siteID =? and type =? and innercode like '"
							+ innercode + "%' order by orderflag,innercode",
					ApplicationPage.getCurrentSiteID(), CatalogType)
					.executeDataTable();
		}
		...
		for (int k = 1; k < 30; k++)
			for (int i = 0; i < set.size(); i++) {
				...
				if (schema.getTreeLevel() == 1L)
					schema.setInnerCode(XString.leftPad(String
							.valueOf(Integer.parseInt(schema.getInnerCode()
									.substring(0, 4))
									+ set.size() * k / 3), '0', 4));
...
/*
 * com.xdarkness.cms.site.Catalog JD-Core Version: 0.6.0
 */