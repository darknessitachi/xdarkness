...
public class SourceStat extends AbstractStat {
	...
	public void deal(Visit v) {
		...
		if (!this.siteMap.containsKey(new Long(v.SiteID))) {
			VisitCount.getInstance().initLRUMap(v.SiteID, "Source", "Keyword",
					1000, null);
			VisitCount.getInstance().initLRUMap(v.SiteID, "Source", "Referer",
					1000, null);
			this.siteMap.put(new Long(v.SiteID), "");
		}
		VisitCount.getInstance().add(v.SiteID, "Source", "Host", v.Host);
		if (v.UVFlag)
			if ((XString.isEmpty(v.Referer))
					&& (v.URL.indexOf("Result.jsp") < 0)) {
				...
				if (se == null) {
					...
					if (!domain.equalsIgnoreCase(v.Host))
						VisitCount.getInstance().add(v.SiteID, "Source",
								"Referer", domain);
					else
						VisitCount.getInstance().add(v.SiteID, "Source",
								"Direct", "0");
					...
					VisitCount.getInstance().add(v.SiteID, "Source",
							"SearchEngine", se[0]);
					VisitCount.getInstance().add(v.SiteID, "Source", "Keyword",
							se[1]);
				}
			}
	}
	...
	public void onPeriodChange(int type, long current) {
		if (type == 1) {
			...
			if (period.endsWith("01")) {
				...
				VisitCount.getInstance().clearSubType(getStatType(), "Keyword",
						true);
				VisitCount.getInstance().clearSubType(getStatType(), "Referer",
						true);
			}
		}
	}
	...
	public void update(Transaction tran, VisitCount vc, long current,
		...
		if (!newMonthFlag) {
			...
			for (int i = 0; i < sites.length; i++) {
				URLStat.dealNotNeedInsertItem(vc, period, sites[i], "Source",
						"Referer");
				URLStat.dealNotNeedInsertItem(vc, period, sites[i], "Source",
						"Keyword");
			}
	...
	public static String[] getSearchEngine(Visit v) {
		...
		if (domain.indexOf("baidu.") > 0) {
			keyword = XString.urlDecode(map.getString("wd"), "GBK");
			name = "百度";
		} else if (domain.indexOf("google.") > 0) {
			String charset = map.getString("ie");
			if (XString.isEmpty(charset)) {
				charset = "UTF-8";
			}
			keyword = XString.urlDecode(map.getString("q"), charset);
			name = "谷歌";
			...
			keyword = XString.urlDecode(map.getString("p"), charset);
			name = "雅虎";
		} else if (domain.indexOf("msn.") > 0) {
			keyword = XString.urlDecode(map.getString("q"), "UTF-8");
			name = "MSN";
		} else if (domain.indexOf("soso.") > 0) {
			keyword = XString.urlDecode(map.getString("w"), "GBK");
			name = "搜搜";
		} else if (domain.indexOf("sogou.") > 0) {
			keyword = XString.urlDecode(map.getString("query"), "GBK");
			name = "搜狗";
		} else if (domain.indexOf("zhongsou.") > 0) {
			keyword = XString.urlDecode(map.getString("word"), "GBK");
			name = "中搜";
...
/*
 * com.xdarkness.cms.stat.impl.SourceStat JD-Core Version: 0.6.0
 */