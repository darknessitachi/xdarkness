...
public class DistrictReport extends Page {
	public void getChartData() {
		...
		DataTable dt = null;
		if ("C".equals(type)) {
			dt = getCountryTable(ApplicationPage.getCurrentSiteID(), start, end,
					false);
		} else if ((XString.isEmpty(code)) || (code.equals("null"))
				|| (code.equals("000000")))
			dt = getProvinceTable(ApplicationPage.getCurrentSiteID(), start, end,
					false);
		else {
			dt = getCityTable(ApplicationPage.getCurrentSiteID(), code, start, end,
					false);
		}
	...
	public static DataTable getCountryTable(long siteID, Date start, Date end,
			boolean chartFlag) {
		...
		for (int k = 0; k < maps.length; k++) {
			...
			for (int i = map.size() - 1; i >= 0; i--) {
				...
				if ((!district.equals("000000"))
						&& (!district.startsWith("000"))) {
					map.put("000000", map.getInt("000000")
							+ map.getInt(district));
					map.remove(district);
				}
			}
		...
		Mapx map = new QueryBuilder(
				"select code,name from ZDDistrict where TreeLevel<3")
				.executeDataTable().toMapx(0, 1);
		map.put("000999", "未知区域");
		for (int i = 0; i < dt.getRowCount(); i++) {
			String item = dt.getString(i, "Item");
			if ((chartFlag) && (item.equals("000000")))
				item = "<a href='District.jsp?Type=P&Code=000000'>"
						+ map.getString(item) + "</a>";
			else {
	...
	public static DataTable getProvinceTable(long siteID, Date start, Date end,
			boolean chartFlag) {
		DataTable src = getDistrictTable(siteID, start, end);
		Mapx pvMap = src.toMapx("District", "PV");
		Mapx uvMap = src.toMapx("District", "UV");
		...
		for (int k = 0; k < maps.length; k++) {
			...
			for (int i = map.size() - 1; i >= 0; i--) {
				...
				if ((!district.equals("000000"))
						&& ((district.startsWith("000")) || (!district
								.endsWith("0000")))) {
					map.remove(district);
				}
			}
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			String item = dt.getString(i, "Item");
			if (chartFlag)
				item = "<a href='District.jsp?Type=P&Code=" + item + "'>"
						+ map.getString(item) + "</a>";
			else {
				item = map.getString(item);
			}
	...
	public static DataTable getCityTable(long siteID, String province,
			Date start, Date end, boolean chartFlag) {
		DataTable src = getDistrictTable(siteID, start, end);
		Mapx pvMap = src.toMapx("District", "PV");
		Mapx uvMap = src.toMapx("District", "UV");
		Mapx ipMap = src.toMapx("District", "IP");
...
/*
 * com.xdarkness.cms.stat.report.DistrictReport JD-Core Version: 0.6.0
 */