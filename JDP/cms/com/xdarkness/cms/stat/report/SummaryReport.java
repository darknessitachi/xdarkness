...
import com.xdarkness.cms.stat.VisitHandler;
...
public class SummaryReport extends Page {
	public static Mapx init(Mapx params) {
		String startDate = DateUtil.toString(ReportUtil
				.getStartStatDate(ApplicationPage.getCurrentSiteID()));
		int totalDays = ReportUtil.getTotalStatDays(ApplicationPage
				.getCurrentSiteID());
	...
	public static void treeDataBind(TreeAction ta) {
		...
		dt
				.insertRow(new Object[] { "16", "10", "是否支持Applet",
						"JavaEnabled.jsp" });
		dt
				.insertRow(new Object[] { "17", "10", "Flash版本",
						"FlashVersion.jsp" });
		dt.insertRow(new Object[] { "18", "10", "是否允许Cookie",
				"CookieEnabled.jsp" });
	...
	public static void dg1DataBind(DataGridAction dga) {
		dga.bindData(getSummaryTable(ApplicationPage.getCurrentSiteID()));
	}
	...
	public static void dgSiteDataBind(DataGridAction dga) {
		...
		DataTable dt = getDayHitData(ApplicationPage.getCurrentSiteID(), start, end);
		dga.bindData(dt);
	}
	...
	public static DataTable getDateRangeTable(long siteID, String start,
			String end) {
	...
	public static DataTable getSummaryTable(long siteID) {
		...
		QueryBuilder qb = new QueryBuilder(
				"select * from ZCStatItem where SiteID=? and Type='Global' order by period desc",
				siteID);
		...
		for (int i = 0; i < src.getRowCount(); i++) {
			...
			if ((!monthFlag) && (!period.startsWith(monthPrefix))) {
				...
				vs[4] = new Double(NumberUtil.round(totals[1] == 0 ? 0.0D
						: totals[3] * 100.0D / totals[1], 2));
			...
			int returnVisitor = 0;
			if (XString.isNotEmpty(src.getString(i, "ReturnVisitor"))) {
				returnVisitor = Integer.parseInt(src.getString(i,
						"ReturnVisitor"));
			}
			int stickTotalTime = 0;
			if (XString.isNotEmpty(src.getString(i, "StickTotalTime"))) {
				stickTotalTime = Integer.parseInt(src.getString(i,
						"StickTotalTime"));
			}
			int stickTime = 0;
			if (XString.isNotEmpty(src.getString(i, "StickTime"))) {
				stickTime = Integer.parseInt(src.getString(i, "StickTime"));
			}
			int visitDepth = 0;
			if (XString.isNotEmpty(src.getString(i, "VisitDepth"))) {
			...
			if (i == 0) {
				if (!period.equals(DateUtil.toString(today))) {
					...
					pv = (int) (pv + VisitCount.getInstance().get(siteID,
							"Global", "PV", "0"));
					uv = (int) (uv + VisitCount.getInstance().get(siteID,
							"Global", "UV", "0"));
					ip = (int) (ip + VisitCount.getInstance().get(siteID,
							"Global", "IP", "0"));
					returnVisitor = (int) (returnVisitor + VisitCount
							.getInstance().get(siteID, "Global",
									"ReturnVisitor", "0"));
					...
					vs[2] = new Integer(uv);
					vs[3] = new Integer(ip);
					vs[4] = new Double(NumberUtil.round(uv == 0 ? 0.0D
							: returnVisitor * 100.0D / uv, 2));
					...
					int minute = Integer.parseInt(DateUtil
							.toString(today, "mm"));
					qb = new QueryBuilder(
							"select * from ZCStatItem where SiteID=? and Type='Hour' and period=?",
							siteID, DateUtil.toString(yesterday, "yyyyMM"));
					DataTable dtHour = qb.executeDataTable();
					dtHour = ReportUtil.toItemTable(dtHour, yesterday,
							yesterday);
					...
					if ((hour != 0) && (dtHour.getRowCount() != 0)) {
						...
						for (int j = 0; j < dtHour.getRowCount(); j++) {
							...
							if (item != hour) {
								gpv += Integer.parseInt(dtHour.getString(j,
										"PV"));
								guv += Integer.parseInt(dtHour.getString(j,
										"UV"));
								gip += Integer.parseInt(dtHour.getString(j,
										"IP"));
							} else {
								gpv += Integer.parseInt(dtHour.getString(j,
										"PV"))
						...
						vs[0] = "今日预计";
						if (gpv != 0)
							vs[1] = new Integer(new Double(pv * 1.0D / gpv
									* ypv).intValue());
						else {
							vs[1] = new Integer(new Double(pv * 24 / hour)
									.intValue());
						}
						if (guv != 0)
							vs[2] = new Integer(new Double(uv * 1.0D / guv
									* yuv).intValue());
						else {
							vs[2] = new Integer(new Double(uv * 24 / hour)
									.intValue());
						}
						if (gip != 0)
							vs[3] = new Integer(new Double(ip * 1.0D / gip
									* yip).intValue());
						else {
							vs[3] = new Integer(new Double(ip * 24 / hour)
									.intValue());
						}
						dt.insertRow(vs);
					}
		...
		vs[2] = new Integer(totals[1]);
		vs[3] = new Integer(totals[2]);
		vs[4] = new Double(NumberUtil.round(totals[1] == 0 ? 0.0D : totals[3]
				* 100.0D / totals[1], 2));
		vs[5] = new Integer(totals[4]);
		vs[6] = new Integer(totals[5]);
		vs[7] = new Integer(totals[6]);
...
/*
 * com.xdarkness.cms.stat.report.SummaryReport JD-Core Version: 0.6.0
 */