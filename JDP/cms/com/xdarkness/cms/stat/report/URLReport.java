...
public class URLReport extends Page {
	public static void dgTopDataBind(DataGridAction dga) {
		...
		DataTable dt = getTopURLData(ApplicationPage.getCurrentSiteID(), start, end);
	...
	public static void dgEntranceDataBind(DataGridAction dga) {
		...
		DataTable dt = getEntranceData(ApplicationPage.getCurrentSiteID(), start,
				end);
	...
	public static void dgExitDataBind(DataGridAction dga) {
		...
		DataTable dt = getExitData(ApplicationPage.getCurrentSiteID(), start, end);
	...
	public static DataTable getTopURLData(long siteID, Date start, Date end) {
		...
		for (int i = dt.getRowCount() - 1; i >= 0; i--) {
			...
			if (c == 0)
				...
				dt
						.set(i, "Rate", NumberUtil.round(c * 100.0D / total, 2)
								+ "%");
			}
		}
		return dt;
