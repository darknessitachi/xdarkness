...
import com.xdarkness.cms.dataservice.Form;
...
import com.xdarkness.cms.stat.report.SourceReport;
...
import com.xdarkness.statical.TemplateData;
import com.xdarkness.framework.DBUtil;
import com.xdarkness.framework.jaf.controls.HtmlP;
...
public class CmsTemplateData extends TemplateData {
	...
	public void setCatalog(DataRow catalog) {
		...
		ColumnUtil.extendCatalogColumnData(this.catalog, this.site
				.getLong("ID"), getLevelStr());
	}
	...
	public DataTable getList(String item, String name, String displayLevel,
			String count) {
		return getCatalogList(item, "article", name, displayLevel, count, null);
	}

	public DataTable getList(String item, String name, String displayLevel,
			String count, String condition) {
		return getCatalogList(item, "article", name, displayLevel, count,
				condition);
	}

	public DataTable getCatalogList(String item, String catalogType,
			String name, String displayLevel, String count, String condition) {
		DataTable dt = null;

		String key = item + "_" + catalogType + "_" + displayLevel + "_" + name
				+ "_" + count + "_" + condition + "_" + this.level;
		...
		if ("catalog".equalsIgnoreCase(item)) {
			...
			if ((XString.isNotEmpty(name))
					&& (!"null".equalsIgnoreCase(name)) && (this.site != null)) {
				if (name.indexOf("/") != -1)
					parentID = CatalogUtil.getIDByNames(this.site
							.getString("ID"), name);
				else {
					parentID = CatalogUtil.getIDByName(this.site
							.getString("ID"), name);
				}

				if ((XString.isEmpty(parentID))
						&& (XString.isDigit(name))) {
					parentID = name;
				}
			...
			if ("root".equalsIgnoreCase(displayLevel)) {
				...
				if ((XString.isEmpty(parentID)) && (this.catalog != null)) {
				...
				parentSql = " and innercode like '"
						+ CatalogUtil.getInnerCode(parentID) + "%'";
			} else {
				if ((XString.isEmpty(parentID)) && (this.catalog != null)) {
					if ("1".equals(this.catalog.getString("isLeaf")))
						parentID = this.catalog.getString("ParentID");
			...
			if ((XString.isEmpty(condition))
					|| ("null".equalsIgnoreCase(condition)))
				condition = "";
			else {
				condition = " and " + condition;
			}
			sql = "select * from zccatalog where siteID=? and PublishFlag='Y' "
					+ parentSql + typeSql + condition
					+ " Order by orderflag,id ";
			if ("magazine".equalsIgnoreCase(catalogType)) {
				sql = "select * from zccatalog where siteID=? " + parentSql
						+ typeSql + condition
			...
			for (int i = 0; i < dt.getRowCount(); i++) {
				dt.set(i, "_RowNo", i + 1);
				dt.set(i, "ImagePath", levelString
						+ dt.getString(i, "ImagePath"));
				String url = CatalogUtil.getLink(dt.getString(i, "ID"),
						levelString);
				dt.set(i, "Link", url);
			}
			ColumnUtil.extendCatalogColumnData(dt, this.site.getLong("ID"),
					levelString);
		}
	...
	public DataTable getDocList(String item, String catalogName,
			String displayLevel, String type, String count) {
		return getDocList(item, catalogName, displayLevel, null, type, count,
				"");
	}

	public DataTable getDocList(String item, String catalogName, String type,
			String count) {
		return getDocList(item, catalogName, "all", null, type, count, "");
	}

	public DataTable getDocList(String item, String catalogName,
			String displayLevel, String orderType, String count,
			String condition) {
		return getDocList(item, catalogName, displayLevel, "", orderType,
				count, condition);
	}

	public DataTable getDocList(String item, String catalogName,
			String displayLevel, String hasAttribute, String orderType,
			String countStr, String condition) {
		DataTable dt = null;
		String key = item + "_" + catalogName + "_" + displayLevel + "_"
				+ hasAttribute + "_" + orderType + "_" + countStr + "_"
				+ condition + "_" + this.level;
		...
		dt = getPagedDocList(item, catalogName, displayLevel, hasAttribute,
				orderType, condition, pageSize, 0);
		TemplateCache.setDataTable(key, dt);
		return dt;
	}

	public DataTable getPagedDocList(String item, String catalogName,
			String displayLevel, String hasAttribute, String orderType,
			String condition, int pageSize, int pageIndex) {
		DataTable dt = null;

		QueryBuilder qb = getDocListQueryBuilder(item, catalogName,
				displayLevel, hasAttribute, orderType, condition, "*");
		...
		if (("article".equalsIgnoreCase(item))
			...
			dt.insertColumns(new String[] { "FirstImagePath",
					"FirstVideoImage", "FirstVideoHtml", "FirstTag",
					"FirstTagUrl" });
			dealTag(dt);
			...
			if (("article".equalsIgnoreCase(item))
				...
				if ("4".equals((String) dt.get(i, "Type"))) {
					...
					content = content.replaceAll(imagePath, PubFun
							.getLevelStr(CatalogUtil
									.getDetailLevel(docCatalogID)));
					content = content.replaceAll(attachPath, Config
							.getValue("ServicesContext")
							+ "AttachDownLoad.jsp");
				}

				PubFun.dealArticleMedia(dt, levelString,
						"image,attchment,video");
				...
				if ((imageRelaTable != null)
						&& (imageRelaTable.getRowCount() > 0)) {
					String articleCatalogID = imageRelaTable.getString(0,
							"catalogid");
					String articleID = imageRelaTable.getString(0, "id");
					String nameRule = CatalogUtil.getSchema(articleCatalogID)
							.getDetailNameRule();
					if (XString.isNotEmpty(nameRule)) {
						HtmlNameParser nameParser = new HtmlNameParser(
								this.site, null, imageRelaTable.get(0),
								nameRule);
				...
				columnValue[i] = (levelString
						+ CatalogUtil.getPath(docCatalogID) + docID + ".shtml");
			} else if ("video".equalsIgnoreCase(item)) {
				...
				for (int n = 0; n < vCount; n++) {
					...
					path = path + dt.getString(i, "path") + n + "_"
							+ dt.getString(i, "FileName");
				}
				dt.set(i, "filePath", path);
				String nameRule = CatalogUtil.getSchema(docCatalogID)
						.getDetailNameRule();
				if (XString.isNotEmpty(nameRule)) {
					HtmlNameParser nameParser = new HtmlNameParser(this.site,
							null, dt.get(i), nameRule);
					HtmlNameRule h = nameParser.getNameRule();
				...
				dt.set(i, "RelaVideoHtml", PubFun
						.getVideoHtml(dt.getDataRow(i)));
			} else if ("attachment".equalsIgnoreCase(item)) {
				if ((XString.isNotEmpty(docCatalogID))
						&& ("N".equals(CatalogUtil
								.getAttachDownFlag(docCatalogID))))
					columnValue[i] = (levelString + dt.getString(i, "path") + dt
							.getString(i, "filename"));
				else if ("N".equals(SiteUtil.getAttachDownFlag(dt.getString(i,
						"SiteID"))))
					columnValue[i] = (levelString + dt.getString(i, "path") + dt
							.getString(i, "filename"));
				else
					columnValue[i] = (Config.getValue("ServicesContext")
							+ "/AttachDownLoad.jsp?id=" + docID);
			} else {
				String nameRule = CatalogUtil.getSchema(docCatalogID)
						.getDetailNameRule();
				if (XString.isNotEmpty(nameRule)) {
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			dt.set(i, "BranchName", PubFun.getBranchName(dt.getString(i,
					"BranchInnerCode")));
		}

		if ((this.doc != null)
				&& (("relate".equalsIgnoreCase(orderType)) || ("recommend"
	...
	public int getPagedDocListTotal(String item, String catalogName,
			String displayLevel, String hasAttribute, String orderType,
			String condition) {
		QueryBuilder qb = getDocListQueryBuilder(item, catalogName,
				displayLevel, hasAttribute, orderType, condition, "*");
	...
	public QueryBuilder getDocListQueryBuilder(String item, String catalogName,
			String displayLevel, String hasAttribute, String orderType,
			String condition, String cols) {
		String typeStr = "";
		String key = item + "_" + catalogName + "_" + displayLevel + "_"
				+ hasAttribute + "_" + orderType + "_" + condition + "_" + cols
				+ "_querybuilder";
		...
		if (("article".equalsIgnoreCase(item))
			...
			if (("relate".equalsIgnoreCase(orderType)) && (this.doc != null)) {
				...
				typeStr = typeStr
					...
					&& (this.doc != null)) {
				String recommendArticle = this.doc
						.getString("RecommendArticle");
				if (XString.isNotEmpty(recommendArticle))
				...
				typeStr = typeStr
						+ " order by hitcount desc,topflag desc,orderflag desc";
				...
				typeStr = typeStr
						+ " order by topflag desc, publishdate desc,orderflag desc, id desc";
			} else {
				typeStr = typeStr
						+ " order by topflag desc,orderflag desc,publishdate desc, id desc";
			}

			if ((XString.isNotEmpty(hasAttribute))
					&& (!"null".equals(hasAttribute))) {
				String[] attribute = hasAttribute.split(",");
				if (attribute.length > 0) {
					String attributeSql = "";
					for (int i = 0; i < attribute.length; i++) {
						String attr = attribute[i].trim();
						if (XString.isNotEmpty(attr)) {
							attributeSql = attributeSql
									+ " and attribute like '%" + attribute[i]
									+ "%'";
						}
					}
		...
		if ((XString.isNotEmpty(catalogName))
				&& (!"null".equalsIgnoreCase(catalogName))) {
			if (catalogName.indexOf(",") != -1)
				catalogID = CatalogUtil.getIDsByName(this.site.getString("ID"),
						catalogName);
			else if (catalogName.indexOf("/") != -1)
				catalogID = CatalogUtil.getIDByNames(this.site.getString("ID"),
						catalogName);
			else {
				catalogID = CatalogUtil.getIDByName(this.site.getString("ID"),
						catalogName);
			}

			if ((XString.isEmpty(catalogID))
					&& (XString.isDigit(catalogName)))
		...
		if ((XString.isNotEmpty(catalogID))
			...
			for (int m = 0; m < catalogIDs.length; m++) {
				parentInnerCode = parentInnerCode
						+ CatalogUtil.getInnerCode(catalogIDs[m]) + ",";
			}

			if (XString.isNotEmpty(parentInnerCode))
				parentInnerCode = parentInnerCode.substring(0, parentInnerCode
						.length() - 1);
		} else {
		...
		} else if ((XString.isNotEmpty(catalogID))
				&& (catalogID.indexOf(",") != -1)) {
			String[] innercodes = parentInnerCode.split(",");
			for (int m = 0; m < innercodes.length; m++)
				if (m == 0) {
		...
		if ((XString.isEmpty(condition)) || ("null".equals(condition))) {
			condition = "1=1";
		}
		String sql = "";
		...
		sql = sql + item + " where " + condition + " and " + catalogStr
				+ statusStr + typeStr;
		qb.setSQL(sql);
	...
	public DataTable getMagazineList(String item, String type, String name,
			String count, String condition) {
		DataTable dt = null;

		String key = item + "_" + type + "_" + name + "_" + count + "_"
				+ condition;
		dt = TemplateCache.getDataTable(key);
		if (dt != null) {
			return dt;
	...
	public DataTable getMagazineIssueList(String item, String name,
			String type, String count, String condition) {
		DataTable dt = null;

		String key = item + "_" + name + "_" + type + "_" + count + "_"
				+ condition;
		dt = TemplateCache.getDataTable(key);
		if (dt != null) {
			return dt;
		...
		if ("magazineissue".equalsIgnoreCase(item)) {
			...
			if (magazineDT.getRowCount() > 0) {
				...
				if ((this.catalog != null) && (!type.equalsIgnoreCase("all"))) {
					...
					int catalogType = (int) CatalogUtil
							.getCatalogType(catalogID);
					if ((catalogType == 3) && (catalogName.indexOf("年第") != -1)) {
						String currentYear = catalogName.substring(0,
								catalogName.indexOf("年第"));
						String currentPeriodNum = catalogName.substring(
								catalogName.indexOf("年第") + 2, catalogName
										.indexOf("期"));
				...
				if (dt.getRowCount() > 0) {
					for (int i = 0; i < dt.getRowCount() - 1; i++) {
						String prevCatalogID = CatalogUtil.getIDByNames(
								this.site.getString("ID"), name + "/"
										+ dt.getString(i, "Year") + "年第"
										+ dt.getString(i, "PeriodNum") + "期");
						dt.set(i, "PrevLink", CatalogUtil.getLink(
								prevCatalogID, levelStr));
					}

					for (int i = 1; i < dt.getRowCount(); i++) {
						String nextCatalogID = CatalogUtil.getIDByNames(
								this.site.getString("ID"), name + "/"
										+ dt.getString(i - 1, "Year") + "年第"
										+ dt.getString(i - 1, "PeriodNum")
										+ "期");
						dt.set(0, "NextLink", CatalogUtil.getLink(
								nextCatalogID, levelStr));
					}
					...
					DataTable magazineIssueDT = new QueryBuilder(
							...
							magazineDT.getString(0, "ID"), dt
									.getString(0, "ID")).executeDataTable();
					if (magazineIssueDT.getRowCount() > 0) {
						String nextCatalogID = CatalogUtil.getIDByNames(
								this.site.getString("ID"), name
										+ "/"
										+ magazineIssueDT.getString(0, "Year")
										+ "年第"
										+ magazineIssueDT.getString(0,
												"PeriodNum") + "期");
						dt.set(0, "NextLink", CatalogUtil.getLink(
								nextCatalogID, levelStr));
					} else {
					...
					if (magazineIssueDT.getRowCount() > 0) {
						String nextCatalogID = CatalogUtil.getIDByNames(
								this.site.getString("ID"), name
										...
										+ magazineIssueDT.getString(0,
												"PeriodNum") + "期");
						dt.set(dt.getRowCount() - 1, "PrevLink", CatalogUtil
								.getLink(nextCatalogID, levelStr));
					} else {
						dt.set(dt.getRowCount() - 1, "PrevLink", "#");
					}

					for (int m = 0; m < dt.getRowCount(); m++) {
						String ID = CatalogUtil.getIDByNames(this.site
								.getString("ID"), name + "/"
								+ dt.getString(m, "Year") + "年第"
								+ dt.getString(m, "PeriodNum") + "期");
						ColumnUtil.extendCatalogColumnData(dt.getDataRow(m),
								SiteUtil.getURL(this.site.getString("ID")));
						if (XString.isDigit(ID))
							dt
									.set(m, "Link", CatalogUtil.getLink(ID,
											levelStr));
						else
							dt.set(m, "Link", "#");
					}
	...
	public DataTable getFriendLinkList(String item, String group, String count,
			String condition) {
		DataTable dt = new DataTable();
		if ((count == null) || ("".equalsIgnoreCase(count))) {
			count = "20";
		}
		String sql = "select * from zclink where siteID = "
				+ this.site.getString("ID") + " and LinkGroupID = "
				+ "(select id from zclinkgroup where siteID = "
				+ this.site.getString("ID")
	...
	public DataTable getBlockList(String item, String count, String condition) {
		...
		if ("1".equalsIgnoreCase(blockType)) {
			if ((catalogID != null) && (!"".equalsIgnoreCase(catalogID))) {
				sql = "select * from zcarticle where catalogid = ? order by "
						+ sortField + " desc";
				qb.add(catalogID);
			} else {
	...
	public String getTree(String id, String method, String tagBody,
			String style, String levelStr) {
	...
	public String getAD(String name, String type) {
		...
		if ((XString.isEmpty(name)) || (name == null)) {
			sb.append("广告的代码为空，请修改模板");
		} else {
			DataTable positionDT = new QueryBuilder(
					"select * from zcadposition where PositionName = ? and siteid=?",
			...
			if (positionDT.getRowCount() <= 0) {
				...
				DataTable adDt = new QueryBuilder(
						"select * from ZCAdvertisement where PositionID = ? and IsOpen = 'Y'",
						positionDT.getString(0, "ID")).executeDataTable();
				sb.append(adDt.getString(0, "AdContent"));
			} else if ((type != null) && (XString.isNotEmpty(type))
				...
				sb.append("<script language='javascript' src='" + getLevelStr()
						+ jsName + "'></script>");

				if (!new File(new StringBuffer(String.valueOf(Config
						.getContextRealPath())).append(
						Config.getValue("Statical.TargetDir")).append("/")
						.append(
								SiteUtil.getAlias(positionDT.getString(0,
										"SiteID"))).append("/").toString()
						.replaceAll("/+", "/")
	...
	public String getForm(String code) {
		...
		if (XString.isEmpty(code)) {
			...
			ZCCustomTableSet set = new ZCCustomTableSchema()
					.query(new QueryBuilder("where Code=?", code));
	...
	public String getVote(String name, String id) {
		...
		if ((XString.isNotEmpty(id)) && (XString.isDigit(id)))
			qb = new QueryBuilder(" where id=? and siteid=?", id, this.site
					.getString("ID"));
		else if ((XString.isNotEmpty(name)) && (!"null".equals(name)))
			qb = new QueryBuilder(" where title=? and siteid=?", name,
					this.site.getString("ID"));
		else {
		...
		sb.append("<script language='javascript' src='"
				+ new StringBuffer(String.valueOf(getLevelStr())).append(
	...
	public DataTable getVoteSubjects(String voteID, int count) {
		QueryBuilder qb = new QueryBuilder(
				"select * from zcvoteSubject where VoteID = ? order by ID",
				voteID);
	...
	public DataTable getVoteItems(String subjectID, String inputType, int count) {
		QueryBuilder qb = new QueryBuilder(
				"select * from zcvoteitem where subjectID = ? order by ID",
				subjectID);
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			...
			if ("0".equals(dt.getString(i, "ItemType")))
				html = "<label><input name='Subject_" + subjectID + "' type='"
						+ inputType + "' value='" + dt.getString(i, "id")
						+ "' />" + dt.getString(i, "item") + "</label>\n";
			else {
				html = "<label><input name='Subject_"
						+ subjectID
						+ "' type='"
						+ inputType
						+ "' value='"
						...
						+ subjectID + "_Item_" + dt.getString(i, "id")
						+ "');\\\"/>\n";
			}
			dt.set(i, "html", html);
		}
	...
	public String getComment(String count) {
		return "<script src=\"" + Config.getValue("ServicesContext")
				+ Config.getValue("CommentListJS") + "?RelaID="
				+ this.doc.getString("ID") + "&CatalogID="
				+ this.doc.getString("CatalogID") + "&CatalogType="
				+ this.catalog.getString("Type") + "&SiteID="
				+ this.site.getString("id") + "&Count=" + count
				+ "\"></script>";
	}

	public String getImagePlayer(String name, String width, String height,
			String count, String charwidth) {
		if ((XString.isEmpty(charwidth))
				|| ("null".equalsIgnoreCase(charwidth))) {
			charwidth = "100";
		}
		StringBuffer sb = new StringBuffer();
		...
		if ("1".equals(imagePlayer.getImageSource())) {
			...
			for (int i = 0; i < dt.getRowCount(); i++) {
				...
				if (XString.isNotEmpty(dt.getString(i, "ShortTitle")))
					texts.append(XString.subStringEx(dt.getString(i,
							"ShortTitle"), Integer.parseInt(charwidth)));
				else
					texts.append(XString
			...
			for (int i = 0; i < dt.getRowCount(); i++) {
				...
				pics.append(getLevelStr() + dt.getString(i, "path") + "1_"
						+ dt.getString(i, "FileName"));
				if (XString.isNotEmpty(getLevelStr()
						+ dt.getString(i, "LinkURL"))) {
					links.append(dt.getString(i, "LinkURL"));
				}
				if (XString.isNotEmpty(dt.getString(i, "LinkText"))) {
					texts.append(XString.subStringEx(dt.getString(i,
		...
		sb.append("<param name=\"movie\" value=\"" + getLevelStr()
				+ "images/ImagePlayer.swf\">");
		sb.append("<param name=\"quality\" value=\"high\">");
		sb.append("<param name=\"wmode\" value=\"transparent\">");
		sb.append("<param name=\"FlashVars\" value=\"pics=" + pics.toString()
				+ "&links=" + links.toString() + "&texts=" + texts.toString()
				+ "&borderwidth=" + pWidth + "&borderheight=" + imgHeight
				+ showText + "&overtxtcolor=FFFF00&txtcolor=FFFF00\"/>");
		sb
				.append("<embed src=\""
						+ getLevelStr()
						+ "images/ImagePlayer.swf\" type=\"application/x-shockwave-flash\" wmode=\"transparent\" "
						+ "FlashVars=\"wmode=transparent&pics="
						+ pics.toString() + "&links=" + links.toString()
						+ "&texts=" + texts.toString() + "&borderwidth="
	...
	public String getLinkURL(String type, String code, String name,
			String spliter) {
		StringBuffer sb = new StringBuffer();
		if ("catalog".equalsIgnoreCase(type)) {
			...
			if ((name != null) && (!"".equals(name)) && (!"null".equals(name))) {
				...
				if (XString.isDigit(name)) {
					...
					if (name.indexOf("/") != -1)
						catlaogID = CatalogUtil.getIDByNames(this.site
								.getString("ID"), name);
					else {
						catlaogID = CatalogUtil.getIDByName(this.site
								.getString("ID"), name);
					}
					if (XString.isNotEmpty(catlaogID)) {
			...
			sb.append(PubFun.getCurrentPage(catalogID, this.level, name,
					spliter, "_self"));
		} else if ("HomeURL".equalsIgnoreCase(type)) {
			if ((XString.isEmpty(name)) || ("null".equalsIgnoreCase(name))) {
				name = "首页";
			}
			...
			String BoardID = "";
			if (XString.isNotEmpty(name)) {
				BoardID = new QueryBuilder(
						"select ID from ZCMessageBoard where Name = ? and SiteID = ?",
						name, this.site.getString("ID")).executeString();
			}
			sb.append(Config.getValue("ServicesContext") + "/"
					+ "MessageBoardPage.jsp?BoardID=" + BoardID);
		}
		return sb.toString();
	}
	...
	public DataRow getCatalog(String type, String name) {
		...
		if ("catalog".equalsIgnoreCase(type)) {
			...
			if ((name != null) && (!"".equals(name)) && (!"null".equals(name))) {
				...
				String id;
				if (name.indexOf("/") != -1)
					id = CatalogUtil.getIDByNames(this.site.getString("ID"),
							name);
				else {
					id = CatalogUtil.getIDByName(this.site.getString("ID"),
							name);
				}

				if ((XString.isEmpty(id)) && (XString.isDigit(name))) {
					id = name;
				}
				...
				docRow.insertColumn("link", CatalogUtil.getLink(
						catalog.getID(), levelString));
			}

			ColumnUtil.extendCatalogColumnData(docRow, this.site.getLong("ID"),
					getLevelStr());
		}
		return docRow;
	}
		...
		if ("magazine".equalsIgnoreCase(type)) {
			...
			if (magazineDT.getRowCount() > 0) {
				DataTable dt = new QueryBuilder(
						"select * from ZCMagazineIssue where MagazineID=? and Year='"
								+ magazineDT.getString(0, "CurrentYear")
								+ "' and PeriodNum='"
								+ magazineDT.getString(0, "CurrentPeriodNum")
								+ "'", magazineDT.getString(0, "ID"))
						.executePagedDataTable(1, 0);
				if ((dt != null) && (dt.getRowCount() > 0)) {
					docRow = dt.get(0);
					String catalogName = docRow.getString("Year") + "年第"
							+ docRow.getString("PeriodNum") + "期";
					String parentID = new QueryBuilder(
							"select ID from zccatalog where type='3' and name=? and siteID=? and parentID=0",
							name, this.site.getString("ID")).executeString();
					DataTable catalogDT = new QueryBuilder(
							"select * from zccatalog where type='3' and name='"
									+ catalogName
									+ "' and siteID=? and parentID=?",
							this.site.getString("ID"), parentID)
							.executePagedDataTable(1, 0);
					if ((catalogDT != null) && (catalogDT.getRowCount() > 0)) {
						docRow.insertColumn("CatalogInnerCode", catalogDT
								.getString(0, "InnerCode"));
						docRow.insertColumn("CatalogID", catalogDT.getString(0,
								"ID"));
						docRow.insertColumn("CatalogName", catalogDT.getString(
								0, "Name"));
					}
				}
			}
	...
	public String getPage(String type, String value, String name, String target) {
		...
		if ("currentpage".equalsIgnoreCase(type)) {
			...
			Object obj = this.catalog.get("ID");
			long catalogID = obj == null ? 0L : ((Long) obj).longValue();
			sb.append(PubFun.getCurrentPage(catalogID, this.level, name, value,
					target));
	...
	public DataTable getPagedDataTable(DataTable dt, int type) {
		...
		for (int i = 0; i < count; i++) {
			values[i] = dt.getDataRow(this.PageIndex * this.PageSize + i)
					.getDataValues();
		}
	...
	public String getLevelStr() {
		if (XString.isEmpty(this.levelStr)) {
	...
	public static DataTable getSiteMap(String siteID) {
		if (!XString.checkID(siteID)) {
			return null;
		}

		DataTable dt = new QueryBuilder(
				"select treelevel,innercode,name,id,orderflag from ZCCatalog where siteID=? and type=1  order by orderflag,innercode",
				siteID).executeDataTable();

		dt.insertColumn("Link");
		for (int i = 0; i < dt.getRowCount(); i++) {
			String url = CatalogUtil.getLink(dt.getLong(i, "ID"), PubFun
					.getLevelStr(CatalogUtil.getLevel(dt.getLong(i, "ID"))));
	...
	public DataTable getCustomData(String tableName, String CountStr,
			String condition) {
		int count = 20;
		if (XString.isDigit(CountStr)) {
			count = Integer.parseInt(CountStr);
		}
		String wherePart = "where 1=1 ";
		if ((XString.isNotEmpty(condition)) && (!condition.equals("null"))) {
			wherePart = wherePart + condition;
		}
		return CustomTableUtil.getData(tableName, new QueryBuilder(wherePart),
				count, 0);
	}
	...
	public DataTable getKeywords(String type, String CountStr) {
		...
		return SourceReport.getKeywordData(this.site.getLong("id"), startDate,
				today, count);
	}
	...
	public DataTable getArticlePages() {
		...
		String[] contents = this.doc.getString("FullContent").split(
				"<!--_SKY_PAGE_BREAK_-->", -1);
	...
	public void dealTag(DataTable dtArticle) {
		...
		searchUrl = XString.replaceEx(searchUrl, "Result.jsp", "Tag.jsp");
		for (int j = 0; j < dtArticle.getRowCount(); j++) {
			String tag = dtArticle.getString(j, "Tag");
			if (XString.isNotEmpty(tag)) {
				String[] tags = tag.split("\\s");
				tag = tags[0];
				ZCTagSchema tagSchema = SiteUtil.getTag(dtArticle.getLong(j,
						"SiteID"), tag);
				if (tagSchema == null) {
					continue;
				}
				String tagUrl = tagSchema.getLinkURL();
				if (XString.isEmpty(tagUrl)) {
					tagUrl = searchUrl + "?site="
							+ dtArticle.getLong(j, "SiteID") + "&query=%00"
							+ XString.urlEncode(tag, "UTF-8");
				}
	...
	public int getTotal(String type, long catlaogID) {
		int count = new QueryBuilder("select count(*) from zc" + type
				+ " where catalogid=" + catlaogID + " and status=30")
				.executeInt();
		return count;
	}
...
/*
 * com.xdarkness.cms.template.CmsTemplateData JD-Core Version: 0.6.0
 */