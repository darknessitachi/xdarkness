...
import com.xdarkness.schema.ZDIPRangeSchema;
import com.xdarkness.schema.ZDIPRangeSet;
import com.xdarkness.schema.ZDIPSchema;
import com.xdarkness.schema.ZDIPSet;
...
public class ImportIPDB {
	...
	public static void exportIPRangeAsSQL() {
		...
		for (int i = 0; i < set.size(); i++) {
			...
			sb.append("insert into ZDIPRange (DistrictCode,IPRanges) values ('"
					+ r.getDistrictCode() + "','" + r.getIPRanges() + "');\n");
		}
		FileUtil.writeText("F:/IPRanges.sql", sb.toString(), "UTF-8");
	}

	public static void importTXT() throws SQLException {
		String[] pros = { "新疆", "青海", "宁夏", "内蒙", "黑龙江", "吉林", "辽宁", "河北",
				"天津", "北京", "山东", "安徽", "江苏", "浙江", "上海", "福建", "广东", "广西",
				"香港", "台湾", "澳门", "云南", "贵州", "西藏", "四川", "陕西", "山西", "河南",
				"湖北", "湖南", "重庆", "江西", "海南", "甘肃" };
		String fileName = "F:/qqwry/ip.txt";
		new QueryBuilder("truncate table ZDIP").executeNoQuery();
		DataTable dt = DataTableUtil.txtToDataTable(fileName, new String[] {
				"IP3", "IP4", "Address", "Memo" }, "\\s+");
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			...
			long ip1 = Long.parseLong(arr1[0]) * 16777216L
					+ Long.parseLong(arr1[1]) * 65536L
					+ Long.parseLong(arr1[2]) * 256L + Long.parseLong(arr1[3])
					- 1L;
			long ip2 = Long.parseLong(arr2[0]) * 16777216L
					+ Long.parseLong(arr2[1]) * 65536L
					+ Long.parseLong(arr2[2]) * 256L + Long.parseLong(arr2[3])
					- 1L;
			if (ip1 < 0L) {
				ip1 = 0L;
			}
	...
	public static void fixAddress() throws SQLException {
		new QueryBuilder("update ZDIP set Address='' where Address='CZ88.NET'")
				.executeNoQuery();
		new QueryBuilder("update ZDIP set Memo='' where Memo='CZ88.NET'")
				.executeNoQuery();
		new QueryBuilder(
				"update ZDIP set Address='内蒙古乌兰察布市' where Address='内蒙古乌兰察布盟'")
				.executeNoQuery();
		new QueryBuilder(
				"update ZDIP set Address='新疆伊犁哈萨克自治州' where Address='新疆伊犁州'")
				.executeNoQuery();
		new QueryBuilder(
				"update ZDIP set Address='新疆昌吉回族自治州' where Address='新疆昌吉州'")
				.executeNoQuery();
		new QueryBuilder(
				"update ZDIP set Address='新疆博尔塔拉蒙古自治州' where Address='新疆博尔塔拉州'")
				.executeNoQuery();
		new QueryBuilder(
				"update ZDIP set Address='内蒙古呼伦贝尔市' where Address='内蒙古呼伦贝尔盟'")
				.executeNoQuery();
	...
	public static void fixDistrictCode() throws SQLException {
		DataTable dt = new QueryBuilder("select Address,IP1,IP2 from ZDIP")
				.executeDataTable();
		...
		QueryBuilder qb = new QueryBuilder(
				"update ZDIP set DistrictCode=? where ip1=? and ip2=?");
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			...
			for (int j = 0; j < names.length; j++) {
				...
				if ((XString.isNotEmpty(parentName))
						&& (!code.startsWith("00"))) {
					if ((address.indexOf(name) >= 0)
							&& (address.indexOf(parentName) >= 0)) {
						qb.add(code);
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			...
			for (int j = 0; j < names.length; j++) {
				...
				String parentName = map2.getString(parentCode);
				if ((XString.isEmpty(parentName)) || (code.startsWith("00"))) {
					if (address.indexOf(name) >= 0) {
						qb.add(code);
						qb.add(dt.getString(i, "IP1"));
						qb.add(dt.getString(i, "IP2"));
					...
					if ((name.length() <= 2)
							|| (address.indexOf(name.substring(0, 2)) < 0))
						continue;
					qb.add(code);
					qb.add(dt.getString(i, "IP1"));
					qb.add(dt.getString(i, "IP2"));
	...
	public static void fixContry() {
		XConnection conn = XConnectionPoolManager.getConnection("DB_TY");
		DataAccess da = new DataAccess(conn);
		try {
			new QueryBuilder("delete from ZDDistrict where Code like '0%'")
					.executeNoQuery();
			DataTable dt = new QueryBuilder(
			"select * from T_DM_GJDQ order by dmsx").executeDataTable();
			for (int i = 1; i <= dt.getRowCount(); i++) {
				ZDDistrictSchema d = new ZDDistrictSchema();
				d.setCode(XString.leftPad(i+"", '0', 6));
	...
	public static void checkIP() {
		DataTable dt = new QueryBuilder(
				"select IP1,IP2,DistrictCode from ZDIP order by ip1")
				.executeDataTable();
		long last = 0L;
		for (int i = 0; i < dt.getRowCount(); i++) {
			if ((last != 0L)
	...
	public static void importIPRange() throws SQLException {
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			...
			if (XString.isEmpty(DistrictCode)) {
				continue;
			}
	...
	public static void verify() {
		DataTable dt = new QueryBuilder("select IP3,IP4,DistrictCode from ZDIP")
				.executeDataTable();
		for (int i = 0; i < dt.getRowCount(); i++) {
			...
			if ((XString.isNotEmpty(code)) && (!code.equals(str))) {
