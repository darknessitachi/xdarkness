...
import com.xdarkness.search.ZvingSimilarity;
...
import com.xdarkness.framework.util.LogUtil;
 ...
 public abstract class Indexer
 {
   ...
   public void setPath(String path) {
     ...
     try {
       if (!new File(path).exists()) {
         ...
         if (XString.isEmpty(lastDateStr))
           this.nextLastDate = (this.lastDate = new Date());
         else {
   ...
   {
     synchronized (mutex) {
       if (XString.isEmpty(this.indexPath)) {
         throw new RuntimeException("没有指定索引存放的目录!"); } LogUtil.getLogger().info("开始建立索引" + this.indexPath + "......");
       long startTime = System.currentTimeMillis();
       String lastDateStr;
       Date date;
       try { try { if (new File(this.indexPath + "/write.lock").exists())
         ...
         try
         {
           if (this.writer != null) {
             if (new File(this.indexPath + "/optimize.lock").exists()) {
                lastDateStr = FileUtil.readText(this.indexPath + "/optimize.lock").trim();
               if (XString.isNotEmpty(lastDateStr)) {
                  date = DateUtil.parseDateTime(lastDateStr);
...
/*    com.xdarkness.search.index.Indexer
