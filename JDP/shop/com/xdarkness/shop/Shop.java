...
public class Shop extends Page {
	...
	public static Mapx initDialog(Mapx params) {
		...
		if (o1 != null) {
			...
			if (catalog.fill()) {
				...
				map.put("radioPublishFlag", HtmlUtil.dataTableToRadios(
						"PublishFlag", dt, catalog.getPublishFlag()));
				map.put("optionCatalogType", HtmlUtil.mapxToOptions(
						Catalog.CatalogTypeMap, catalog.getType()));
				return map;
			}
			return null;
		}
		params.put("SiteID", ApplicationPage.getCurrentSiteID());
		params.put("radioPublishFlag", HtmlUtil.dataTableToRadios(
				"PublishFlag", dt, "Y"));
		params.put("optionCatalogType", HtmlUtil
				.mapxToOptions(Catalog.CatalogTypeMap));
	...
	private static DataTable getTreeDataTable(String fileName, long parentID,
			int TreeLevel) {
		...
		for (int i = 0; i < catalogs.length; i++) {
			...
			lastID = i + 1;
			Object[] catalogRow = { new Long(lastID), new Long(parentID),
					new Integer(TreeLevel), catalog.trim() };
	...
	public void importCatalog() {
		...
		for (int i = 0; i < catalogs.length; i++) {
			...
			catalogName = XString.toDBC(catalogName);
			catalogName = XString.rightTrim(catalogName);
			...
			if (parentID != 0L) {
				...
				if (isNewData)
					trans.add(pCatalog, OperateType.INSERT);
				else
					trans.add(pCatalog, OperateType.UPDATE);
			} else {
				ZCSiteSchema site = new ZCSiteSchema();
	...
	public Mapx moveChild(ZCCatalogSchema catalog, long TreeLevel,
		...
		map.put("update zccatalog set TreeLevel=" + TreeLevel + ",innercode='"
				...
				+ "',modifytime='" + DateUtil.getCurrentDate() + "' where id="
				+ catalogID, new Integer(7));
	...
	public static void deleteCatalog(Transaction trans, long ID) {
		...
		trans.add(catalog.query(new QueryBuilder(" where InnerCode like ?",
				catalog.getInnerCode() + "%")), OperateType.DELETE_AND_BACKUP);
...
/*
 * com.xdarkness.shop.Shop JD-Core Version: 0.6.0
 */