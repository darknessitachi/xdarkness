...
public class TemplateData {
	...
	public String getPreviousPage() {
		...
		if (this.PageIndex != 0)
			return this.OtherFileName.replaceAll("@INDEX", String
					.valueOf(this.PageIndex));
		if (this.PageIndex == 0) {
			return "#";
		}
	...
	public String getPageBar(int id) {
		...
		sb.append("共" + this.Total + "条记录，每页" + this.PageSize
				+ "条，当前第<span class='fc_ch1'>" + (this.PageIndex + 1)
				+ "</span>/<span class='fc_ch1'>" + this.PageCount
				+ "</span>页&nbsp;&nbsp;&nbsp;&nbsp;");
		if (this.PageIndex > 0) {
			sb.append("<a href='" + getFirstPage()
		...
		String listNameRule = getFirstFileName().substring(0,
				getFirstFileName().lastIndexOf("."));
		...
		sb
				.append("&nbsp;&nbsp;<input type='button' onclick=\"if(/[^\\d]/.test(document.getElementById('_PageBar_Index_"
						...
						+ id
						+ "').value:1;if(PageIndex==1){window.location='"
						+ getFirstFileName()
		...
		if (getTotal() > 1) {
			...
			for (int i = 0; i < this.PageCount; i++) {
				...
				if (i == 0)
					...
					href = this.OtherFileName.replaceAll("@INDEX", String
							.valueOf(i + 1));
				}
	...
	public DataRow getPageRow() {
		...
		String[] cols = { "Total", "PageCount", "PageSize", "FirstPage",
				"PrevPage", "NextPage", "LastPage", "PageIndex", "PageList" };
		String[] values = { this.Total+"", this.PageCount+"", this.PageSize+"",
				getFirstPage(), getPreviousPage(), getNextPage(),
...
/*
 * com.xdarkness.statical.TemplateData JD-Core Version: 0.6.0
 */