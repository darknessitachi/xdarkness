...
public class Context {
	...
	public Context(Transaction tran, ZWStepSchema step)
			throws WorkflowException {
		this.tran = tran;
		this.flow = WorkflowUtil.findWorkflow(step.getWorkflowID());
		this.instance = WorkflowUtil.findInstance(step.getInstanceID());
		this.step = step;
		this.variables = WorkflowUtil.getDataVariables(this.instance
				.getDataID(), step.getDataVersionID());
	}
	...
	public Context(Transaction tran, WorkflowInstance instance,
			ZWStepSchema step) {
		this.tran = tran;
		this.flow = WorkflowUtil.findWorkflow(step.getWorkflowID());
		...
		this.step = step;
		if (step.getID() != 1L)
			this.variables = WorkflowUtil.getDataVariables(
					instance.getDataID(), step.getDataVersionID());
	}
	...
	public void save() {
		this.tran.add(this.step, OperateType.UPDATE);
	...
	public String getWorkflowName() {
		return WorkflowUtil.findWorkflow(this.instance.getWorkflowID())
				.getName();
	}

	public String getStepName() {
		return WorkflowUtil.getStepName(this.flow.getID(), this.step
				.getNodeID());
	}
	...
	public void gotoStep(String stepName) throws Exception {
		...
		for (int i = 0; i < nodes.length; i++) {
			...
			if ((!node.getType().equals("ActionNode"))
					&& (node.getName().equalsIgnoreCase(stepName))) {
				...
				WorkflowAction.executeMethod(this, this.flow.findNode(newStep
						.getNodeID()), "Pre");
				context.save();
				...
				if ("EndNode".equalsIgnoreCase(node.getType())) {
					...
					this.tran.add(context.getInstance(), OperateType.UPDATE);
				}
				this.tran.add(newStep, OperateType.INSERT);
			}
		}
	}
}
...
/*
 * com.xdarkness.workflow.Context JD-Core Version: 0.6.0
 */