...
import com.xdarkness.workflow.methods.MethodScript;
 ...
 public class WorkflowAction
 {
       ...
       if (checkConditions(wt)) {
         ...
         if ("1".equals(wt.getTargetNode().getData().get("SignJointly")))
         {
           ...
           for (int j = 0; j < users.length; j++)
             if (j == 0) {
               ...
               this.context.getTransaction().add(step, OperateType.INSERT);
             }
         }
         else if (XString.isNotEmpty(selectedUser)) {
           if (!XString.checkID(selectedUser)) {
         ...
         if (!this.context.getInstance().getState().equals("Completed"))
         {
           if ("EndNode".equalsIgnoreCase(wt.getTargetNode().getType()))
           {
             ...
             this.context.getTransaction().add(this.context.getInstance(), OperateType.UPDATE);
           }
           this.context.getTransaction().add(newStep, OperateType.INSERT);
         }
         this.context.save();
       }
     }
     ...
     throws Exception
   {
     ...
     for (int i = 0; i < keyArray.length; i++) {
       ...
       if (("Script".equalsIgnoreCase(key)) && (XString.isNotEmpty(value))) {
         ConditionScript cs = new ConditionScript();
         cs.setScript(value);
         flag = cs.validate(this.context);
       } else if (("Method".equalsIgnoreCase(key)) && (XString.isNotEmpty(value))) {
...
/*    com.xdarkness.workflow.WorkflowAction
