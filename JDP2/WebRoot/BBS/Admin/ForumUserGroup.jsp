...
function add(){
	...
	function addSave(){
		...
		Server.sendRequest("com.xdarkness.bbs.admin.ForumUserGroup.add",dc,function(response){
	...
	function del(ID){
		Dialog.confirm("删除后不可恢复，确认要删除？",function(){
			...
			dc.add("ID",ID);
			Server.sendRequest("com.xdarkness.bbs.admin.ForumUserGroup.del",dc,function(response){
				Dialog.alert(response.Message);
				if(response.Status==1){
					DataList.loadData('list1');
				}
...
<div id="foruminfo">
  <div id="nav">
    ...
    <sky:button onClick="save()"> <img src="../../Icons/icon022a16.gif" width="20" height="20" />保存</sky:button>
  </div>
</div>
<div class="forumbox">
  <form id="form1" method="post" name="moderate">
    <table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolor="#eeeeee" class="forumTable" style="padding-top:0px;padding-left:6px;padding-right:6px;padding-bottom:8px;">
      ...
      <sky:datalist id="list1" method="com.xdarkness.bbs.admin.ForumUserGroup.getList" page="false">
        <tr nowrap="nowrap">
