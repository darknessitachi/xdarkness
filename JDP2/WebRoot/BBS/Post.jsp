...
Page.onLoad(function(){
	...
	Server.sendRequest("com.xdarkness.bbs.Post.addViewCount",dc);
...
function isOperaterMember(ID, type) {
	...
	Server.sendRequest("com.xdarkness.bbs.Post.isOperaterMember",dc,function(response){
...
function save(){
	 ...
	 Server.sendRequest("com.xdarkness.bbs.Post.add",dc,function(response){
			if(response.Status==1){
...
function del(ID){
	...
	Dialog.confirm("确定删除选中的帖子吗？",function(){
		...
		dc.add("ID",ID);
		dc.add("SiteID",$V("SiteID"));
   		Server.sendRequest("com.xdarkness.bbs.Post.del",dc,function(response){
			if(response.Status==1){
				DataList.loadData("list1");
...
function edit(ID){
	...
	if(flag){
		...
		dc.add("ID",ID);
		dc.add("SiteID",$V("SiteID"));
   		Server.sendRequest("com.xdarkness.bbs.Post.edit",dc,function(response){
   			if(response.Status==1){
...
function editSave(ID){
	...
	Server.sendRequest("com.xdarkness.bbs.Post.editSave",dc,function(response){
...
function hide(ID){
		var dc = new DataCollection();
		dc.add("ID",ID);
   		Server.sendRequest("com.xdarkness.bbs.Post.hide",dc,function(response){
			if(response.Status==1){
				DataList.loadData("list1");
			}
		});
...
<div class="wrap">
<sky:init method="com.xdarkness.bbs.Post.init">
...
<sky:datalist id="list1" method="com.xdarkness.bbs.Post.getList" page="true" size="12">
	<form method="post">
