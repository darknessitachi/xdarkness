...
function addSave(){
	...
	Server.sendRequest("com.xdarkness.cms.dataservice.Column.add",dc,function(response){
...
function editSave(){
	...
	dc.add("FormID",$V("FormID"));
	Server.sendRequest("com.xdarkness.cms.dataservice.Column.edit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData("dg1");
...
function del(){
	...
	Dialog.alert("确定要删除该字段吗？",function(){
		...
		dc.add("IDs",arr.join());
		Server.sendRequest("com.xdarkness.cms.dataservice.Column.del",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					DataGrid.loadData("dg1");
				}
...
function afterRowDragEnd(rowIndex,oldIndex){
	...
	Server.sendRequest("com.xdarkness.cms.dataservice.Column.sortColumn",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				DataGrid.loadData("dg1");
			}
...
function createTb(){
	 var dc = new DataCollection();
	 dc.add("FormID",$V("FormID"))
		Server.sendRequest("com.xdarkness.cms.dataservice.FormTable.create",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					$D.close();
				}
...
<table width="100%" border="0" cellspacing="0" cellpadding="6" class="blockTable">
	<tr>
		...
		<sky:button onClick="add()"><img src="../Icons/icon024a2.gif" />新建</sky:button>
		<sky:button onClick="edit()"><img src="../Icons/icon024a4.gif" />编辑</sky:button>
		<sky:button onClick="del()"><img src="../Icons/icon024a3.gif" />删除</sky:button>
		<sky:button onClick="preview()"><img src="../Icons/icon024a15.gif"/>预览</sky:button>
		<sky:button onClick="createTb()"><img src="../Icons/icon024a2.gif" />创建物理表</sky:button>
	</tr>
	<tr>   <td style="padding:0px 5px;">
<sky:datagrid id="dg1"
			method="com.xdarkness.cms.dataservice.Column.dg1DataBind">
			<table width="100%" cellpadding="2" cellspacing="0"
				...
				<tr xtype="head" class="dataTableHead">
					...
					<td width="4%" xtype="Checkbox" field ="IsMandatory" checkedvalue="Y"><b>必填</b></td>
