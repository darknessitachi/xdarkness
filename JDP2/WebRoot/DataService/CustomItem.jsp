...
function Save(){
	...
	Server.sendRequest("com.xdarkness.platform.CustomColumnItem.save",dc,function(response){
...
function del(){
	...
	if(!window.confirm("确定要删除该字段吗？")){
		...
		dc.add("IDs",arr.join());
		Server.sendRequest("com.xdarkness.platform.CustomColumnItem.del",dc,function(response){
			if(response.Status==1){
				alert("删除成功");
...
function CopyToSave(){
	var dc = $DW.Form.getData("form2");
	dc.add("IDs",$DW.$V("IDs"));
	Server.sendRequest("com.xdarkness.platform.CustomColumnItem.CopyToSave",dc,function(response){
		if(response.Status==1){
			alert(response.Message);
			$D.close();
		}
...
<body scroll="no">
<sky:init method="com.xdarkness.platform.CustomColumnItem.init">
<div style="padding:2px;">
	<table width="100%" cellpadding="4">
		<tr>
			<td>
				<sky:button onClick="add();"><img src="../Platform/Images/tab1_tri.gif"/>新建</sky:button>
				<sky:button onClick="del();"><img src="../Platform/Images/tab1_tri.gif"/>删除</sky:button>
				<sky:button onClick="CopyTo();"><img src="../Platform/Images/tab1_tri.gif"/>复制到其他栏目</sky:button>
			</td>
		</tr>
	</table>
<div id="maincontent">
<sky:datagrid id="dg1" method="com.xdarkness.platform.CustomColumnItem.dg1DataBind" size="20" editAction="showEditDialog">
  <table width="100%" cellpadding="2" cellspacing="0" class="dataTable">
    <tr xtype="head" class="dataTableHead">
      <td  width="5">&nbsp;</td>
      <td width="20" xtype="selector" field="id">&nbsp;</td>
