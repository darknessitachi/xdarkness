...
function addSave(){
	...
	Server.sendRequest("com.xdarkness.cms.dataservice.Vote.add",dc,function(response){
...
function del(){
	...
	Dialog.confirm("确定要删除该调查吗？",function(){
		var dc = new DataCollection();
			dc.add("IDs",arr.join());
			Server.sendRequest("com.xdarkness.cms.dataservice.Vote.del",dc,function(response){
				Dialog.alert(response.Message,function(){
					if(response.Status==1){
						Tree.loadData("tree1",function(){Tree.select("tree1","cid",currentTreeItem.getAttribute("cid"))});
						DataGrid.loadData("dg1");
...
function handStop(){
	...
	Dialog.confirm("确定要终止该投票吗？",function(){
		var dc = new DataCollection();
			dc.add("IDs",arr.join());
			Server.sendRequest("com.xdarkness.cms.dataservice.Vote.handStop",dc,function(response){
				Dialog.alert(response.Message,function(){
					if(response.Status==1){
						DataGrid.loadData("dg1");
					}
...
function editSave(){
    ...
    dc.add("Prop4", $DW.$NV("Prop4")); 
	Server.sendRequest("com.xdarkness.cms.dataservice.Vote.edit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				if(currentTreeItem){
		...
		<tr valign="top">
			...
			<tr>
				<td style="padding:6px;" class="blockTd"><sky:tree id="tree1"
					...
					method="com.xdarkness.cms.dataservice.Vote.treeDataBind" level="2"
					lazy="true">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="6" class="blockTable">
				...
				<tr>
					<td style="padding:0 8px 4px;"><sky:button onClick="add()"><img src="../Icons/icon032a2.gif" />新建</sky:button> 
					<sky:button onClick="edit()"><img src="../Icons/icon032a4.gif" />修改</sky:button> 
					<sky:button onClick="del()"><img src="../Icons/icon032a3.gif" />删除</sky:button>
					<sky:button onClick="handStop()"><img src="../Icons/icon032a5.gif" />手工终止</sky:button>
					<sky:button onClick="preview()"><img src="../Icons/icon032a15.gif" />预览</sky:button>
					<sky:button onClick="getResult()"><img src="../Icons/icon032a1.gif" />结果</sky:button>
					<sky:button onClick="resultDetails()"><img src="../Icons/icon032a1.gif" />结果明细</sky:button>
					</td>
				</tr>
				<tr>
