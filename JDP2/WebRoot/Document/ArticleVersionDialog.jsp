...
function del(){
	...
	Server.sendRequest("com.xdarkness.cms.document.ArticleHistory.del",dc,function(response){
...
function restore(){
	...
	Server.sendRequest("com.xdarkness.cms.document.ArticleHistory.restore",dc,function(response){
		if(response.Status==0){
			alert(response.Message);
		}else{
			alert("成功恢复文档");
...
function add(){
	var dc = new DataCollection();
	dc.add("ArticleID",$V("ArticleID"));
	Server.sendRequest("com.xdarkness.cms.document.ArticleHistory.addVersion",dc,function(response){
		if(response.Status==0){
			alert(response.Message);
		}else{
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		<div style="float: left"><sky:button onClick="add()">
			<img src="../Icons/icon018a9.gif" />保存为新版本</sky:button> <sky:button
			onClick="restore()">
			<img src="../Icons/icon018a16.gif" />恢复</sky:button> <sky:button onClick="del()">
			<img src="../Icons/icon018a5.gif" />删除</sky:button></div>
		</td>
	</tr>
	<tr>
		...
		<sky:datagrid id="dg1"
			method="com.xdarkness.cms.document.ArticleHistory.historyDataBind"
			size="12">
			<table width="100%" cellpadding="2" cellspacing="0" class="dataTable">
				<tr xtype="head" class="dataTableHead">
					<td width="4%" height="30">&nbsp;</td>
					<td width="4%" xtype="selector" field="BackupNo">&nbsp;</td>
