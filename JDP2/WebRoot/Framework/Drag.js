	...
	if(DragManager.StartFlag){
		...
		if(DragManager.DragProxy){
			...
			if(window==$E.getTopLevelWindow()){
				/*if(evt.srcElement.tagName == "B"
					|| evt.srcElement.tagName == "DIV"
					|| evt.srcElement.tagName == "SPAN"
					|| evt.srcElement.tagName == "BODY"
					|| evt.srcElement.tagName == "IMG"
					
					|| (evt.srcElement.id.indexOf("tabdiv")!=-1))
					*/
					
					//if(evt.srcElement.tagName != "TD" ||
					//	!evt.srcElement.parentElement.xtype || evt.srcElement.parentElement.xtype!='dragTr' )
					//{
			...
			/* 控制拖拽代理不得超出浏览器窗体 */
			var _left = (pos.x-DragManager.DragProxy.cx);
			if(_left < 0) {
				_left = 0;
			}
			
			var _awayX = (DragManager.DragProxy.style.left.replace('px','') - _left);
			
			var _top = 0;
			if(DragManager.DragProxy==DragManager.DragSource){
				_top = (pos.y-DragManager.DragProxy.cy);
			}else{
				_top = (pos.y+5);
			}
			if(_top<0) {
			...
			var _awayY = DragManager.DragProxy.style.top.replace('px','') - _top;
			
			var _clientWidth = window.top.document.body.clientWidth; 
			var _clientHeight = window.top.document.body.clientHeight; 
			var dim = $E.getDimensions(DragManager.DragSource);
			if(_top + dim.height > _clientHeight) {
				_top = _clientHeight - dim.height;
			}
			if(_left + dim.width > _clientWidth) {
				_left = _clientWidth - dim.width;
			}
			
			DragManager.DragProxy.style.left = _left+"px";
			DragManager.DragProxy.style.top = _top+"px";
		}
...
DragManager.onDragExit = function(){
	...
	if(DragManager.DragProxy) {
		var tables = $T('table', DragManager.DragProxy);//DragManager.DragProxy.$T('table');
		for(var i=0;i<tables.length;i++) {
			var table = tables[i];
			if(table.xtype == 'innerTable') {
				table.style.display='';
				var preEle = table.previousElement(); 
...
DragManager.doDrag = function(evt,proxy){
	...
	for(var i=0;i<tables.length;i++) {
		...
		if(table.xtype == 'innerTable') {
			...
			preEle.style.width = dim.width;
			preEle.style.height = dim.height;
			table.style.display='none';
			preEle.style.display = '';
		}
	}
