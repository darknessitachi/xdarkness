...
(function() {
	...
	var phrases = {
		'progressOverall': '总文件大小 ({total})',
		'currentTitle': '当前文件',
		'currentFile': '正在上传 "{name}"',
		'currentProgress': '已上传 {bytesLoaded}，速度 {rate}，还剩 {timeRemaining}.',
		'fileName': '{name}',
		'remove': '删除',
		'removeTitle': '点击删除这个文件.',
		'fileError': '上传失败',
		'validationErrors': {
			'duplicate': '文件 <em>{name}</em> 已经添加过, 不允许添加重复的文件.',
			'sizeLimitMin': '文件 <em>{name}</em> (<em>{size}</em>) 太小, 文件大小最小为 {fileSizeMin}.',
			'sizeLimitMax': '文件 <em>{name}</em> (<em>{size}</em>) 太大, 文件大小最大为 <em>{fileSizeMax}</em>.',
			'fileListMax': '文件 <em>{name}</em> 不能被添加, 上传列表最大允许上传 <em>{fileListMax}</em> 个文件.',
			'fileListSizeMax': '文件 <em>{name}</em> (<em>{size}</em>) 太大, 总文件大小最大允许上传 <em>{fileListSizeMax}</em>.'
		},
		'errors': {
			'httpStatus': '服务器返回 HTTP状态 <code>#{code}</code>',
			'securityError': '安全的错误发生： ({text})',
			'ioError': '一个发送或加载操作失败 ({text})'
		}
	};
