...
function addSave(){
	...
	dc.add("IDs",arr.join());
	Server.sendRequest("com.xdarkness.platform.MenuGroup.add",dc,function(response){
...
function del(){
	...
	dc.add("IDs",arr.join());
	Server.sendRequest("com.xdarkness.platform.MenuGroup.del",dc,function(response){
		alert(response.Message);
		if(response.Status==1){
			window.location="MenuGroup.jsp";
		}
...
function save(){
	...
	dc.add("IDs",arr.join());
	Server.sendRequest("com.xdarkness.platform.MenuGroup.save",dc,function(response){
		alert(response.Message);
		if(response.Status==1){
			$D.close();
			window.location="MenuGroup.jsp";
...
<div id="maincontent">
<sky:datagrid id="dg1" method="com.xdarkness.platform.MenuGroup.dg1DataBind"  editAction="showEditDialog" size="20">
  <table width="100%" cellpadding="2" cellspacing="0" class="dataTable">
    <tr xtype="head" class="dataTableHead">
      <td  width="11">&nbsp;</td>
      <td width="5%" xtype="selector" field="MenuGroupID">&nbsp;</td>
