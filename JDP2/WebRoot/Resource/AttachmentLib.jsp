...
function changeSave(){
	...
	Server.sendRequest("com.xdarkness.cms.resource.AttachmentLib.AttachmentLibEdit",dc,function(response){
...
function addSave(){
	...
	Server.sendRequest("com.xdarkness.cms.resource.AttachmentLib.add",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				Tree.loadData("tree1");
				$D.close();
...
function save(){
	DataGrid.save("dg1","com.xdarkness.cms.resource.AttachmentLib.dg1Edit",function(){
...
function copySave(){
	...
	Server.sendRequest("com.xdarkness.cms.resource.Attachment.copy",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
			}
...
function transferSave(){
	...
	dc.add("IDs",arr.join());
	dc.add("CatalogID",$DW.$V("CatalogID"));
	Server.sendRequest("com.xdarkness.cms.resource.Attachment.transfer",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
...
function delLib(param){
	...
	Dialog.confirm("该分类下的附件都将被删除，你确认要删除此附件分类吗？",function(){
		...
		Server.sendRequest("com.xdarkness.cms.resource.AttachmentLib.delLib",dc,function(response){
		Dialog.alert(response.Message,function(){
				if(response.Status==1){
...
function del(){
	...
	Dialog.confirm("这些附件可能与文章或者图片有关联，你确认要删除吗？",function(){
		var dc = new DataCollection();
		dc.add("IDs",arr.join());
		Server.sendRequest("com.xdarkness.cms.resource.Attachment.del",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					if ($V("SearchName") != "请输入附件名称") {
						DataGrid.setParam("dg1","Name",$V("SearchName"));
...
function publish(){
	...
	var dc = new DataCollection();
	dc.add("IDs",arr.join());
	Server.sendRequest("com.xdarkness.cms.resource.AttachmentLib.publish",dc,function(response){
...
function afterRowDragEnd(type,targetDr,sourceDr,rowIndex,oldIndex){
	...
	Server.sendRequest("com.xdarkness.cms.resource.AttachmentLib.sortColumn",dc,function(response){
		if(response.Status==1){
			DataGrid.loadData("dg1");
		}else{
			Dialog.alert(response.Message);	
...
function DetailEditSave(){
	...
	Server.sendRequest("com.xdarkness.cms.resource.Attachment.dialogEdit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData("dg1");
...
<table width="100%" border="0" cellspacing="6" cellpadding="0" style="border-collapse: separate; border-spacing: 6px;">
	<tr valign="top">
		...
		<table width="180" border="0" cellspacing="0" cellpadding="6" class="blockTable">
			<tr>
				<td style="padding:6px;" class="blockTd"><sky:tree id="tree1" style="height:450px;width:160px;" method="com.xdarkness.cms.resource.AttachmentLib.treeDataBind" lazy="true" expand="false">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="6" class="blockTable">
			<tr>
				...
				<div style="float: left"><sky:button id="addLibButton" priv="attach_manage" onClick="add()">
					<img src="../Icons/icon003a2.gif" />新建分类</sky:button> <sky:button id="uploadButton" priv="attach_modify" onClick="upload()">
					<img src="../Icons/icon003a8.gif" />上传</sky:button> <sky:button id="uploadButton" priv="attach_modify" onClick="RepeatUpload()">
					<img src="../Icons/icon003a8.gif" />重新上传</sky:button> <sky:button id="editButton" priv="attach_modify" onClick="save()">
					<img src="../Icons/icon003a4.gif" />保存</sky:button> <sky:button id="publishButton" priv="attach_modify" onClick="publish()">
					<img src="../Icons/icon003a4.gif" />发布</sky:button> <sky:button id="copyButton" priv="attach_modify" onClick="copy()">
					<img src="../Icons/icon003a17.gif" />复制</sky:button> <sky:button id="transferButton" priv="attach_modify" onClick="transfer()">
					<img src="../Icons/icon003a7.gif" />转移</sky:button> <sky:button id="delButton" priv="attach_modify" onClick="del()">
					<img src="../Icons/icon003a3.gif" />删除</sky:button></div>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<sky:datagrid id="dg1" method="com.xdarkness.cms.resource.AttachmentLib.dg1DataBind" size="15">
					<table width="100%" cellpadding="2" cellspacing="0" class="dataTable" afterDrag="afterRowDragEnd">
						<tr xtype="head" class="dataTableHead">
							<td width="3%" xtype="RowNo" drag="true"><img
