...
function changeSave(){
	...
	Server.sendRequest("com.xdarkness.cms.resource.AudioLib.AudioLibEdit",dc,function(response){
...
function addSave(){
	...
	Server.sendRequest("com.xdarkness.cms.resource.AudioLib.add",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
			Tree.loadData("tree1");
				$D.close();
...
function save(){
	DataGrid.save("dg1","com.xdarkness.cms.resource.AudioLib.dg1Edit",function(){
		DataGrid.loadData('dg1');
	});
}
...
function copySave(){
	...
	Server.sendRequest("com.xdarkness.cms.resource.Audio.copy",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
			}
...
function transferSave(){
	...
	dc.add("IDs",arr.join());
	dc.add("CatalogID",$DW.$V("CatalogID"));
	Server.sendRequest("com.xdarkness.cms.resource.Audio.transfer",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataGrid.loadData("dg1");
...
function delLib(param){
	...
	Dialog.confirm("该分类下的音频都将被删除，你确认要删除此音频分类吗？",function(){
		...
		Server.sendRequest("com.xdarkness.cms.resource.AudioLib.delLib",dc,function(response){
		Dialog.alert(response.Message,function(){
				if(response.Status==1){
...
function del(){
	...
	Dialog.confirm("这些音频可能与文章或者音频播放器有关联，你确认要删除吗？",function(){
		var dc = new DataCollection();
		dc.add("IDs",arr.join());
		Server.sendRequest("com.xdarkness.cms.resource.Audio.del",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				DataGrid.loadData("dg1");
				}
...
function publish(){
	...
	var dc = new DataCollection();
	dc.add("IDs",arr.join());
	Server.sendRequest("com.xdarkness.cms.resource.AudioLib.publish",dc,function(response){
...
function afterRowDragEnd(type,targetDr,sourceDr,rowIndex,oldIndex){
	...
	Server.sendRequest("com.xdarkness.cms.resource.AudioLib.sortColumn",dc,function(response){
		if(response.Status==1){
			DataGrid.loadData("dg1");
		}else{
			Dialog.alert(response.Message);	
...
function DetailSave(){
	...
	Server.sendRequest("com.xdarkness.cms.resource.Audio.dialogEdit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
			}
	...
	<tr valign="top">
		...
		<table width="180" border="0" cellspacing="0" cellpadding="1" class="blockTable">
			<tr>
				<td style="padding:6px;" class="blockTd">
					<sky:tree id="tree1" style="height:450px;width:160px;" method="com.xdarkness.cms.resource.AudioLib.treeDataBind" level="2" lazy="true">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="6" class="blockTable">
			<tr>
				...
				<div style="float: left"><sky:button id="addLibButton" priv="audio_manage" onClick="add()">
					<img src="../Icons/icon051a2.gif" />新建分类</sky:button> <sky:button id="uploadButton" priv="audio_modify" onClick="upload()">
					<img src="../Icons/icon051a13.gif" />上传</sky:button> <sky:button id="uploadButton" priv="attach_modify" onClick="RepeatUpload()">
					<img src="../Icons/icon051a13.gif" />重新上传</sky:button><sky:button id="editButton" priv="audio_modify" onClick="DetailEdit()">
					<img src="../Icons/icon051a4.gif" width="20" height="20" />编辑</sky:button>
					<sky:button id="editButton" priv="audio_modify" onClick="save()">
					<img src="../Icons/icon051a16.gif" width="20" height="20" />保存</sky:button> 
					<sky:button id="publishButton" priv="audio_modify" onClick="publish()">
					<img src="../Icons/icon051a5.gif" />发布</sky:button> <sky:button id="copyButton" priv="audio_modify" onClick="copy()">
					<img src="../Icons/icon051a8.gif" />复制</sky:button> <sky:button id="transferButton" priv="audio_modify" onClick="transfer()">
					<img src="../Icons/icon051a7.gif" />转移</sky:button> <sky:button id="delButton" priv="audio_modify" onClick="del()">
					<img src="../Icons/icon051a3.gif" />删除</sky:button></div>
				</td>
			</tr>
			<tr>
				<td style="padding:4px 10px;">列出： 从 <input type="text" id="StartDate" style="width:100px; " xtype='date'> 至 <input type="text" id="EndDate" style="width:100px; " xtype='date'> &nbsp;关键词: <input name="SearchName" type="text" id="SearchName" onFocus="delKeyWord();" style="width:110px"> <input type="button" name="Submitbutton" value="查询" onClick="doSearch()" id="Submitbutton"></td>
			</tr>
			<tr>
				<td style="padding:0px 5px;"><sky:datagrid id="dg1" method="com.xdarkness.cms.resource.AudioLib.dg1DataBind" size="15">
