...
function setImageCover(imageID){
	...
	dc.add("ID",imageID);
	Server.sendRequest("com.xdarkness.cms.resource.ImageLib.setImageCover",dc,function(response){
...
function setTop(imageID){
	var dc = new DataCollection();
	dc.add("ID",imageID);
	Server.sendRequest("com.xdarkness.cms.resource.ImageLib.setTopper",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				if ($V("Name") != "请输入图片名称") {
...
function changeSave(){
	...
	Server.sendRequest("com.xdarkness.cms.resource.ImageLib.ImageLibEdit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				Tree.loadData("tree1");
				$D.close();
...
function addSave(){
	...
	Server.sendRequest("com.xdarkness.cms.resource.ImageLib.add",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				Tree.loadData("tree1");
				$D.close();
...
function editSave(){
	var dc = $DW.Form.getData("form2");
	Server.sendRequest("com.xdarkness.cms.resource.Image.imagesEdit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				if ($V("Name") != "请输入图片名称") {
...
function copySave(){
	...
	Server.sendRequest("com.xdarkness.cms.resource.Image.copy",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
			}
...
function transferSave(){
	...
	dc.add("IDs",arr.join());
	dc.add("CatalogID",$DW.$V("CatalogID"));
	Server.sendRequest("com.xdarkness.cms.resource.Image.transfer",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				DataList.setParam("dg1",Constant.PageIndex,0);
...
function delLib(param){
	...
	Dialog.confirm("该分类下的图片都将被删除，你确认要删除此图片分类吗？",function(){
		...
		Server.sendRequest("com.xdarkness.cms.resource.ImageLib.delLib",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
...
function del(imageID){
	...
	Dialog.confirm("你确认删除这"+count+"张图片吗？",function(){
		var dc = new DataCollection();
		dc.add("IDs",imageID);
		Server.sendRequest("com.xdarkness.cms.resource.Image.del",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
					if ($V("Name") != "请输入图片名称") {
						DataList.setParam("dg1","Name",$V("Name"));
...
function sortLibSave(){
	...
	Server.sendRequest("com.xdarkness.cms.site.Catalog.sortCatalog",dc,function(response){
		if(response.Status==1){
			Tree.loadData("tree1");
			$D.close();
		}
...
function publish(){
	...
	var dc = new DataCollection();
	dc.add("IDs",arr.join());
	Server.sendRequest("com.xdarkness.cms.resource.ImageLib.publish",dc,function(response){
...
<table width="100%" border="0" cellspacing="6" cellpadding="0" style="border-collapse: separate; border-spacing: 6px;">
	<tr valign="top">
		...
		<table width="180" border="0" cellspacing="0" cellpadding="6" class="blockTable">
			<tr>
				<td style="padding:6px;" colspan="2" class="blockTd"><sky:tree id="tree1" style="height:450px;width:160px;" method="com.xdarkness.cms.resource.ImageLib.treeDataBind" level="2" lazy="true" expand="false">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="blockTable">
			<tr>
				...
				<div style="float: left"><sky:button id="addLibButton" priv="image_manage" onClick="add()">
					<img src="../Icons/icon030a1.gif" />新建分类</sky:button> <sky:button id="uploadButton" priv="image_modify" onClick="upload()">
					<img src="../Icons/icon030a2.gif" />上传</sky:button> <sky:button id="editButton" priv="image_modify" onClick="edit()">
					<img src="../Icons/icon030a4.gif" />编辑</sky:button> <sky:button id="publishButton" priv="image_modify" onClick="publish()">
					<img src="../Icons/icon030a5.gif" />发布</sky:button> <sky:button id="copyButton" priv="image_modify" onClick="copy()">
					<img src="../Icons/icon030a12.gif" />复制</sky:button> <sky:button id="transferButton" priv="image_modify" onClick="transfer()">
					<img src="../Icons/icon030a7.gif" />转移</sky:button> <sky:button id="delButton" priv="image_modify" onClick="del()">
					<img src="../Icons/icon030a3.gif" />删除</sky:button></div>
				</td>
			</tr>
			<tr>
				<td style="padding:2px 10px;"><label> <input name="checkbox" type="checkbox" id="AllCheck" onClick="selectAll(this,'ImageID')"> 全选</label> &nbsp;&nbsp;列出： 从 <input type="text" id="StartDate" style="width:100px; " xtype='date'> 至 <input type="text" id="EndDate" style="width:100px; " xtype='date'> &nbsp;关键词: <input name="Name" type="text" id="Name" onFocus="delKeyWord();" style="width:110px"> <input type="button" name="Submitbutton" value="查询" onClick="doSearch()" id="Submitbutton"></td>
			</tr>
			<tr>
				<td style="padding:0 5px">
				<ul class="img-wrapper" style="height:380px; overflow:hidden; display:block; margin:0;">
					<sky:datalist id="dg1" method="com.xdarkness.cms.resource.ImageLib.dg1DataList" size="14" page="true">
						<li>
						<dl>
			...
			<tr>
				<td style="padding:0px 5px;"><sky:pagebar target="dg1" /></td>
			</tr>
		</table>
		</td>
	</tr>
