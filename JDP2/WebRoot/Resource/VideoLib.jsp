...
function setVideoCover(videoID){
	...
	dc.add("ID",videoID);
	Server.sendRequest("com.xdarkness.cms.resource.VideoLib.setVideoCover",dc,function(response){
...
function setTop(videoID){
	var dc = new DataCollection();
	dc.add("ID",videoID);
	Server.sendRequest("com.xdarkness.cms.resource.VideoLib.setTopper",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				if ($V("Name") != "请输入视频名称") {
...
function changeSave(){
	...
	Server.sendRequest("com.xdarkness.cms.resource.VideoLib.VideoLibEdit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				Tree.loadData("tree1");
				$D.close();
...
function addSave(){
	...
	Server.sendRequest("com.xdarkness.cms.resource.VideoLib.add",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				Tree.loadData("tree1");
				$D.close();
...
function copySave(){
	...
	Server.sendRequest("com.xdarkness.cms.resource.Video.copy",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
			$D.close();
			}
...
function transferSave(){
	...
	dc.add("IDs",arr.join());
	dc.add("CatalogID",$DW.$V("CatalogID"));
	Server.sendRequest("com.xdarkness.cms.resource.Video.transfer",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				if ($V("Name") != "请输入视频名称") {
...
function editSave(){
	...
	Server.sendRequest("com.xdarkness.cms.resource.Video.edit",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				$D.close();
				if ($V("Name") != "请输入视频名称") {
...
function delLib(param){
	...
	Dialog.confirm("该分类下的视频都将被删除，你确认要删除此视频分类吗？",function(){
		...
		Server.sendRequest("com.xdarkness.cms.resource.VideoLib.delLib",dc,function(response){
			Dialog.alert(response.Message,function(){
				if(response.Status==1){
...
function del(videoID){	
	...
	Dialog.confirm("这些视频可能与文章或者视频播放器有关联，你确认要删除吗？",function(){
		var dc = new DataCollection();
		dc.add("IDs",videoID);
		Server.sendRequest("com.xdarkness.cms.resource.Video.del",dc,function(response){
		Dialog.alert(response.Message,function(){
			if(response.Status==1){
				if ($V("Name") != "请输入视频名称") {
					DataList.setParam("dg1","Name",$V("Name"));
...
function publish(){
	...
	var dc = new DataCollection();
	dc.add("IDs",arr.join());
	Server.sendRequest("com.xdarkness.cms.resource.VideoLib.publish",dc,function(response){
...
<table width="100%" border="0" cellspacing="6" cellpadding="0" style="border-collapse: separate; border-spacing: 6px;">
	<tr valign="top">
		...
		<table width="180" border="0" cellspacing="0" cellpadding="6" class="blockTable">
			<tr>
				<td style="padding:6px;" class="blockTd"><sky:tree id="tree1" style="height:450px;width:160px;" method="com.xdarkness.cms.resource.VideoLib.treeDataBind" lazy="true" expand="false">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="blockTable">
			<tr>
				...
				<div style="float: left"><sky:button id="addLibButton" priv="video_manage" onClick="add()">
					<img src="../Icons/icon012a1.gif" />新建分类</sky:button> <sky:button id="uploadButton" priv="video_modify" onClick="upload()">
					<img src="../Icons/icon012a2.gif" />上传</sky:button> <sky:button id="editButton" priv="video_modify" onClick="edit()">
					<img src="../Icons/icon012a4.gif" />修改</sky:button> <sky:button id="publishButton" priv="video_modify" onClick="publish()">
					<img src="../Icons/icon012a5.gif" />发布</sky:button> <sky:button id="copyButton" priv="video_modify" onClick="copy()">
					<img src="../Icons/icon012a12.gif" />复制</sky:button> <sky:button id="transferButton" priv="video_modify" onClick="transfer()">
					<img src="../Icons/icon012a7.gif" />转移</sky:button> <sky:button id="delButton" priv="video_modify" onClick="del()">
					<img src="../Icons/icon012a3.gif" />删除</sky:button></div>
				</td>
			</tr>
			<tr>
				<td style="padding:2px 10px;"><label><input type="checkbox" id="AllCheck" onClick="selectAll(this,'VideoID')">全选</label> &nbsp;&nbsp;列出： 从 <input type="text" id="StartDate" style="width:100px; " xtype='date'> 至 <input type="text" id="EndDate" style="width:100px; " xtype='date'> &nbsp;关键词: <input name="Name" type="text" id="Name" onFocus="delKeyWord();" style="width:110px"> <input type="button" name="Submitbutton" value="查询" onClick="doSearch()" id="Submitbutton"></td>
			</tr>
			<tr>
				<td style="padding:0 5px;">
				<ul class="img-wrapper" style="height:380px; overflow:hidden; display:block; margin:0;">
					<sky:datalist id="dg1" method="com.xdarkness.cms.resource.VideoLib.dg1DataList" size="14" page="true">
						<li>
						<dl>
