...
<%@ page session="false" contentType="text/html; charset=utf-8" pageEncoding="UTF-8" 
	import="com.xdarkness.search.*"
	import="com.xdarkness.cms.api.*" 
	import="com.xdarkness.framework.*"
	import="com.xdarkness.framework.utility.*"
	import="com.xdarkness.framework.data.*"
	import="java.util.*"
%>
		...
		<table cellspacing="0" cellpadding="0">
			<tr>
				...
				<td valign="middle" nowrap>
					...
					<a href="AdvanceResult.jsp?query=<%=StringUtil.urlEncode(query)%>&site=<%=site%>">高级搜索</a>				</td>
			</tr>
		</table>
		</td>
		<td></td>
...
<table cellspacing="0" cellpadding="0" border="0" bgcolor="#ffffff"	align="right" width="24%">
	<tbody>
		<tr>
			...
			<p><b>排序方式</b><br />
			<nobr>
			<%=SearchAPI.getURL(map, "相关度", "order", "rela")%>
			<%=SearchAPI.getURL(map, "发布时间", "order", "time")%>
			</nobr></p>
			<p><b>按时间筛选结果</b><br />
			<nobr>
			<%=SearchAPI.getURL(map, "一周内", "time", "week")%>
			<%=SearchAPI.getURL(map, "一月内", "time", "month")%>
			<%=SearchAPI.getURL(map, "三月内", "time", "quarter")%>
			<%=SearchAPI.getURL(map, "全部", "time", "all")%>
			</nobr></p>
			</div>
			</td>
		</tr>
