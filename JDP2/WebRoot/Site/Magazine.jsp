...
function addMagSave(){
	...
	Server.sendRequest("com.xdarkness.cms.site.Magazine.add",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
function editMagSave(){
	...
	Server.sendRequest("com.xdarkness.cms.site.Magazine.edit",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
function clean(){
	...
	Dialog.confirm('警告：您确认要清空"'+$V("Name")+'"的所有文档吗？',function(){
		dc.add("CatalogID",$V("CatalogID"));
		Server.sendRequest("com.xdarkness.cms.site.Catalog.clean",dc,function(response){
			if(response.Status==0){
				Dialog.alert(response.Message);
			}else{
...
<table width="100%" border="0" cellspacing="6" cellpadding="0"
	...
	<tr valign="top">
		...
		<table width="180" border="0" cellspacing="0" cellpadding="6"
			...
			<tr>
				<td style="padding:6px;" class="blockTd"><sky:tree id="tree1"
					...
					method="com.xdarkness.cms.site.Magazine.treeDataBind" level="2"
					lazy="true">
		...
		<td><sky:tab>
			...
			<sky:childtab id="Extend" onClick="onTabChange('Extend')"
				src="about:blank">
			...
			<sky:childtab id="Column" onClick="onTabChange('Column')"
				src="about:blank">
