...
function save(){
	...
	DataGrid.save("dg1","com.xdarkness.cms.site.Catalog.dg1Edit",function(){window.location.reload();});
}
...
function addSave(){
	...
	Server.sendRequest("com.xdarkness.cms.site.Catalog.add",dc,function(response){
...
function addIssueSave(){
  ...
  if($DW.Verify.hasError()){
	...
	Server.sendRequest("com.xdarkness.cms.site.NewspaperIssue.add",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
function addNewspaperSave(){
	...
	Server.sendRequest("com.xdarkness.cms.site.Newspaper.add",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
			Dialog.alert("新建成功",function(){
...
function editNewspaperSave(){
	...
	Server.sendRequest("com.xdarkness.cms.site.Newspaper.edit",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
function moveSave(){
  ...
  if(arrDest == null || arrDest.length==0){
	...
	Server.sendRequest("com.xdarkness.cms.site.Catalog.move",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
			$D.close();
...
function del(){
	...
	Dialog.confirm('警告：您确认要删除"'+$V("Name")+'"这个栏目吗？\n这样会把栏目下的所有相关文章都删除掉！',function(){
			dc.add("CatalogID",$V("CatalogID"));
			dc.add("CatalogType",$V("CatalogType"));
			Server.sendRequest("com.xdarkness.cms.site.Catalog.del",dc,function(response){
				if(response.Status==0){
					Dialog.alert(response.Message);
				}else{
...
function delNewspaper(){
	...
	Dialog.confirm('警告：您确认要删除"'+$V("Name")+'"这个期刊吗？\n这样会把期刊下的所有相关文章都删除掉！',function(){
			dc.add("CatalogID",$V("CatalogID"));
			dc.add("CatalogType",$V("CatalogType"));
			Server.sendRequest("com.xdarkness.cms.site.Newspaper.del",dc,function(response){
				if(response.Status==0){
					Dialog.alert(response.Message);
				}else{
...
function editSave(){
	...
	Server.sendRequest("com.xdarkness.cms.site.Catalog.save",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
<table width="100%" border="0" cellspacing="6" cellpadding="0"
	...
	<tr valign="top">
		...
		<table width="180" border="0" cellspacing="0" cellpadding="6"
			...
			<tr>
				<td style="padding:6px;" class="blockTd"><sky:tree id="tree1"
					style="height:440px"
					method="com.xdarkness.cms.site.Newspaper.treeDataBind" level="2"
					lazy="true">
		...
		<td><sky:tab>
			<sky:childtab id="Basic" selected="true" onClick="onTabChange('Basic')"
				src="NewspaperSiteBasic.jsp">
				<img src="../Icons/icon002a1.gif" />
				<b>基础信息</b>
			</sky:childtab>
			<sky:childtab id="Template" onClick="onTabChange('Template')"
				src="about:blank">
				<img src="../Icons/icon003a12.gif" />
				<b>模板设置</b>
			</sky:childtab>
			<sky:childtab id="Block" onClick="onTabChange('Block')"
				src="about:blank">
				<img src="../Icons/icon003a1.gif" />
				<b>页面片段</b>
			</sky:childtab>
			<sky:childtab id="Field" onClick="onTabChange('Field')"
				src="SiteField.jsp">
				<img src="../Icons/icon002a4.gif" />
				<b>自定义字段</b>
			</sky:childtab>
			<sky:childtab id="Priv" onClick="onTabChange('Priv')" src="about:blank">
				<img src="../Icons/icon010a8.gif" />
				<b>权限管理</b>
			</sky:childtab>
			<sky:childtab id="Issue" onClick="onTabChange('Issue')"
				src="about:blank">
				<img src="../Icons/icon010a8.gif" />
				<b>期号管理</b>
			</sky:childtab>
			<sky:childtab id="Page" onClick="onTabChange('Page')" src="about:blank">
				<img src="../Icons/icon010a8.gif" />
				<b>版面管理</b>
			</sky:childtab>
		</sky:tab></td>
