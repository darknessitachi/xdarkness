...
function del(){
	...
	Dialog.confirm("确认删除选中的期号吗？",function(){
			...
			Server.sendRequest("com.xdarkness.cms.site.NewspaperIssue.del",dc,function(response){
				if(response.Status==0){
...
function editSave(){
  ...
  if($DW.Verify.hasError()){
	...
	Server.sendRequest("com.xdarkness.cms.site.NewspaperIssue.edit",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
function generate(){
	...
	Server.sendRequest("com.xdarkness.cms.site.NewspaperIssue.generate",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
<body>
<sky:init method="com.xdarkness.cms.site.NewspaperIssue.init">
	<div style="padding:2px;">
	<table width="100%" cellpadding="0" cellspacing="0">
		<tr>
			<td style="padding-bottom:4px"><sky:button onClick="add();">
				<img src="../Platform/Images/tab1_tri.gif" />新建</sky:button> <sky:button
				onClick="edit();">
				<img src="../Platform/Images/tab1_tri.gif" />编辑</sky:button> <sky:button
				onClick="del();">
				<img src="../Platform/Images/tab1_tri.gif" />删除</sky:button> <sky:button
				onClick="copy();">
				<img src="../Platform/Images/tab1_tri.gif" />复制</sky:button> <sky:button
				onClick="generate();">
				<img src="../Platform/Images/tab1_tri.gif" />出版</sky:button></td>
		</tr>
	</table>

	<input name="NewspaperID" type="hidden" id="NewspaperID"
		value="${NewspaperID}" /> <sky:datagrid id="dg1"
		method="com.xdarkness.cms.site.NewspaperIssue.dg1DataBind" size="14">
