...
function addSave(){
  ...
  if($DW.Verify.hasError()){
	...
	Server.sendRequest("com.xdarkness.cms.site.NewspaperPage.add",dc,function(response){
...
function del(){
	...
	Dialog.confirm("确认删除选中的期号吗？",function(){
			...
			Server.sendRequest("com.xdarkness.cms.site.NewspaperPage.del",dc,function(response){
				if(response.Status==0){
...
function editSave(){
  ...
  if($DW.Verify.hasError()){
	...
	Server.sendRequest("com.xdarkness.cms.site.NewspaperPage.edit",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
function generate(){
	...
	Server.sendRequest("com.xdarkness.cms.site.NewspaperPage.generate",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
<body>
<sky:init method="com.xdarkness.cms.site.NewspaperPage.init">
	...
	<sky:datagrid id="dg1"
		method="com.xdarkness.cms.site.NewspaperPage.dg1DataBind" size="14">
		<table width="100%" cellpadding="2" cellspacing="0" class="dataTable">
			<tr xtype="head" class="dataTableHead">
				<td width="7%" xtype="RowNo">序号</td>
