...
function addSave(){
  ...
  if(blockType == 2){
	...
	Server.sendRequest("com.xdarkness.cms.site.PageBlock.add",dc,function(response){
...
function copySave(){
	...
	Server.sendRequest("com.xdarkness.cms.site.PageBlock.copy",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
function del(){
	...
	Dialog.confirm("确认删除选中的页面片段吗？",function(){
			...
			Server.sendRequest("com.xdarkness.cms.site.PageBlock.del",dc,function(response){
				if(response.Status==0){
...
function editSave(){
  ...
  if(blockType == 2){
	...
	Server.sendRequest("com.xdarkness.cms.site.PageBlock.edit",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
function generate(){
	...
	Server.sendRequest("com.xdarkness.cms.site.PageBlock.generate",dc,function(response){
		if(response.Status==0){
			Dialog.alert(response.Message);
		}else{
...
<body>
<sky:init method="com.xdarkness.cms.site.PageBlock.init">
	...
	<table width="100%" cellpadding="0" cellspacing="0"
		...
		<tr>
			<td><sky:button id="BtnAdd" priv="site_manage"
				onClick="add();">
				<img src="../Icons/icon018a2.gif" />新建</sky:button> <sky:button id="BtnEdit"
				priv="site_manage" onClick="edit();">
				<img src="../Icons/icon018a4.gif" />编辑</sky:button> <sky:button id="BtnDel"
				priv="site_manage" onClick="del();">
				<img src="../Icons/icon018a3.gif" />删除</sky:button> <sky:button id="BtnGenerate"
				priv="site_manage" onClick="generate();">
				<img src="../Icons/icon018a6.gif" />生成</sky:button></td>
		</tr>
	</table>
	...
	<sky:datagrid id="dg1" method="com.xdarkness.cms.site.PageBlock.dg1DataBind" size="14">
