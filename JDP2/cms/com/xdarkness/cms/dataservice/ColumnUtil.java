...
public class ColumnUtil {
	...
	public static DataTable getColumnValue(String relaType, String relaID) {
		return new QueryBuilder(
				"select * from zdcolumnvalue where relatype=? and relaid = ?",
				relaType, relaID).executeDataTable();
	}

	public static SchemaSet getValueFromRequest(long catalogID, long docID,
			Mapx Request) {
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			...
			column.fill();
			if (ImageInput.equals(column.getInputType().trim())) {
				String textvalue = Request.getString("_C_"
						+ value.getColumnCode());
				if ((XString.isNotEmpty(textvalue))
						&& (textvalue.indexOf("upload") > 0)) {
					textvalue = textvalue
							.substring(textvalue.indexOf("upload"));
				}
				value.setTextValue(textvalue);
			} else {
				value.setTextValue(Request.getString("_C_"
						+ value.getColumnCode()));
			}
			set.add(value);
		}
	...
	public static void extendDocColumnData(DataTable dt, String catalogID) {
		DataTable columnDT = new QueryBuilder(
				"select columncode from zdcolumnrela where relatype='1' and relaid = ?",
		...
		if (XString.isEmpty(relaidsb.toString())) {
			return;
		}
		...
		DataTable valueDT = new QueryBuilder(
				"select * from zdcolumnvalue where relatype='2' and relaid in ("
						+ relaidsb + ")").executeDataTable();
		...
		for (int j = 0; j < dt.getRowCount(); j++)
			for (int k = colCount; k < dt.getColCount(); k++)
				for (int index = 0; index < valueDT.getRowCount(); index++) {
					if ((!dt.getString(j, "ID").equals(
							valueDT.getString(index, "RelaID")))
							|| (!dt.getDataColumn(k).getColumnName().equals(
									valueDT.getString(index, "ColumnCode"))))
						continue;
	...
	public static void extendCatalogColumnData(DataTable dt, long siteID,
			String levelStr) {
		for (int i = 0; i < dt.getRowCount(); i++) {
			DataRow dr = dt.getDataRow(i);
	...
	public static void extendCatalogColumnData(DataRow dr, String levelStr) {
		extendCatalogColumnData(dr, ApplicationPage.getCurrentSiteID(), levelStr);
	}

	public static void extendCatalogColumnData(DataRow dr, long siteID,
			String levelStr) {
		DataTable valueDT = new QueryBuilder(
		...
		for (int j = 0; j < valueDT.getRowCount(); j++)
			if (ImageInput.equals(valueDT.getString(j, "InputType")))
				...
				dr.insertColumn(valueDT.getString(j, "ColumnCode"), valueDT
						.getString(j, "TextValue"));
	}
	...
	public static String getHtml(String relaType, String relaID,
			String valueRelaType, String valueRelaID) {
		return getHtml(getColumn(relaType, relaID), getColumnValue(
				valueRelaType, valueRelaID));
	}

	public static String getHtml(String relaType, String relaID,
			String valueRelaType, String valueRelaID, String hidden) {
		return getHtml(getColumn(relaType, relaID, hidden), getColumnValue(
				valueRelaType, valueRelaID));
	}
	...
	private static String getHtml(DataRow dr, DataTable valueDT) {
		...
		if ("Y".equals(isMandatory)) {
			...
			if (ImageInput.equals(inputType)) {
				verifyStr = verifyStr + "&&图片不能为空|Script=checkMandatory(\"_C_"
						+ columnCode + "\")";
			}
		}
		...
		if (inputType.equals(Input)) {
			sb.append("<input type='text' size='40' id='" + columnCode
		...
		if (inputType.equals(DateInput)) {
			sb.append("<input name='" + columnCode + "' id='" + columnCode
					+ "' value='" + defaultValue
					+ "' type='text' size='14' xtype='Date' " + verifyStr
					+ " />");
		}
		...
		if (inputType.equals(ImageInput)) {
			...
			if (XString.isEmpty(defaultValue)) {
				defaultValue = (Config.getContextPath()
						...
						+ ApplicationPage.getCurrentSiteAlias() + "/upload/Image/nopicture.jpg")
						.replaceAll("//", "/");
			}
			...
			if (valueDT != null) {
				for (int i = 0; i < valueDT.getRowCount(); i++) {
					DataRow r = valueDT.getDataRow(i);
					if ((!columnCode.equalsIgnoreCase("_C_"
							+ r.getString("columnCode")))
							|| (r.getString("TextValue") == null))
						continue;
					defaultValue = (Config.getContextPath()
							+ Config.getValue("UploadDir") + "/"
							+ ApplicationPage.getCurrentSiteAlias() + "/" + r
							.getString("TextValue")).replaceAll("///", "/")
							.replaceAll("//", "/");
				}
			}
			...
			sb
					.append("<img src='"
							+ defaultValue.replaceAll("1_", "s_")
							+ "' name='Img"
							+ columnCode
							+ "' id='Img"
							+ columnCode
							+ "'><input name='button' type='button' onClick=\"custom_img_upload('"
							+ columnCode
							+ "');\" value='浏览...' /> 图片路径 <input type='text' id='"
							+ columnCode
							+ "' name='"
							+ columnCode
							+ "' size='40' onblur='document.getElementById(\"Img"
							+ columnCode + "\").src=this.value;' value='"
	...
	public static String getText(String relaType, String relaID,
			String valueRelaType, String valueRelaID) {
		return getText(getColumn(relaType, relaID), getColumnValue(
				valueRelaType, valueRelaID));
	}
...
/*
 * com.xdarkness.cms.dataservice.ColumnUtil JD-Core Version: 0.6.0
 */