...
public abstract class AbstractStat {
	...
	public void init() {
		...
		DataTable dt = new QueryBuilder(
				"select * from ZCStatItem where Period=? and Type=?", period,
				getStatType()).executeDataTable();
		if (dt.getRowCount() == 0) {
			...
			for (int i = 0; i < dt.getRowCount(); i++) {
				...
				if (avgSubTypes != null) {
					for (int j = 0; j < avgSubTypes.length; j++) {
						if (avgSubTypes[j].equals(subtype)) {
							VisitCount.getInstance().addAverage(siteID, type,
									subtype, item, 0L, false);
							flag = true;
							break;
						}
					}
				...
				if (!flag)
					VisitCount.getInstance().add(siteID, type, subtype, item,
							0, false);
			}
		}
	}
	...
	public void update(Transaction tran, VisitCount vc, long current,
		...
		if (newMonthFlag) {
			...
			for (int i = 0; i < sites.length; i++) {
				...
				for (int j = 0; j < subtypes.length; j++) {
					...
					for (int k = 0; k < items.length; k++) {
						long count = vc.get(sites[i], type, subtypes[j],
								items[k]);
						if (count == 0L) {
							continue;
						}
						...
						set.add(si);
						VisitCount.getInstance().set(sites[i], type,
								subtypes[j], items[k], 0L, false);
					}
				}
			}
			...
			QueryBuilder qb = new QueryBuilder("update ZCStatItem set Count"
					+ day + "=Count" + day + "+? where SiteID=? and "
			...
			for (int i = 0; i < sites.length; i++) {
				...
				for (int j = 0; j < subtypes.length; j++) {
					...
					for (int k = 0; k < items.length; k++) {
						long count = vc.get(sites[i], type, subtypes[j],
								items[k]);
						boolean isNeedInsert = vc.isNeedInsert(sites[i], type,
								subtypes[j], items[k]);
						if (count == 0L) {
							continue;
						}
						if (!isNeedInsert) {
							...
							qb.add(items[k]);
							qb.addBatch();
						} else if (XString.isNotEmpty(items[k])) {
							ZCStatItemSchema si = new ZCStatItemSchema();
							si.setSiteID(sites[i]);
							si.setPeriod(period);
							si.setType(type);
