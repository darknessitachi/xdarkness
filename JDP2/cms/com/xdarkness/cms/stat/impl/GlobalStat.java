...
public class GlobalStat extends AbstractStat {
	private static final String[] avgSubTypes = { "StickTime", "VisitDepth",
			"StickTotalTime" };
	...
	private static final long AliveTimeOut = 600000L;
	...
	public void deal(Visit v) {
		if ("Unload".equals(v.Event)) {
			VisitCount.getInstance().addAverage(v.SiteID, "Global",
					"StickTime", "0", v.StickTime);
		} else {
			VisitCount.getInstance().add(v.SiteID, "Global", "PV", "0");
			if (v.UVFlag) {
				VisitCount.getInstance().add(v.SiteID, "Global", "UV", "0");
				if (v.RVFlag)
					VisitCount.getInstance().add(v.SiteID, "Global",
							"NewVisitor", "0");
				else {
					VisitCount.getInstance().add(v.SiteID, "Global",
							"ReturnVisitor", "0");
		...
		synchronized (map) {
			...
			if (obj == null) {
				...
				if (time - vh.LastTime < 600000L) {
					...
					long avgStickTime = VisitCount.getInstance().get(time,
							"Global", "StickTime", "0");
					...
					VisitCount.getInstance()
							.addAverage(
									...
									"StickTotalTime",
									"0",
									avgStickTime + (vh.LastTime - vh.StartTime)
											/ 1000L);
					VisitCount.getInstance().add(v.SiteID, "URL", "Exit",
							vh.URL);
					vh.StartTime = time;
					vh.LastTime = time;
					vh.Depth = 1;
		...
		for (int k = 0; k < sites.length; k++) {
			...
			synchronized (map) {
				if (time - lastOnlineCheckTime >= 60000L) {
					...
					long m15 = VisitCount.getInstance().get(siteID, "Hour",
							"15Min", h);
					long m10 = VisitCount.getInstance().get(siteID, "Hour",
							"10Min", h);
					long m5 = VisitCount.getInstance().get(siteID, "Hour",
							"5Min", h);

					if (m15 < c15) {
						VisitCount.getInstance().set(siteID, "Hour", "15Min",
								h, c15);
					}
					if (m10 < c10) {
						VisitCount.getInstance().set(siteID, "Hour", "10Min",
								h, c10);
					}
					if (m5 < c5) {
						VisitCount.getInstance().set(siteID, "Hour", "5Min", h,
								c5);
					}
					lastOnlineCheckTime = time;
				}
				if (time - lastIDClearTime >= 30000L) {
					Object[] ks = map.keyArray();
					Object[] vs = map.valueArray();
					for (int i = 0; i < ks.length; i++) {
						VisitHistory vh = (VisitHistory) vs[i];
						...
						VisitCount.getInstance().add(siteID, "Global",
								"VisitDepth", item);
						VisitCount.getInstance().addAverage(
								siteID,
								"Global",
								"StickTotalTime",
								"0",
								avgStickTime + (vh.LastTime - vh.StartTime)
										/ 1000L);
						VisitCount.getInstance().add(siteID, "URL", "Exit",
								vh.URL);
						map.remove(ks[i]);
					}
...
/*
 * com.xdarkness.cms.stat.impl.GlobalStat JD-Core Version: 0.6.0
 */