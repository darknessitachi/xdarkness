...
import com.xdarkness.cms.stat.Counter;
import com.xdarkness.cms.stat.Visit;
import com.xdarkness.cms.stat.VisitCount;
import com.xdarkness.schema.ZCVisitLogSchema;
import com.xdarkness.schema.ZCVisitLogSet;
...
public class LeafStat extends AbstractStat {
	...
	public void deal(Visit v) {
		if (v.LeafID != 0L) {
			if (!this.siteMap.containsKey(new Long(v.SiteID))) {
				VisitCount.getInstance().initLRUMap(v.SiteID, "Article",
						"StickTime", 10000,
						new LeafExitEventListener("Article", "StickTime"));
				VisitCount.getInstance().initLRUMap(v.SiteID, "Image",
						"StickTime", 5000,
						new LeafExitEventListener("Image", "StickTime"));
				VisitCount.getInstance().initLRUMap(v.SiteID, "Video",
						"StickTime", 5000,
						new LeafExitEventListener("Video", "StickTime"));
				VisitCount.getInstance().initLRUMap(v.SiteID, "Article", "PV",
						10000, new LeafExitEventListener("Article", "PV"));
				VisitCount.getInstance().initLRUMap(v.SiteID, "Image", "PV",
						5000, new LeafExitEventListener("Image", "PV"));
				VisitCount.getInstance().initLRUMap(v.SiteID, "Video", "PV",
						5000, new LeafExitEventListener("Video", "PV"));
				this.siteMap.put(new Long(v.SiteID), "");
			}
			if ("Unload".equals(v.Event)) {
				VisitCount.getInstance().addAverage(v.SiteID, v.Type,
						"StickTime", v.LeafID + "", v.StickTime);
			} else if (XString.isNotEmpty(v.Type)) {
				if (v.Type.equals("AD")) {
					VisitCount.getInstance().add(v.SiteID, "Leaf", "AD",
							v.LeafID+"");
				} else {
					VisitCount.getInstance().add(v.SiteID, v.Type, "PV",
							v.LeafID+"");
	...
	public static void dealADClick(String siteID, String ADID) {
		VisitCount.getInstance()
				.add(Long.parseLong(siteID), "Leaf", "AD", ADID);
	}
	...
	public void update(Transaction tran, VisitCount vc, long current,
			boolean isNewMonth, boolean isNewPeriod) {
		...
		for (int i = 0; i < sites.length; i++) {
			...
			for (int k = 0; k < items.length; k++) {
				...
				long sticktime = vc.get(sites[i], "Article", "StickTime",
						items[k]);
				...
				qbArticle.addBatch();
				VisitCount.getInstance().set(sites[i], "Article", "PV",
						items[k], 0);
			}
			...
			for (int k = 0; k < items.length; k++) {
				...
				long sticktime = vc.get(sites[i], "Image", "StickTime",
						items[k]);
				...
				qbImage.add(items[k]);
				qbImage.addBatch();
				VisitCount.getInstance().set(sites[i], "Image", "PV", items[k],
						0);
			}
			...
			for (int k = 0; k < items.length; k++) {
				...
				long sticktime = vc.get(sites[i], "Video", "StickTime",
						items[k]);
				...
				qbVideo.add(items[k]);
				qbVideo.addBatch();
				VisitCount.getInstance().set(sites[i], "Video", "PV", items[k],
						0);
			}
		}
		tran.add(qbAD);
	...
	public void updateLastVisitlog(long siteID) {
		...
		for (int i = 0; i < arr.length; i++) {
			...
			if (v.District.startsWith("00")) {
				...
				String prov = map
						.getString(v.District.substring(0, 2) + "0000");
		...
		if (!"Y".equals(Config.getValue("RetainAllVisitLog"))) {
			if (q.size() != 100) {
				DataTable dt = new QueryBuilder(
						"select ID from ZCVisitLog where SiteID=?", siteID)
						.executePagedDataTable(100, 0);
				if (dt.getRowCount() > 100 - q.size()) {
					long ID = dt.getLong(dt.getRowCount() + q.size() - 100,
							"ID");
					tran.add(new QueryBuilder(
							"delete from ZCVisitLog where SiteID=? and ID<?",
							siteID, ID));
				}
			} else {
				tran.add(new QueryBuilder(
						"delete from ZCVisitLog where SiteID=?", siteID));
			}
		}
		tran.add(set, OperateType.DELETE_AND_INSERT);
		tran.commit();
		q.clear();
	}
