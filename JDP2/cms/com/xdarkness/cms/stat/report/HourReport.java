...
public class HourReport extends Page {
	public void getChartData() {
		...
		DataTable dt = getHourData(ApplicationPage.getCurrentSiteID(), start, end);
	...
	public static void dg1DataBind(DataGridAction dga) {
		...
		if (dt.getRowCount() > 0) {
			...
			ReportUtil.addTotal(dt, new String[] { "PV", "UV", "IP",
					"ReturnVisitor" });
	...
	public static DataTable getHourData(long siteID, Date start, Date end) {
		...
		QueryBuilder qb = new QueryBuilder(
				"select * from ZCStatItem where SiteID=? and Type='Hour' and SubType in ('PV','IP','UV','ReturnVisitor') and Period>=? and Period<=?");
		qb.add(siteID);
		qb.add(period1);
		qb.add(period2);
