...
public class ReportUtil {
	...
	public static int getTotalStatDays(long siteID) {
		int c = new Double(Math
				.ceil((System.currentTimeMillis() - getStartStatDate(siteID)
			...
			for (int i = 5; i < dt.getColCount(); i++) {
				if (dt.getInt(0, i) != 0) {
					...
					date = DateUtil.parse(dt.getString(0, "Period")
							+ (i > 9 ? i : new StringBuffer("0").append(i)
									.toString()), "yyyyMMdd");
		...
		for (int i = 0; i < src.getRowCount(); i++) {
			...
			if (period.equals(period1)) {
				...
				for (int j = colStart; j >= 4 + day1; j--) {
					int d = j - 4;
					String key = period
							+ (d > 9 ? d : new StringBuffer("0").append(d)
									.toString());
				...
				int colStart = src.getColCount() - 1;
				if (colStart > DateUtil.getMaxDayOfMonth(DateUtil.parse(period,
						"yyyyMM")) + 4) {
					colStart = DateUtil.getMaxDayOfMonth(DateUtil.parse(period,
							"yyyyMM")) + 4;
				}
	...
	public static DataTable toItemTable(DataTable src, Date start, Date end,
		...
		for (int i = 0; i < src.getRowCount(); i++) {
			...
			if (period.equals(period1)) {
				...
				for (int j = colStart; j >= 4 + day1; j--) {
					...
					map.put(subtype, new Integer(((Integer) map.get(subtype))
							.intValue()
							+ count));
				}
			} else if (period.equals(period2)) {
		...
		for (int i = 0; i < ks.length; i++) {
			...
			for (int j = 0; j < subtypes.length; j++) {
				...
				dt.set(dt.getRowCount() - 1, subtypes[j].toString(),
						new Integer(count));
			}
		}
		return dt;
	...
	public static void computeRate(DataTable dt, String columName,
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			...
			dt.set(i, destColumnName, new Double(NumberUtil.round(count
					* 100.0D / total, 2)));
		}
	}
...
/*
 * com.xdarkness.cms.stat.report.ReportUtil JD-Core Version: 0.6.0
 */