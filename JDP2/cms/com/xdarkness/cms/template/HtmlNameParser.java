...
public class HtmlNameParser {
	...
	public HtmlNameRule getNameRule() {
		HtmlNameRule rule = (HtmlNameRule) cacheMap.get(this.catalog
				.getString("id")
				+ this.nameRule);
		if (rule == null) {
			rule = parse();
			if (!XString.isEmpty(this.nameRule)) {
				cacheMap
						.put(this.catalog.getString("id") + this.nameRule, rule);
			}
		}
		return rule;
	}
	...
	private HtmlNameRule parse() {
		...
		if (XString.isNotEmpty(this.nameRule)) {
			Date date = null;
			long timestamp;
			if ((this.document != null)
					&& (this.document.get("PublishDate") != null)) {
			...
			while (m.find(lastEndIndex)) {
				...
				if ("year".equals(field))
					...
					if (this.document != null)
						sb.append(CatalogUtil.getPath(this.catalog
								.getString("id")));
					else {
						sb.append(CatalogUtil.getFullPath(this.catalog
								.getString("id")));
					}
				}
			...
			fullPath = CatalogUtil.getPath(this.catalog.getString("ID"))
					+ this.document.getString("id") + ".shtml";
		} else {
		...
		int level = XString.count(fullPath, "/");
...
/*
 * com.xdarkness.cms.template.HtmlNameParser JD-Core Version: 0.6.0
 */