...
import com.xdarkness.jaf.controls.html.HtmlSelect;
...
public class SelectTag extends BodyTagSupport {
	...
	public String getHtml(String content) {
		...
		sb.append("<div id='" + this.id + "' name='" + this.name
						+ "' selectedIndex='" + this.selectedIndex
						+ "' xtype='Select'");
		
		sb.append("  class='" + StringUtil.getString(this.className) + " zSelect'");
			...
			try {
				...
				try {
					if (Integer.parseInt(select.getAttribute("listwidth")) > 0)
						this.listWidth = Integer.parseInt(select
								.getAttribute("listwidth"));
				} catch (Exception localException1) {
				}
				try {
					if (Integer.parseInt(select.getAttribute("listheight")) > 0)
						this.listHeight = Integer.parseInt(select
								.getAttribute("listheight"));
		...
		while (m.find(lastIndex)) {
			...
			if (this.showValue)
				sb.append(getOption(m.group(3)
						+ ((StringUtil.isNotEmpty(m.group(4))) ? "-"
								+ m.group(4) : ""), m.group(3)));
			else {
	...
	private String getCodeData() {
		...
		if (dt != null) {
			...
			if (dt.getColCount() > 2) {
				...
				sb.append("$('" + this.id
						+ "').DataSource.init(_Zving_Cols,_Zving_Values);");
				sb.append("}\n</script>\n");
			}
		}
		return sb.toString();
