...
import com.xdarkness.schema.ZCWebGatherSet;
...
public class FromWeb extends Page {
	public static Mapx init(Mapx map) {
		DataTable dt = new QueryBuilder(
				"select code,id,name from ZCCustomTable").executeDataTable();
	...
	public void add() {
		...
		if (XString.isNotEmpty(id)) {
			...
			wg.setSiteID(ApplicationPage.getCurrentSiteID());
		...
		for (int i = 0; i < this.request.size(); i++) {
			...
			if (key.startsWith("RefCode")) {
				...
				String content = this.request.getString(key.replaceAll(
						"RefCode", "Template"));
			...
			if (key.startsWith("URL")) {
				...
				String content = this.request.getString(key);
		...
		boolean flag = XString.isEmpty(id) ? wg.insert() : wg.update();
		if (flag)
	...
	public void delResult() {
		...
		try {
			...
			this.response.setStatus(0);
			this.response.setMessage("清空采集结果时发生错误:任务\"" + wg.getName()
					+ "\"正在执行中!");
		}
	}
...
/*
 * com.xdarkness.datachannel.FromWeb JD-Core Version: 0.6.0
 */