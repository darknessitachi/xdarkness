...
public class InnerSyncService extends Ajax {
	...
	public void sendData() {
		...
		prepareData(this.response, InnerCode,
				new Date(Long.parseLong(LastTime)), "Y"
						.equals(SyncArticleModify), "Y"
						.equals(SyncCatalogInsert), "Y"
						.equals(SyncCatalogModify), false);
		$S("Success", "1");
	}
	...
	public void sendImage() {
		...
		String data = XString.base64Encode(bs);
		$S("Data", data);
		$S("Success", "1");
	}
	...
	public void receiveData() {
		...
		if ("Y".equals(config.getAllowInnerDeploy())) {
			if ((XString.isNotEmpty(config.getInnerDeployPassword()))
	...
	public static void executeGather(ZCInnerGatherSchema gather,
			LongTimeTask task) {
		...
		try {
			...
			for (int i = 0; i < dt.getRowCount(); i++) {
				...
				if (ServerAddr.equalsIgnoreCase("localhost")) {
					map = new Mapx();
					NextTime = prepareData(map, CatalogUtil
							.getInnerCode(CatalogID), new Date(Long
							.parseLong(LastTime)), "Y".equals(gather
							.getSyncArticleModify()), "Y".equals(gather
							.getSyncCatalogInsert()), "Y".equals(gather
							.getSyncCatalogModify()), false);
					map.put("Success", "1");
				} else {
					...
					Request.put("SyncArticleModify", gather
							.getSyncArticleModify());
					Request.put("SyncCatalogInsert", gather
							.getSyncCatalogInsert());
					Request.put("SyncCatalogModify", gather
							.getSyncCatalogModify());
					Request.put("LastTime", LastTime);
					map = Framework.callRemoteMethod(ServerAddr,
							"com.xdarkness.datachannel.InnerSyncService.sendData",
							Request);
				...
				dealData(map, CatalogUtil.getIDByInnerCode(gather
						.getCatalogInnerCode()), ServerAddr, CatalogID, null);
				dt.set(i, "LastTime", NextTime.getTime());
			}
			gather.setTargetCatalog(dt.toString());
			gather.update();
		} finally {
			ExecutingGatherID = 0L;
		}
		ExecutingGatherID = 0L;
	}

	public static synchronized void executeDeploy(ZCInnerDeploySchema deploy,
			LongTimeTask task) {
		...
		try {
			...
			if (ServiceContext.endsWith("/")) {
				ServiceContext = ServiceContext.substring(0, ServiceContext
						.length() - 1);
			}
			ServiceContext = ServiceContext.substring(0, ServiceContext
					.lastIndexOf("/") + 1);
			String SourceCatalogID = CatalogUtil.getIDByInnerCode(deploy
					.getCatalogInnerCode());
			...
			for (int i = 0; i < dt.getRowCount(); i++) {
				...
				String CatalogName = dt.getString(i, "CatalogName");
				String Password = dt.getString(i, "Password");
				task.setCurrentInfo("正在分发数据到站点 [" + SiteName + "] 下的栏目 ["
						+ CatalogName + "]");
				...
				if ((XString.isEmpty(LastTime))
						|| (LastTime.equalsIgnoreCase("_SKY_NULL"))) {
					LastTime = "0";
				}
				Request.put("LastTime", LastTime);

				Date NextTime = prepareData(Request, InnerCode, new Date(Long
						.parseLong(LastTime)), "Y".equals(SyncArticleModify),
						"Y".equals(SyncCatalogInsert), "Y"
								.equals(SyncCatalogModify), deploy
								.getDeployType().equals("2"));

				String md5 = XString.md5Hex(ServerAddr + "," + SiteID + ","
						+ CatalogID);
				...
				if (articles != null) {
					for (int j = articles.getRowCount() - 1; j >= 0; j--) {
						if ((!deploy.getDeployType().equals("2"))
								|| (articles.getString(j, "ClusterTarget")
										.indexOf(md5) >= 0))
							continue;
				...
				if (ServerAddr.equalsIgnoreCase("localhost")) {
					if (deploy.getAddUser().equals("admin")) {
						...
						dealData(Request, CatalogID, ServerAddr,
								SourceCatalogID, deploy);
						map.put("Success", "1");
					} else {
					...
					map = Framework.callRemoteMethod(ServerAddr,
							"com.xdarkness.datachannel.InnerSyncService.receiveData",
							Request);
			...
			ExecutingDeployID = 0L;
		}
		ExecutingDeployID = 0L;
	}

	public static synchronized Date prepareData(Mapx map, String InnerCode,
			Date LastTime, boolean isSyncArticleModify,
			boolean isSyncCatalogInsert, boolean isSyncCatalogModify,
			boolean isManualDeploy) {
		String siteurl = SiteUtil.getURL(CatalogUtil
				.getSiteIDByInnerCode(InnerCode))
				+ "/";
		if (!siteurl.endsWith("/")) {
			siteurl = siteurl + "/";
		}
		String uploadPath = Config.getContextPath()
				...
				+ "/"
				+ SiteUtil
						.getAlias(CatalogUtil.getSiteIDByInnerCode(InnerCode))
				+ "/";
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			...
			content = content.replaceAll(uploadPath + "upload/Attach", siteurl
					+ "upload/Attach");
			content = content.replaceAll(uploadPath + "upload/Audio", siteurl
					+ "upload/Audio");
			content = content.replaceAll("../upload/Video", siteurl
					+ "upload/Video");
		...
		if (isSyncArticleModify) {
			...
			if (isManualDeploy) {
				qb
						.append(" and ClusterTarget is not null and ClusterTarget<>''");
			}
			qb.append(" order by id");
			...
			for (int i = 0; i < dt.getRowCount(); i++) {
				...
				String content = dt.getString(i, "Content");
				content = content.replaceAll(uploadPath + "upload/Attach",
						siteurl + "upload/Attach");
				content = content.replaceAll(uploadPath + "upload/Audio",
						siteurl + "upload/Audio");
				content = content.replaceAll("../upload/Video", siteurl
						+ "upload/Video");
				content = content.replaceAll("zcmsvideorela=\".*?\"", "");
				content = content.replaceAll("zcmsattachrela=\".*?\"", "");
			...
			if (XString.isNotEmpty(ids))
				ids = ids + "," + XString.join(dt.getColumnValues("ID"));
			else {
				ids = XString.join(dt.getColumnValues("ID"));
			}
			if (ids.endsWith(",")) {
			...
			map.put("DeletedArticles", XString.join(dt.getColumnValues(0)));
		}

		if (XString.isNotEmpty(ids)) {
			dt = new QueryBuilder(
					"select * from ZDColumnValue where RelaID in (" + ids
							+ ") and RelaType=?", "2").executeDataTable();
			map.put("ZDColumnValue-Article", dt);

			dt = new QueryBuilder("select * from ZCImageRela where RelaID in ("
					+ ids + ") and RelaType=?", "ArticleImage")
					.executeDataTable();
			...
			if (XString.isNotEmpty(ids)) {
				qb = new QueryBuilder(
						"select * from ZCImage where Status=? and id in ("
								+ ids + ") order by id");
				qb.add(30);
				dt = qb.executeDataTable();
				map.put("ZCImage", dt);
		...
		if (isSyncCatalogInsert) {
			qb = new QueryBuilder(
					"select * from ZCCatalog where InnerCode like ? and AddTime>? order by id");
			qb.add(InnerCode + "%");
			qb.add(LastTime);
			dt = qb.executePagedDataTable(50, 0);
			...
			if (XString.isNotEmpty(ids)) {
				...
				dt = new QueryBuilder(
						"select * from ZDColumnRela where RelaType in ("
								+ types + ") and RelaID in(" + ids + ")")
						.executeDataTable();
				map.put("ZDColumnRela", dt);

				String columnIDs = XString.join(dt
						.getColumnValues("ColumnID"));
				if (XString.isNotEmpty(columnIDs)) {
					dt = new QueryBuilder(
							"select * from ZDColumn where id in (" + columnIDs
									+ ")").executeDataTable();
					map.put("ZDColumn", dt);
				}

				dt = new QueryBuilder(
						"select * from ZDColumnValue where RelaType=0 and RelaID in("
	...
	public static void dealData(Mapx map, String catalogID, String ServerAddr,
			String sourceCatalogID, ZCInnerDeploySchema deploy) {
		...
		DataTable articleColumnValues = (DataTable) map
				.get("ZDColumnValue-Article");
		DataTable catalogColumnValues = (DataTable) map
				.get("ZDColumnValue-Catalog");
		...
		for (int i = 0; i < catalogSet.size(); i++) {
			...
			if (XString.isNotEmpty(ClusterSourceID)) {
		...
		if (SyncCatalogModify) {
			...
			if ((dt != null) && (dt.getRowCount() > 0)) {
				...
				for (int i = 0; i < dt.getRowCount(); i++) {
					...
					String ClusterSourceID = ServerAddr + ","
							+ dt.getString(i, "SiteID") + ","
							+ dt.getString(i, "ID");
					int index = 0;
					for (int j = 0; j < catalogSet.size(); j++) {
						if ((!XString.isNotEmpty(catalogSet.get(j)
								.getClusterSourceID()))
								|| (!catalogSet.get(j).getClusterSourceID()
										.equals(ClusterSourceID)))
							continue;
						c2 = catalogSet.get(j);
						index = j;
						break;
					...
					c.setID(c2.getID());
					c
							.setParentID(idmap.getString("Catalog."
									+ c.getParentID()));
					...
					c.setInnerCode(CatalogUtil
							.createCatalogInnerCode(parentInnerCode));
					String name = Catalog.checkAliasExists(c.getAlias(),
							catalog.getSiteID(), c.getParentID());
					if (XString.isNotEmpty(name)) {
						LogUtil.warn("网站群同步栏目添加时发现栏目别名[" + c.getAlias()
								+ "]己被栏目[" + name + "]占用!");
					} else {
				...
				tran.add(set, OperateType.UPDATE);
			}
			...
			if ((dt != null) && (dt.getRowCount() > 0)) {
				...
				for (int i = 0; i < dt.getRowCount(); i++) {
					...
					String name = Catalog.checkAliasExists(c.getAlias(),
							catalog.getSiteID(), c.getParentID());
					if (XString.isNotEmpty(name)) {
						LogUtil.warn("网站群同步栏目添加时发现栏目别名" + c.getAlias() + "己被栏目"
								+ name + "占用!");
					} else {
						c.setClusterSourceID(ServerAddr + ","
								+ dt.getString(i, "SiteID") + ","
								+ dt.getString(i, "ID"));
						set.add(c);
						catalogSet.add(c);
						idmap
								.put("Catalog." + dt.getString(i, "ID"), c
										.getID());
					}
				}
				tran.add(set, OperateType.INSERT);
			}
					...
					if ((columns != null) && (columns.getRowCount() > 0))
						for (int j = 0; j < columns.getRowCount(); j++)
							if (columns.getLong(j, "ID") == cr.getColumnID()) {
								...
								cSet.add(column);
								idmap.put("Column."
										+ columns.getString(j, "ID"), column
										.getID());
								cr.setColumnID(column.getID());
							}
				} else {
					idmap.put("Column." + columnRela.getString(i, "ColumnID"),
							set.get(0).getColumnID());
				}
			}
			if ((catalogColumnValues != null)
					&& (catalogColumnValues.getRowCount() > 0)) {
				for (int j = 0; j < catalogColumnValues.getRowCount(); j++) {
					...
					v.setColumnID(idmap.getLong("Column."
							+ catalogColumnValues.getString(j, "ColumnID")));
					v.setRelaID(idmap.getString("Catalog."
							+ catalogColumnValues.getString(j, "RelaID")));
					v.setRelaType(catalogColumnValues.getString(j, "RelaType"));
					ZDColumnValueSet set2 = v.query();
					if (set2.size() > 0) {
						v = set2.get(0);
						v.setTextValue(catalogColumnValues.getString(j,
								"TextValue"));
						vSetUpdate.add(v);
					} else {
						...
						v
								.setColumnID(idmap.getLong("Column."
										+ catalogColumnValues.getString(j,
												"ColumnID")));
						v.setRelaID(idmap.getString("Catalog."
								+ catalogColumnValues.getString(j, "RelaID")));
						v.setRelaType(catalogColumnValues.getString(j,
								"RelaType"));
						vSetInsert.add(v);
					}
				}
			}
			tran.add(crSet, OperateType.INSERT);
			tran.add(cSet, OperateType.INSERT);
			tran.add(vSetUpdate, OperateType.UPDATE);
			tran.add(vSetInsert, OperateType.INSERT);
		}
		...
		String imageCatalogID = new QueryBuilder(
				"select ID from ZCCatalog where type='4' and Name =?  and siteid=?",
				autoSaveLib, catalog.getSiteID()).executeString();

		if (XString.isEmpty(imageCatalogID)) {
			imageCatalogID = new QueryBuilder(
		...
		if (SyncArticleModify) {
			...
			if (!XString.checkID(DeletedArticles)) {
				LogUtil.warn("InnerSyncService.dealData()可能的SQL注入攻击:"
						+ DeletedArticles);
				return;
			}
			if (XString.isNotEmpty(DeletedArticles)) {
				ZCArticleSet set = new ZCArticleSchema()
			...
			if ((dt != null) && (dt.getRowCount() > 0)) {
				for (int i = 0; i < dt.getRowCount(); i++) {
					...
					QueryBuilder qb = new QueryBuilder(
							"delete from ZCImageRela where RelaType=? and RelaID=?");
					qb.add("ArticleImage");
					...
					if (articleColumnValues != null) {
						for (int j = 0; j < articleColumnValues.getRowCount(); j++) {
							if ((!articleColumnValues.getString(j, "RelaType")
									.equals("2"))
									|| (!articleColumnValues.getString(j,
											"RelaID").equals(
											dt.getString(i, "ID"))))
								continue;
							ZDColumnValueSchema v = new ZDColumnValueSchema();
							...
							v.setRelaType(articleColumnValues.getString(j,
									"RelaType"));
							ZDColumnValueSet set2 = v.query();
							if (set2.size() > 0) {
								v = set2.get(0);
								v.setTextValue(articleColumnValues.getString(j,
										"TextValue"));
								vSetUpdate.add(v);
							} else {
					...
					if ((isSameServer)
							&& (dt.getLong(i, "SiteID") == catalog.getSiteID())) {
						continue;
					}
					...
					for (int j = 0; j < imageRelas.getRowCount(); j++) {
						if ((!imageRelas.getString(j, "RelaType").equals(
								"ArticleImage"))
								|| (!imageRelas.getString(j, "RelaID").equals(
										dt.getString(i, "ID"))))
							continue;
						...
						for (int k = 0; k < images.getRowCount(); k++)
							if (images.getLong(k, "ID") == ir.getID()) {
								...
								image.setSourceURL(ServerAddr + "?ImageID="
										+ ir.getID());
								...
								if (set2.size() > 0) {
									...
									image.setCatalogInnerCode(imageCatalog
											.getInnerCode());
									image.setStatus(20L);
									image.setPath("upload/Image/"
											+ CatalogUtil.getPath(imageCatalog
													.getID()));
									image.setOrderFlag(OrderUtil
											.getDefaultOrder());
									image.setSourceURL(ServerAddr + "?ImageID="
											+ ir.getID());

									int random = NumberUtil.getRandomInt(10000);
									String ext = image.getFileName().substring(
											image.getFileName()
													.lastIndexOf(".") + 1);
									String newFileName = image.getID() + random
											+ "." + ext;
									...
									if (isSameServer) {
										...
										data = Framework
												.callRemoteMethod(
														ServerAddr,
														"com.xdarkness.datachannel.InnerSyncService.sendImage",
														request);
									}
									if ("1".equals(data.getString("Success"))) {
										...
										try {
											ImageUtilEx.afterUploadImage(image,
													path);
											idmap.put("Image." + ir.getID(),
													image);
										} catch (Throwable e) {
											e.printStackTrace();
										}
				...
				tran.add(aSet, OperateType.UPDATE);
				tran.add(vSetUpdate, OperateType.UPDATE);
				tran.add(vSet, OperateType.INSERT);
			}
		...
		for (int i = 0; i < dt.getRowCount(); i++) {
			ZCArticleSchema article = new ZCArticleSchema();
			article.setValue(dt.getDataRow(i));
			String sourceID = ServerAddr + "," + article.getSiteID() + ","
			...
			for (int j = 0; j < catalogSet.size(); j++) {
				if (sourceID.equals(catalogSet.get(j).getClusterSourceID())) {
					...
					article.setCatalogInnerCode(catalogSet.get(j)
							.getInnerCode());
					flag = true;
					break;
				}
			...
			if (articleColumnValues != null) {
				for (int j = 0; j < articleColumnValues.getRowCount(); j++) {
					if ((!articleColumnValues.getString(j, "RelaType").equals(
							"2"))
							|| (!articleColumnValues.getString(j, "RelaID")
									.equals(dt.getString(i, "ID"))))
						continue;
					ZDColumnValueSchema v = new ZDColumnValueSchema();
					v.setValue(articleColumnValues.getDataRow(j));
					v.setID(NoUtil.getMaxID("ColumnValueID"));
			...
			for (int j = 0; j < imageRelas.getRowCount(); j++) {
				if ((!imageRelas.getString(j, "RelaType")
						.equals("ArticleImage"))
						|| (!imageRelas.getString(j, "RelaID").equals(
								dt.getString(i, "ID"))))
					continue;
				...
				for (int k = 0; k < images.getRowCount(); k++)
					if (images.getLong(k, "ID") == ir.getID()) {
						...
						if (set2.size() > 0) {
							...
							int random = NumberUtil.getRandomInt(10000);
							String ext = image.getFileName().substring(
									image.getFileName().lastIndexOf(".") + 1);
							String newFileName = image.getID() + random + "."
									+ ext;
							...
							if ("1".equals(data.getString("Success"))) {
								String str = data.getString("Data");
								byte[] bs = XString.base64Decode(str);
								String path = SiteUtil.getAbsolutePath(catalog
										.getSiteID())
										+ image.getPath();
								new File(path).mkdirs();
								image.setFileName(newFileName);
		...
		tran.add(aSet, OperateType.INSERT);
		tran.add(vSet, OperateType.INSERT);
		tran.add(rSet, OperateType.INSERT);
		tran.add(iSet, OperateType.INSERT);
		tran.commit();
	}

	public static String dealImageTag(String html, Mapx idmap,
		...
		while (m.find(lastIndex)) {
			...
			Mapx attrs = XString.splitToMapx(tag, " ", "=");
			...
			if (image != null) {
				...
				String src = SiteUtil.getPath(catalog.getSiteID()) + "/"
...
/*
 * com.xdarkness.datachannel.InnerSyncService JD-Core Version: 0.6.0
 */