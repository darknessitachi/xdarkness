...
import com.xdarkness.search.ZvingAnalyzer;
...
public class CrawlIndexer {
	...
	protected static QueryParser qp1 = new QueryParser("CONTENT",
			queryAnalyzer1);
	...
	public static void index(long id) {
		try {
			...
			while (wdoc != null) {
				...
				if (percent % 100L == 0L) {
					LogUtil.info("Indexing Html Files, Percent : " + percent
							/ 100L + " %");
				}
	...
	public static SearchResult search(long crawlID, String keyword,
	...
	public static SearchResult search(long crawlID, String keyword,
			int pageSize, int pageIndex, Mapx map) {
		try {
			long t = System.currentTimeMillis();
			if (searcher == null) {
			...
			if (orderType.equals("Date"))
				hits = searcher.search(q, new Sort(new SortField("DATE", 5,
						!"AES".equals(orderType))));
			else {
			...
			for (int i = start; (i < start + pageSize) && (i < hits.length()); i++) {
				...
				TokenStream tokenStream = queryAnalyzer1.tokenStream("TITLE",
				...
				content = content.replaceAll("\\>", "&lt;");
				tokenStream = queryAnalyzer1.tokenStream("CONTENT",
...
/*
 * com.xdarkness.search.crawl.CrawlIndexer JD-Core Version: 0.6.0
 */