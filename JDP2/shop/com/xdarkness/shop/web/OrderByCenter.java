...
public class OrderByCenter extends Ajax {
	...
	public static void dg2DataBind(DataListAction dla) {
		...
		DataTable sendTypeDT = new QueryBuilder(
				"select name,price from zssend order by id").executeDataTable();
		dr.insertColumn("SendPrice", sendTypeDT.get(0, "price"));
		dr.insertColumn("PayPrice", fmt.format(Float.parseFloat(sendTypeDT
				.getString(0, "price"))
				+ sumPrices));
		dt.insertRow(dr);
		dla.bindData(dt);
	...
	public static void dg1PrintDataBind(DataGridAction dga) {
		...
		DataTable dt = new QueryBuilder(sql1, dga.getParam("OrderID"))
				.executeDataTable();
		Mapx factoryMap = new QueryBuilder(
				...
				dga.getParam("OrderID")).executeDataTable().toMapx(0,
				1);
		dt.insertColumn("Factory");
		for (int i = 0; (dt != null) && (i < dt.getRowCount()); i++) {
			dt.set(i, "Factory", factoryMap.getString(dt
					.getString(i, "GoodsID")));
		}
		dga.bindData(dt);
	}
	...
	public static Mapx init(Mapx params) {
		params.put("IsValid", HtmlUtil.codeToRadios("IsValid", "Order.IsValid",
				"Y"));
		params.put("HasInvoice", HtmlUtil.codeToRadios("HasInvoice",
				"Order.HasInvoice", "Y"));
		params.put("Status", HtmlUtil.codeToOptions("Order.Status"));
		params.put("SendTimeSlice", HtmlUtil
				.codeToOptions("Order.SendTimeSlice"));
		DataTable sendTypeDT = new QueryBuilder(
				"select name, id , price, info from zssend where siteid = ? order by id",
				params.getString("SiteID")).executeDataTable();
		for (int i = 0; i < sendTypeDT.getRowCount(); i++) {
			sendTypeDT.set(i, "name", "<span class='sendName'>"
					+ sendTypeDT.getString(i, "name")
					+ "</span>&nbsp;<span class='sendPrice'>￥"
					+ sendTypeDT.getString(i, "price")
					+ "</span>&nbsp;<span class='sendInfo'>备注："
					+ sendTypeDT.getString(i, "info") + "</span>");
		}
		sendTypeDT.deleteColumn("info");
		sendTypeDT.deleteColumn("price");
		params.put("SendTypeRadio", HtmlUtil.dataTableToRadios("SendTypeRadio",
				sendTypeDT, true));
		params.put("SendTimeSlice", HtmlUtil.codeToRadios("SendTimeSlice",
				"Order.SendTimeSlice"));
		DataTable paymentTypeDT = new QueryBuilder(
				"select name,id, info from zspayment where siteid = ? order by id",
				params.getString("SiteID")).executeDataTable();
		for (int i = 0; i < paymentTypeDT.getRowCount(); i++) {
			paymentTypeDT.set(i, "name", "<span class='paymentName'>"
					+ paymentTypeDT.getString(i, "name")
					+ "</span>&nbsp;<span class='paymentInfo'>备注："
					+ paymentTypeDT.getString(i, "info") + "</span>");
		}
		params.put("PaymentTypeRadio", HtmlUtil.dataTableToRadios(
				"PaymentTypeRadio", paymentTypeDT, true));
		return params;
	}
	...
	public static Mapx initOrderResult(Mapx params) {
		...
		if (ID != null) {
			...
			params.put("HasInvoice", HtmlUtil.codeToMapx("Order.HasInvoice")
					.getString(order.getHasInvoice()));
			Mapx sendTypeMap = new QueryBuilder(
					"select name,id from zssend order by id")
					.executeDataTable().toMapx(1, 0);
			Mapx paymentTypeMap = new QueryBuilder(
					"select name,id from zspayment order by id")
					.executeDataTable().toMapx(1, 0);
			params.put("SendType", sendTypeMap.getString(order.getSendType()));
			params.put("SendTimeSlice", CacheManager.get("Code",
					"Order.SendTimeSlice", order.getSendTimeSlice()));
			params.put("PaymentType", paymentTypeMap.getString(order
					.getPaymentType()));
			Mapx districtMap = new QueryBuilder(
					"select code,name from zddistrict").executeDataTable()
					.toMapx(0, 1);
			...
			params.put("District", districtMap.getString(params
					.getString("District")));
	...
	public void add() {
		...
		if ($V("SendAddr").equals("Other")) {
			...
			addr.setUserName(XString.isNotEmpty(User.getUserName()) ? User
					.getUserName() : UserName);
			addr.setAddUser(XString.isNotEmpty(User.getUserName()) ? User
					.getUserName() : UserName);
			addr.setAddTime(new Date());
			trans
					.add(new QueryBuilder(
							"update zdmemberaddr set IsDefault = 'N' where UserName = ?",
							XString.isNotEmpty(User.getUserName()) ? User
									.getUserName() : UserName));
			trans.add(addr, OperateType.INSERT);
		}
		order.setAddUser(XString.isNotEmpty(User.getUserName()) ? User
				.getUserName() : UserName);
		...
		trans.add(order, OperateType.INSERT);
		...
		for (int i = 0; i < ids.length; i++) {
			...
			if (goods.fill()) {
				...
				orderItem.setDiscountPrice(fmt.format(goods.getPrice()
						* orderItem.getDiscount()));
				orderItem.setCount(map.getInt(ids[i]));
				orderItem.setAmount((float) orderItem.getCount()
				...
				trans.add(orderItem, OperateType.INSERT);
			}
		}
		if (trans.commit()) {
...
/*
 * com.xdarkness.shop.web.OrderByCenter JD-Core Version: 0.6.0
 */