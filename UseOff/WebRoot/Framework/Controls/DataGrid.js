...
DataGrid.onRowClick = function(ele,evt){
	...
	var multiSelect = dg.$A("multiSelect")!="false";
	/*如果不是按住ctrl键或者表格不允许多选
	  单击的是行选择器，则跳过，否则将所有行设置为非选中状态*/
...
DataGrid.onSort = function(ele){
	var direction = (ele.getAttribute("direction") || "").toUpperCase();
	if(direction =="ASC"){
		direction = "";
	}else	if(direction==""){
		direction = " DESC";
	...
	sortString = tmp.join() || Constant.Null;
...
DataGrid.init = function(ele){
	...
	/*if(Browser.isGecko){//禁止选择,IE下在DataGridAction中实现
...
DataGrid.getColumnInfo = function(ele){
	...
	for(var i=0;i<headTR.cells.length;i++){
		...
		var xtype = td.$A("xtype");
		xtype = xtype?xtype.toLowerCase():null;
		if(xtype=="rowno"){
			continue;
		}
		if(xtype=="selector"){
		...
		if(td.innerText){//没有标题的列不要
			var w = Browser.isIE?td.currentStyle.width:td.offsetWidth;//$E.getDimensions()在单元格宽度为百分比值时有误
			w = w+"";
			if(w.indexOf("%")>0){
...
DataGrid.toExcelSubmit = function(ele,toExcelPageFlag,columnIndexes,columnWidths,rows){
	...
	var input = $(xls+"_SKY_ToExcelPageFlag") || document.createElement("input");
	input.type = "hidden";
	input.id =xls+"_SKY_ToExcelPageFlag";
	input.name = xls+"_SKY_ToExcelPageFlag";
	input.value= toExcelPageFlag? "1":"0";
	f.appendChild(input);

	input = $(xls+"_SKY_Widths");
	if(!input){
		input  = document.createElement("input");
	}
	input.type="hidden";
	input.id =xls+"_SKY_Widths";
	input.name = xls+"_SKY_Widths";
	input.value = columnWidths.join();
	f.appendChild(input);
	
	input = $(xls+"_SKY_Indexes") || document.createElement("input");;
	input.type="hidden";
	input.id =xls+"_SKY_Indexes";
	input.name = xls+"_SKY_Indexes";
	input.value = columnIndexes.join();
	f.appendChild(input);
	
	if(rows){
		input = $(xls+"_SKY_Rows") || document.createElement("input");
		input.type="hidden";
		input.id =xls+"_SKY_Rows";
		input.name = xls+"_SKY_Rows";
...
DataGrid.loadData = function(ele,func,dc){
	...
	try{
		...
		Server.sendRequest("com.abigdreamer.java.net.jaf.controls.grid.DataGridPage.doWork",dc,function(response){
		
try{
	var dg = ele.getParentByAttr("xtype","_DataGrid");
			if(dg){
				ele = dg;
			}
			if(!ele.parentNode){
			...
			if(DataGrid.getParam(id,"_SKY_SCROLL")=="true"){				
				ele = ele.getParentByAttr("xtype","_DataGridWrapper");
				ele.outerHTML =  response.get("HTML");
				newEle = $(id);
			}else{
				newEle = document.createElement("div");
				newEle.setAttribute("xtype","_DataGrid");
...
/*自动用空白行撑开DataGrid*/
DataGrid.autoFillBlank = function(ele){
	ele = $(ele);
	if(ele.$A("autoFill")!="false"){
		...
		for(var i=ele.rows.length-1;i>0;i--){
			if(ele.rows[i].getAttribute("xtype")=="blank"){
				if(size<=0){
					$(ele.rows[i]).hide();
				}else{
					$(ele.rows[i]).show();
...
DataGrid.deleteRow = function(ele,index){
	...
	if(index==1){
		...
		for(var i=0;i<tr.cells.length;i++){
			var xtype = tr.cells[i].getAttribute("xtype");
			if(xtype&&xtype.toLowerCase()=="rowno"){
				cellIndex = i;
				break;
			}
		}
...
/*在增、删行后重设行号和选择器顺序*/
DataGrid.resetRowEx = function(ele,rowNo){
		ele = $(ele);
		var cellIndex = null;
		var tr = ele.rows[0];
		for(var i=0;i<tr.cells.length;i++){
...
/*改变编辑状态*/
DataGrid.changeStatus = function(ele,type){
	ele = $(ele);
	var dg = ele.parentNode.parentNode;
	...
	try{
		for(var i=0;i<dg.rows.length;i++){
			var row = dg.rows[i];
			if(row.getAttribute("xtype")=="edit"){
				editTR = row;
			}
			if(row.getAttribute("xtype")=="template"){
				templateTR = row;
			}
			if(row.getAttribute("xtype")=="head"){
				headTR = row;
			}
		}
		if(!headTR){
	...
	for(var i=0;i<headTR.cells.length;i++){
		...
		var xtype = cell.getAttribute("xtype");
		var field = cell.getAttribute("field");
		var modifyFlag = false;
		if(xtype&&xtype.toLowerCase()=="selector"){
			$(dg.id+"_RowCheck"+ele.rowIndex).disabled = ele.EditStatus?true:false;
		}else if(xtype&&xtype.toLowerCase()=="rowno"){
			ele.cells[i].innerHTML = ele.cells[i].getAttribute("rowno");
		}else if(xtype&&xtype.toLowerCase()=="checkbox"){
			var checkedvalue = ele.cells[i].getAttribute("checkedvalue") || "Y";;
			if(v1){
		...
		}else if(xtype&&xtype.toLowerCase()=="dropdownlist"){
			if(v1){
				modifyFlag = v1[ds.ColMap[field.toLowerCase()]]!=dr.get(field);
			}
			...
			var arr = [];
			if(xtype&&xtype.toLowerCase()=="tree"){
...
/*供DataGrid之外的按钮调用*/
DataGrid.edit = function(event,ele){
	ele = $(ele);
	if(!ele){
...
DataGrid.dragOver = function(evt,ele){
	...
	for(var i=0;i<row.cells.length;i++){
		...
		var style = Browser.isGecko?document.defaultView.getComputedStyle(cell, null):cell.currentStyle;
