...
DataList.loadData = function(ele,func){
	ele = $(ele);
	var id  = ele.id;
	Server.sendRequest("com.abigdreamer.java.net.jaf.controls.DataListPage.doWork",ele.Params,function(response){
		var html = response.get("HTML");
		var p = ele.parentNode;
		while(p){
			if(p.tagName&&p.tagName!="TR"&&p.tagName!="TABLE"&&p.tagName!="TBODY"){
				var phtml = p.innerHTML;
				var i1 = phtml.indexOf("<!--_SKY_DATALIST_START_" + id + "-->");
				var i2 = phtml.indexOf("<!--_SKY_DATALIST_END_" + id + "-->");
