...
Selector.initHtml = function(ele){
	...
	if(len>0||ele.innerHTML.trim()==""){
		...
		var arr = [];
		arr.push("<input type='text' id='"+id+"_textField' xtype='select' "+verifyStr+" "+classStr+" name='"+name+"' autocomplete='off' style='height:18px;line-height:18px;_line-height:12px; padding:0;padding-top:2px;vertical-align:top;border:0 none; background:transparent none; cursor:default;"+(isIE8?"line-height:18px;":"")+styleText+"' value=''/>");
		...
		if(html.indexOf("_SKY_NOID_")>0){
			html = html.replace("_SKY_NOID_",id);
		}
		ele.innerHTML = html;
	}else{
		...
		if(html.indexOf("_SKY_NOID_")>0){
			ele.innerHTML = html.replace("_SKY_NOID_",id);
		}
	}
...
Selector.loadData = function(ele,func){
	...
	Server.sendRequest("com.abigdreamer.java.net.jaf.controls.CodeSourcePage.getData",ele.Params,function(response){
