...
Constant.TreeLevel = "_SKY_TREE_LEVEL";
Constant.TreeStyle = "_SKY_TREE_STYLE";
...
Tree.onMouseOver = function(evt){
	...
	var xtype = ele.getAttribute("xtype");
	if(xtype&&xtype.toLowerCase()=="rootmenu"){
		return;
	}else{
		ele.className = 'over';	
	}
...
Tree.loadData = function(ele,func){
	...
	Server.sendRequest("com.abigdreamer.java.net.jaf.controls.TreePage.doWork",ele.Params,function(response){
		var newEle = document.createElement("div");
		newEle.setAttribute("xtype","_Tree");
		newEle.innerHTML = response.get("HTML");
		var tree = $E.getParentByAttr(ele,"xtype","_Tree");
		if(tree){
			ele = tree;
		}
...
Tree.lazyLoad = function(ele,func){
	ele = $(ele);
	var tree = ele.getParentByAttr("xtype","_Tree");
	...
	Server.sendRequest("com.abigdreamer.java.net.jaf.controls.TreePage.doWork",tree.Params,function(response){
