...
function getCallback(element, interval, callback){
	...
	}catch(e){// todo need check to rewrite
		try{
			element.document.getElementById(element.id).parentElement.removeChild(element.document.getElementById(element.id));
		}catch(e){
		}
	}
};
  ...
  ele = $(ele);
	if(!ele.InitCtrlStyleFlag){//避免多次初始化
	  ...
	  case '':
	  	...
	  	if(ele.outerHTML.indexOf('readOnly') != -1) {
	  		ele.addClassName("inputReadonlyText");
	  	} else {
	  ...
	  case 'button':
	  	if(ele.className==""&&!(/border|background/).test(ele.style.cssText)){//在没有定义class或没有定义background的情况下对按钮应用圆角样式
			...
			if (ele.parentElement.tagName != "A") {
			  ele.outerHTML = "<a href='javascript:void(0);' xtype='zInputBtnWrapper' class='zInputBtn' hidefocus='true' tabindex='-1'>" + ele.outerHTML + "<\/a>";
			}
		}
	    break;
	  case 'checkbox':
...
Effect.initChildren = function(ele){
	...
	arr.each(function(div){
		var xtype = $(div).$A("xtype");
		if(xtype&&xtype.toLowerCase()=="select"){
			Selector.initHtml(div);
			//sarr.push(div);
		}
	});
	/*
	sarr.each(function(ele){debugger;
		try{
 ...
 * Effect.ie6ScrollFix=function(){
	if(window.frameElement && Browser.isIE6 && !Browser.isQuirks){
